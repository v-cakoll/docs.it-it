...
Nelle versioni precedenti di .NET Core, era necessario eseguire il comando [dotnet restore](../tools/dotnet-restore.md) per scaricare le dipendenze immediatamente dopo aver creato un nuovo progetto con il comando [dotnet new](../tools/dotnet-new.md), nonché in seguito all'aggiunta di una nuova dipendenza al progetto. In .NET Core 2.0 il comando `dotnet restore` viene eseguito in modo implicito quando si esegue il comando `dotnet new`. Viene eseguito in modo implicito anche se è necessario aggiornare le dipendenze in seguito all'esecuzione di altri comandi, ad esempio `run`, `build` e `publish`.
...
## <a name="language-support"></a>Supporto dei linguaggi
...
Da [.NET Application Architecture](https://www.microsoft.com/net/learn/architecture) (Architettura delle applicazioni .NET) è possibile accedere a una serie di eBook che offrono linee guida, procedure consigliate e applicazioni di esempio per l'uso di .NET per realizzare:
