---
title: Docker e .NET Core
description: Informazioni sull'uso di Docker con .NET Core usando esercitazioni divertenti.
author: jralexander
ms.author: johalex
ms.date: 01/10/2018
ms.openlocfilehash: e31a3a221604e7aca7d35a4826e78ade0f980a6b
ms.sourcegitcommit: 3d5d33f384eeba41b2dff79d096f47ccc8d8f03d
ms.translationtype: HT
ms.contentlocale: it-IT
ms.lasthandoff: 05/04/2018
ms.locfileid: "33219297"
---
# <a name="docker-and-net-core"></a>Docker e .NET Core 

Le esercitazioni seguenti includono informazioni e istruzioni sull'uso di Docker con .NET Core.

- [Introduzione a .NET e Docker](intro-net-docker.md)
- [Apprendere le nozioni di base di Docker con .NET Core](docker-basics-dotnet-core.md)
- [Compilazione di immagini Docker per applicazioni .NET Core](building-net-docker-images.md)
- [Visual Studio Tools per Docker](/aspnet/core/host-and-deploy/docker/visual-studio-tools-for-docker)

Per esercitazioni sullo sviluppo di applicazioni Web ASP.NET Core, vedere la [documentazione di ASP.NET Core](/aspnet/core/).
