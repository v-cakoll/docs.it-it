...
Questo articolo illustra le modifiche che influiscono sulla compatibilità e il modo in cui il team .NET valuta ogni tipo di modifica. Conoscere il modo in cui il team .NET si avvicina alle possibili modifiche di rilievo è particolarmente utile per gli sviluppatori che aprono richieste pull che modificano il comportamento delle [API .NET esistenti](https://github.com/dotnet/runtime).

Le sezioni seguenti descrivono le categorie di modifiche apportate alle API .NET Core e il loro effetto sulla compatibilità delle applicazioni. Le modifiche sono consentite ✔️, non consentite ❌ o richiedono il giudizio e una valutazione del comportamento prevedibile, ovvio e coerente del comportamento precedente ❓.

> [!NOTE]
>
> - Oltre a fungere da guida per il modo in cui vengono valutate le modifiche alle librerie .NET, gli sviluppatori di librerie possono usare questi criteri anche per valutare le modifiche apportate alle librerie destinate a più implementazioni e versioni di .NET.
> - Per informazioni sulle categorie di compatibilità, ad esempio la compatibilità con le versioni successive e precedenti, vedere [compatibilità](categories.md).
...
- ✔️ **consentito: trasferimento di un tipo da un assembly a un altro**
  ...
  L'assembly *precedente* deve essere contrassegnato con l'oggetto <xref:System.Runtime.CompilerServices.TypeForwardedToAttribute> che punta al nuovo assembly.

- ✔️ **consentito: modifica di un tipo [struct](../../csharp/language-reference/builtin-types/struct.md) in un `readonly struct` tipo**

  La modifica `readonly struct` di un tipo in un `struct` tipo non è consentita.
...
- ❌Non **consentito: modifica di un `readonly struct` tipo in un tipo [struct](../../csharp/language-reference/builtin-types/struct.md) **

  `struct` `readonly struct` Tuttavia, è consentita la modifica di un tipo in un tipo.

- ❌Non **consentito: modifica di un tipo [struct](../../csharp/language-reference/builtin-types/struct.md) in un `ref struct` tipo e viceversa**
...
- ✔️ **consentito: modifica da `ref readonly` a a un `ref` valore restituito (ad eccezione di metodi o interfacce virtuali)**
...
- ❌Non **consentito: passaggio da un `ref` valore restituito a un `ref readonly` valore restituito**

- ❌️Non **consentito: passaggio da `ref readonly` a a un `ref` valore restituito su un metodo o un'interfaccia virtuale**
...
- ❌Non **consentito: creazione di un membro virtuale astratto**
...
- ❌️Non **consentito: aggiunta di [ReadOnly](../../csharp/language-reference/keywords/readonly.md) a un campo**
...
- ❌Non **consentito: modifica dei valori restituiti per una proprietà, un campo, un valore restituito di un metodo o un parametro [out](../../csharp/language-reference/keywords/out-parameter-modifier.md) **
...
- ❌**Non consentito: rimozione di un'eccezione in qualsiasi altro caso non elencato sopra**

### <a name="attributes"></a>Attributi

- ✔️ **consentito: modifica del valore di un attributo *non* osservabile**

- ❌Non **consentito: modifica del valore di un attributo osservabile *is* **
...
- ❌Non **consentito: modifica del numero di volte in cui vengono chiamati gli eventi specificati**

- ❌Non **consentito: aggiunta <xref:System.FlagsAttribute> di a un tipo di enumerazione**
