...
ms.locfileid: "70926375"
---
# <a name="understanding-systemruntimeloaderassemblyloadcontext"></a>Informazioni su System. Runtime. loader. AssemblyLoadContext

La <xref:System.Runtime.Loader.AssemblyLoadContext> classe è univoca per .NET Core. Questo articolo tenta di integrare la <xref:System.Runtime.Loader.AssemblyLoadContext> documentazione dell'API con informazioni concettuali.
...
È il provider del runtime per l'individuazione e il caricamento delle dipendenze. Ogni volta che viene caricata una <xref:System.Runtime.Loader.AssemblyLoadContext> dipendenza, viene richiamata un'istanza per individuarla.
...
- Per supportare il caricamento e lo scaricamento di codice dinamico, viene creato un contesto isolato per il caricamento del codice e delle <xref:System.Runtime.Loader.AssemblyLoadContext> relative dipendenze nella propria istanza.
...
Una singola <xref:System.Runtime.Loader.AssemblyLoadContext> istanza è limitata al caricamento di una sola versione <xref:System.Reflection.Assembly> di <xref:System.Reflection.AssemblyName.Name?displayProperty=nameWithType>per nome di assembly semplice.

Questa restrizione può costituire un problema durante il caricamento dinamico dei moduli di codice. Ogni modulo è compilato in modo indipendente e può dipendere da versioni diverse <xref:System.Reflection.Assembly>di. Questo problema si verifica in genere quando diversi moduli dipendono da versioni diverse di una libreria di uso comune.

Per supportare il caricamento dinamico del codice, <xref:System.Runtime.Loader.AssemblyLoadContext> l'API consente di caricare versioni in conflitto di <xref:System.Reflection.Assembly> un nella stessa applicazione. Ogni <xref:System.Runtime.Loader.AssemblyLoadContext> istanza fornisce un dizionario univoco per ogni <xref:System.Reflection.AssemblyName.Name?displayProperty=nameWithType> istanza di <xref:System.Reflection.Assembly> .
...
L' <xref:System.Runtime.Loader.AssemblyLoadContext.Default?displayProperty=nameWithType> istanza viene popolata automaticamente dal runtime all'avvio.  Usa il [Probe predefinito](default-probing.md) per individuare e trovare tutte le dipendenze statiche.
...
<xref:System.Runtime.Loader.AssemblyLoadContext>in sono disponibili vari eventi e funzioni virtuali di cui è possibile eseguire l'override.

L' <xref:System.Runtime.Loader.AssemblyLoadContext.Default?displayProperty=nameWithType> istanza supporta solo l'override degli eventi.
...
Ogni <xref:System.Runtime.Loader.AssemblyLoadContext> istanza rappresenta un ambito univoco per <xref:System.Reflection.Assembly> le istanze <xref:System.Type> e le definizioni.
...
- <xref:System.Reflection.AssemblyName.Name?displayProperty=nameWithType>può fare riferimento a un' <xref:System.Reflection.Assembly> istanza diversa.
- <xref:System.Type.GetType%2A?displayProperty=nameWithType>può restituire un'istanza di tipo diversa per lo stesso `name`tipo.

## <a name="how-are-dependencies-shared"></a>Come vengono condivise le dipendenze?

Le dipendenze possono essere facilmente <xref:System.Runtime.Loader.AssemblyLoadContext> condivise tra le istanze. Il modello generale è quello <xref:System.Runtime.Loader.AssemblyLoadContext> di caricare una dipendenza.  L'altra condivide la dipendenza utilizzando un riferimento all'assembly caricato.

Questa condivisione è necessaria per gli assembly di Runtime. Questi assembly possono essere caricati solo in <xref:System.Runtime.Loader.AssemblyLoadContext.Default?displayProperty=nameWithType>. Lo stesso è necessario per i Framework come `ASP.NET`, `WPF`o `WinForms`.
...
La condivisione viene implementata nel codice dell'istanza <xref:System.Runtime.Loader.AssemblyLoadContext> personalizzata. <xref:System.Runtime.Loader.AssemblyLoadContext>in sono disponibili vari eventi e funzioni virtuali di cui è possibile eseguire l'override. Quando una di queste funzioni restituisce un riferimento a un' <xref:System.Reflection.Assembly> istanza di caricata in un' <xref:System.Runtime.Loader.AssemblyLoadContext> altra istanza, <xref:System.Reflection.Assembly> l'istanza di viene condivisa. L'algoritmo Load standard rinvia a <xref:System.Runtime.Loader.AssemblyLoadContext.Default?displayProperty=nameWithType> per il caricamento per semplificare il modello di condivisione comune.  Vedere [algoritmo di caricamento assembly gestito](loading-managed.md).

## <a name="complications"></a>Complicazioni

### <a name="type-conversion-issues"></a>Problemi di conversione dei tipi

Quando due <xref:System.Runtime.Loader.AssemblyLoadContext> istanze contengono definizioni di tipi con lo `name`stesso, non sono dello stesso tipo. Si tratta dello stesso tipo se e solo se provengono dalla stessa <xref:System.Reflection.Assembly> istanza.
...
- Ogni tipo <xref:System.Runtime.Loader.AssemblyLoadContext>, che può essere ottenuto tramite la <xref:System.Runtime.Loader.AssemblyLoadContext.GetLoadContext(System.Reflection.Assembly)?displayProperty=nameWithType> funzione.
