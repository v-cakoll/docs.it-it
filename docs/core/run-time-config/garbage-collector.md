...
- Se l'affinità del processore GC è abilitata, che `n` è l'impostazione predefinita, l'impostazione del numero di heap affina gli heap/thread GC ai primi `n` processori. Utilizzare le impostazioni affinitize mask o affinitize ranges per specificare esattamente i processori di cui eseguire l'affinità.
- Se l'affinità del processore GC è disabilitata, questa impostazione limita il numero di heap GC.
...
- Se l'affinità del `System.GC.NoAffinitize` `true`processore è disabilitata impostando su , questa impostazione viene ignorata.
- Si applica solo alla procedura di Garbage Collection per server.
...
- Questa impostazione `System.GC.HeapAffinitizeMask`è simile a , ad eccezione del fatto che consente di specificare più di 64 processori.
