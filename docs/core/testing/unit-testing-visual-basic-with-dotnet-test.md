...
Tornare alla directory *unit-testing-vb-using-dotnet-test*. Esegui [`dotnet sln add .\PrimeService\PrimeService.vbproj`](../tools/dotnet-sln.md) per aggiungere il progetto di libreria di classi alla soluzione.
...
Impostare la directory *PrimeService.Tests* come directory [`dotnet new xunit -lang VB`](../tools/dotnet-new.md)corrente e creare un nuovo progetto utilizzando . Questo comando crea un progetto di test che usa xUnit come libreria di test. Il modello generato configura il Test Runner nel file *PrimeServiceTests.vbproj*:
...
Per creare ed eseguire unit test, il progetto di test richiede altri pacchetti. Nel passaggio precedente `dotnet new` ha aggiunto xUnit e il Runner di xUnit. Aggiungere ora la libreria di classi `PrimeService` come un'altra dipendenza del progetto. Utilizzare [`dotnet add reference`](../tools/dotnet-add-reference.md) il comando:
