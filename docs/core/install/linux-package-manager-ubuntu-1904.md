...
Questo articolo descrive come usare un gestore di pacchetti per installare .NET Core in Ubuntu 19,04. Se si sta installando il runtime, si consiglia di installare il [runtime di ASP.NET Core](#install-the-aspnet-core-runtime), perché include sia .NET Core che ASP.NET Core Runtime.
