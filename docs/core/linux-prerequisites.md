...
.NET Core 1.x è supportato nelle seguenti distribuzioni/versioni Linux:
...
> [!NOTE]
> Ubuntu/Linux 16.10 è supportato dalla versione patch più recente di .NET Core 1.1
...
## <a name="installing-net-core-prerequisites-with-the-native-installers"></a>Installazione di prerequisiti di .NET Core con i programmi di installazione nativi

I programmi di installazione .NET Core nativi sono disponibili per le distribuzioni/versioni Linux supportate. I programmi di installazione richiedono l'accesso amministratore (sudo) al server. L'uso di un programma di installazione nativo offre il vantaggio di installare tutte le dipendenze native .NET Core. I programmi di installazione nativi installano.NET Core SDK a livello di sistema.
...
```
---
4. Eseguire il comando `dotnet --help` per provare se l'installazione ha avuto esito positivo.
...
## <a name="install-net-core-for-ubuntu-1404-1604-1610--linux-mint-17-18-64-bit"></a>Installare .NET Core per Ubuntu 14.04, 16.04, 16.10 e Linux Mint 17, 18 (64 bit)

> [!Warning]
> Prima di iniziare, rimuovere eventuali versioni precedenti di .NET Core dal sistema.

### <a name="add-the-dotnet-apt-get-feed"></a>Aggiungere il feed apt-get dotnet

1. Impostare il feed di pacchetti host della versione desiderata.
   ...
   **Ubuntu 14.04/Linux Mint 17**
    ...
    **Ubuntu 16.10**
    ...
    sudo apt-get update
    ```
2. Installare .NET Core.
# <a name="net-core-2xtabnetcore2x"></a>[.NET Core 2.x](#tab/netcore2x)

Dopo l'installazione del feed di pacchetti host, installare .NET Core 2.0.x in Ubuntu o Linux Mint:
```bash
sudo apt-get install dotnet-sdk-2.0.0
```
# <a name="net-core-1xtabnetcore1x"></a>[.NET Core 1.x](#tab/netcore1x)

Dopo l'installazione del feed di pacchetti host, installare .NET Core 1.x in Ubuntu o Linux Mint:
```bash
sudo apt-get install dotnet-dev-1.0.4
```
---
3. Eseguire il comando `dotnet --help` per provare se l'installazione ha avuto esito positivo.

 ```bash
     dotnet --help
 ```
## <a name="install-net-core-for-debian-8-or-9-64-bit"></a>Installare .NET Core per Debian 8 o 9 (64 bit).

> [!Warning]
> Prima di iniziare, rimuovere eventuali versioni precedenti di .NET Core dal sistema.

Per installare .NET Core in Debian 8 o 9 (64 bit):
1. Ottenere i prerequisiti. 
    ```bash
    sudo apt-get install curl libunwind8 gettext
    ```
2. Scaricare i binari di .NET Core SDK (tarball).
...
```
---
3. Estrarre i binari di .NET Core SDK.
    ```bash
...
4. Aggiungere dotnet al PERCORSO.
    ...
    ```
5. Eseguire il comando `dotnet --help` per provare se l'installazione ha avuto esito positivo.

     ```bash
     dotnet --help
     ```

## <a name="install-net-core-for-fedora-24-25-or-26-64-bit"></a>Installare .NET Core per Fedora 24, 25 o 26 (64 bit).

> [!Warning]
> Prima di iniziare, rimuovere eventuali versioni precedenti di .NET Core dal sistema.

Per installare .NET Core per Fedora 26, 25 (.NET Core 2.x) o 24 (.NET Core 1.x): 
1. Ottenere i prerequisiti. 
    ```bash
    sudo dnf install libunwind libicu
    ```
2. Scaricare il binario di .NET Core SDK (tarball).
...
5. Eseguire il comando `dotnet --help` per provare se l'installazione ha avuto esito positivo.
     ...
     dotnet --help
     ```
## <a name="install-net-core-for-suse-linux-enterprise-server-64-bit-net-core-2x-and-opensuse-64-bit"></a>Installare .NET Core per SUSE Linux Enterprise Server (64 bit) (.NET Core 2. x) e openSUSE (64 bit)

> [!Warning]
> Prima di iniziare, rimuovere eventuali versioni precedenti di .NET Core dal sistema.

Per installare .NET Core per SUSE Linux Enterprise Server (SLES) 12 SP2 (64 bit) e openSUSE 42.2 in NET Core 2.x or openSUSE 42.1(64 bit) in .NET Core 1.x:
