...
Lo [strumento di disinstallazione di .NET Core](https://github.com/dotnet/cli-lab/releases) (`dotnet-core-uninstall`) consente di rimuovere gli SDK e i runtime di .NET Core da un sistema. È disponibile una raccolta di opzioni per specificare quali versioni si desidera disinstallare.
...
A causa di queste limitazioni, lo strumento potrebbe non essere in grado di disinstallare tutti gli SDK e i runtime di .NET Core nel computer. È possibile usare il comando `dotnet --info` per trovare tutti gli SDK e i runtime di .NET Core installati, inclusi gli SDK e i runtime che questo strumento non è in grado di rimuovere. Il comando `dotnet-core-uninstall list` Visualizza gli SDK che è possibile disinstallare con lo strumento.

## <a name="install-the-tool"></a>Installare lo strumento

È possibile scaricare lo strumento di disinstallazione di .NET Core dal repository GitHub [DotNet/CLI-Lab](https://github.com/dotnet/cli-lab/releases) .

> [!NOTE]
> Per disinstallare SDK e runtime di .NET Core, lo strumento richiede l'elevazione dei privilegi. Pertanto, deve essere installato in una directory protetta da scrittura, ad esempio *c:\Programmi* in Windows o */usr/local/bin* in MacOS. Vedere anche [accesso con privilegi elevati per i comandi DotNet](../tools/elevated-access.md). Le istruzioni dettagliate per l'installazione sono disponibili nella [pagina delle versioni di GitHub](https://github.com/dotnet/cli-lab/releases).
...
Il comando `dotnet-core-uninstall list` elenca gli SDK di .NET Core installati e i runtime che possono essere rimossi con questo strumento. Alcuni SDK e Runtime possono essere richiesti da Visual Studio e vengono visualizzati con una nota del motivo per cui non è consigliabile disinstallarli.
...
* **`--hosting-bundle`**
  ...
  Elenca tutti i pacchetti di runtime e hosting di .NET Core che possono essere disinstallati con questo strumento.
...
I comandi `dotnet-core-uninstall dry-run` e `dotnet-core-uninstall whatif` visualizzano gli SDK e i runtime di .NET Core che verranno rimossi in base alle opzioni fornite senza eseguire la disinstallazione. Questi comandi sono sinonimi.
...
* **`VERSION`**
  ...
  > Si tratta di file di testo, in genere con un'estensione \*. rsp e ogni versione è elencata in una riga separata.
  > Per specificare un file di risposta per l'argomento `VERSION`, usare il carattere \@ immediatamente seguito dal nome del file di risposta.
...
* **`--major-minor <MAJOR_MINOR>`**
  ...
  Rimuove gli SDK di .NET Core e i runtime che corrispondono alla versione del `major.minor` specificata.
...
* **`--x64`**
  ...
  Deve essere usato con `--sdk`, `--runtime`e `--aspnet-runtime` per rimuovere gli SDK o i runtime x64.

* **`--x86`**

  Deve essere usato con `--sdk`, `--runtime`e `--aspnet-runtime` per rimuovere gli SDK o i Runtime x86.

* **`--force`** Forza la rimozione di versioni che potrebbero essere usate da Visual Studio.

Note:

1. È necessario specificare esattamente uno dei `--sdk`, `--runtime`, `--aspnet-runtime`e `--hosting-bundle`.
2. `--all`, `--all-below`, `--all-but`, `--all-but-latest`, `--all-lower-patches`, `--all-previews`, `--all-previews-but-latest`, `--major-minor`e `[<VERSION>...]` sono esclusive.
3. Se non si specifica `--x64` o `--x86`, verranno rimossi sia x64 che x86.
...
> [!NOTE]
> Per impostazione predefinita, gli SDK di .NET Core e i runtime che potrebbero essere richiesti da Visual Studio o altri SDK non sono inclusi nell'output `dotnet-core-uninstall dry-run`. Negli esempi seguenti alcuni degli SDK e dei runtime specificati potrebbero non essere inclusi nell'output, a seconda dello stato del computer. Per includere tutti gli SDK e i runtime, elencarli in modo esplicito come argomenti oppure usare l'opzione `--force`.
...
Poiché questo strumento presenta un comportamento distruttivo, è **consigliabile eseguire** un'esecuzione a secco prima di eseguire il comando Rimuovi. L'esecuzione a secco mostrerà quali SDK e runtime di .NET Core verranno rimossi quando si usa il comando `remove`. Per informazioni su quali SDK e Runtime è possibile rimuovere, fare [riferimento a.](../versions/remove-runtime-sdk-versions.md#should-i-remove-a-version)

> [!CAUTION]
> Tenere presenti le indicazioni seguenti:
>
>- Questo strumento consente di disinstallare le versioni dei .NET Core SDK richieste da `global.json` file nel computer. È possibile reinstallare gli SDK di .NET Core dalla pagina [Scarica .NET Core](https://dotnet.microsoft.com/download/dotnet-core) .
...
Per impostazione predefinita, tutti i comandi mantengono gli SDK e i Runtime .NET Core che potrebbero essere richiesti da Visual Studio o da altri SDK. Questi SDK e Runtime possono essere disinstallati elencando in modo esplicito come argomenti o usando l'opzione `--force`.

Per disinstallare SDK e runtime di .NET Core, lo strumento richiede l'elevazione dei privilegi. Eseguire lo strumento in un prompt dei comandi di amministratore in Windows e con `sudo` in macOS. I comandi `dry-run` e `whatif` non richiedono l'elevazione dei privilegi.
...
In alcuni casi non è più necessario il `NuGetFallbackFolder` e potrebbe essere opportuno eliminarlo. Per ulteriori informazioni sull'eliminazione di questa cartella, vedere [Remove the NuGetFallbackFolder](../versions/remove-runtime-sdk-versions.md#remove-the-nuget-fallback-folder).
