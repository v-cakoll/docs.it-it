...
### <a name="integrated-security-and-aspnet"></a>Sicurezza integrata e ASP.NET  
 ...
 L'identità di sicurezza delle applicazioni ASP.NET può essere impostata su diverse opzioni. Per comprendere meglio l'identità di sicurezza usata da un'applicazione ASP.NET per la connessione a SQL Server, vedere [rappresentazione ASP.NET](https://docs.microsoft.com/previous-versions/aspnet/xh507fc5(v=vs.100)), [autenticazione ASP.NET](https://docs.microsoft.com/previous-versions/aspnet/eeyk640h(v=vs.100))e [procedura: Accedere SQL Server utilizzando la sicurezza](https://docs.microsoft.com/previous-versions/aspnet/bsz5788z(v=vs.100))integrata di Windows.  
  
## <a name="connecting-to-an-ole-db-data-source"></a>Connessione a un'origine dati OLE DB  
 Il .NET Framework provider di dati per OLE DB fornisce la connettività alle origini dati esposte tramite OLE DB (tramite SQLOLEDB, il provider OLE DB per SQL Server), usando l'oggetto **OleDbConnection** .  
...
- Le parole chiave **URL**, **provider remoto**e **server remoto** non sono supportate.  
...
> [!NOTE]
> L'oggetto **OleDbConnection** non supporta l'impostazione o il recupero di proprietà dinamiche specifiche di un provider di OLE DB. Sono supportate esclusivamente le proprietà che possono essere passate nella stringa di connessione per il provider OLE DB.  
...
## <a name="connecting-to-an-odbc-data-source"></a>Connessione a un'origine dati ODBC  
 Il .NET Framework provider di dati per ODBC fornisce la connettività alle origini dati esposte tramite ODBC utilizzando l'oggetto **OdbcConnection** .  
  
 Il formato della stringa di connessione da usare con il provider di dati .NET Framework per ODBC è molto simile a quello della stringa di connessione ODBC. È anche possibile specificare il nome di un'origine dati (DSN, Data Source Name) ODBC. Per ulteriori dettagli su **OdbcConnection** , vedere <xref:System.Data.Odbc.OdbcConnection>.  
...
## <a name="connecting-to-an-oracle-data-source"></a>Connessione a un'origine dati Oracle  
 Il .NET Framework provider di dati per Oracle fornisce la connettività alle origini dati Oracle tramite l'oggetto **OracleConnection** .  
  
 Il formato della stringa di connessione da usare con il provider di dati .NET Framework per Oracle è molto simile a quello della stringa di connessione del provider OLE DB per Oracle (MSDAORA). Per ulteriori informazioni su **OracleConnection**, vedere <xref:System.Data.OracleClient.OracleConnection>.  
