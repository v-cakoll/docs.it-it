...
È possibile scrivere un oggetto binario di grandi dimensioni (BLOB) in un database come dati binari o di tipo carattere, a seconda del tipo di campo dell'origine dati. BLOB è un termine generico che fa riferimento ai tipi di dati `text`, `ntext`e `image`, che in genere contengono documenti e immagini.  
 ...
 Per scrivere un valore BLOB nel database, eseguire l'istruzione INSERT o UPDATE appropriata e passare il valore BLOB come parametro di input (vedere [Configurazione di parametri e tipi di dati di parametro](../configuring-parameters-and-parameter-data-types.md)). Se il BLOB viene archiviato come testo, ad esempio un campo `text` SQL Server, è possibile passare il BLOB come parametro di stringa. Se il BLOB viene archiviato in formato binario, ad esempio un campo `image` SQL Server, è possibile passare una matrice di tipo `byte` come parametro binario.  
