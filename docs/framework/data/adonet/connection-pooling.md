...
La connessione a un'origine dati può richiedere molto tempo. Per ridurre al minimo il costo dell'apertura delle connessioni, ADO.NET usa una tecnica di ottimizzazione denominata *il pool di connessioni*, che consente di ridurre il costo di apertura e chiusura delle connessioni ripetutamente. Per i provider di dati .NET Framework il pool di connessioni viene gestito in modo diverso.  
  
## <a name="in-this-section"></a>In questa sezione  
 [Pool di connessioni SQL Server (ADO.NET)](../../../../docs/framework/data/adonet/sql-server-connection-pooling.md)  
 Viene fornita una panoramica del pool di connessioni e viene descritto l'utilizzo del pool di connessioni in SQL Server.  
