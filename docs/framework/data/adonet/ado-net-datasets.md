...
## <a name="the-datarelationcollection"></a>DataRelationCollection  
 Un **set di dati** contiene relazioni <xref:System.Data.DataRelationCollection> nell'oggetto. Una relazione, rappresentata dall' <xref:System.Data.DataRelation> oggetto, associa le righe di una **DataTable** alle righe di un'altra **DataTable**. Una relazione è analoga a un eventuale percorso di join esistente tra colonne di chiave primaria e di chiave esterna di un database relazionale. Una **DataRelation** identifica le colonne corrispondenti in due tabelle di un **set di dati**.  
...
## <a name="linq-to-dataset"></a>LINQ to DataSet  
 LINQ to DataSet fornisce funzionalità di query integrate nel linguaggio per i dati disconnessi archiviati in un set di dati. LINQ to DataSet usa la [!INCLUDE[vbteclinq](../../../../includes/vbteclinq-md.md)] sintassi standard e fornisce il controllo della sintassi in fase di compilazione, la tipizzazione statica e il supporto IntelliSense quando si usa l'IDE di Visual Studio.  
