...
## <a name="data-service-queries"></a>Query sul servizio dati  
 ...
 Quando si utilizza il **Aggiungi riferimento al servizio** finestra di dialogo per aggiungere un servizio dati per un [!INCLUDE[dnprdnshort](../../../../includes/dnprdnshort-md.md)]-applicazione client basata sulla classe contenitore di entità viene creata da cui eredita il <xref:System.Data.Services.Client.DataServiceContext> classe. Tale classe include proprietà che restituiscono istanze della classe <xref:System.Data.Services.Client.DataServiceQuery%601> tipizzate. Esiste una proprietà per ogni set di entità esposto dal servizio dati. Tali proprietà semplificano la creazione di un'istanza di <xref:System.Data.Services.Client.DataServiceQuery%601> tipizzata.  
...
-   Quando viene chiamato un operatore di esecuzione di query LINQ, ad esempio <xref:System.Linq.Enumerable.First%2A> o <xref:System.Linq.Enumerable.Single%2A>.  
 ...
 Per ulteriori informazioni, vedere [procedura: eseguire query sul servizio di dati](../../../../docs/framework/data/wcf/how-to-execute-data-service-queries-wcf-data-services.md).  
  
 Il [!INCLUDE[ssAstoria](../../../../includes/ssastoria-md.md)] client supporta le query per gli oggetti ad associazione tardiva, ad esempio quando si usa il *dinamica* tipo in c#. Tuttavia, per preservare le prestazioni, è necessario comporre sempre query fortemente tipizzate per il servizio dati. Il tipo <xref:System.Tuple> e oggetti dinamici non sono supportati dal client.  
  
## <a name="linq-queries"></a>Query LINQ  
 Poiché il <xref:System.Data.Services.Client.DataServiceQuery%601> classe implementa il <xref:System.Linq.IQueryable%601> interfaccia definita tramite LINQ, la [!INCLUDE[ssAstoria](../../../../includes/ssastoria-md.md)] libreria client è in grado di trasformare le query LINQ sui dati del set di entità in un URI che rappresenta un'espressione di query valutata su un servizio dati risorsa. Nell'esempio seguente viene mostrata una query LINQ equivalente alla precedente classe <xref:System.Data.Services.Client.DataServiceQuery%601> che restituisce le entità `Orders` con un costo di spedizione maggiore di 30 dollari e ordina i risultati in base al costo di spedizione:  
...
## <a name="adding-query-options"></a>Aggiunta di opzioni di query  
 ...
 È possibile chiamare consecutivamente il metodo <xref:System.Data.Services.Client.DataServiceQuery%601.AddQueryOption%2A> per costruire espressioni di query complesse. Per ulteriori informazioni, vedere [procedura: aggiungere opzioni di Query da una Query del servizio dati](../../../../docs/framework/data/wcf/how-to-add-query-options-to-a-data-service-query-wcf-data-services.md).  
...
> [!NOTE]
>  Poiché un servizio dati può restituire una risposta di paging, si consiglia di gestire una risposta del servizio dati sottoposta a paging tramite un modello di programmazione dell'applicazione. Per ulteriori informazioni, vedere [durante il caricamento di contenuto posticipato](../../../../docs/framework/data/wcf/loading-deferred-content-wcf-data-services.md).  
  
 È inoltre possibile ridurre la quantità di dati restituiti da una query specificando che solo determinate proprietà di un'entità vengano restituite nella risposta. Per ulteriori informazioni, vedere [proiezioni di Query](../../../../docs/framework/data/wcf/query-projections-wcf-data-services.md).  
...
## <a name="in-this-section"></a>Contenuto della sezione  
 ...
 [Procedura: aggiungere opzioni Query a una Query del servizio dati](../../../../docs/framework/data/wcf/how-to-add-query-options-to-a-data-service-query-wcf-data-services.md)  
 ...
 [Procedura: specificare le credenziali del Client per una richiesta di servizio dati](../../../../docs/framework/data/wcf/specify-client-creds-for-a-data-service-request-wcf.md)  
 ...
 [Procedura: proiettare risultati di Query](../../../../docs/framework/data/wcf/how-to-project-query-results-wcf-data-services.md)  
