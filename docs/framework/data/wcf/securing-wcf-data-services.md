...
Questo articolo descrive le considerazioni sulla sicurezza specifiche per lo sviluppo, la distribuzione e l'esecuzione di WCF Data Services e applicazioni che accedono ai servizi che supportano l'Open Data Protocol (OData). È inoltre consigliabile seguire le indicazioni per la creazione di applicazioni .NET Framework protette.  
  
Quando si pianifica come proteggere un servizio OData basato su WCF Data Services, è necessario risolvere sia l'autenticazione, il processo di individuazione e verifica dell'identità di un'entità, sia l'autorizzazione, il processo che consente di determinare se un'entità autenticata è autorizzata ad accedere alle risorse richieste. Valutare inoltre se crittografare il messaggio tramite SSL.  
