...
description: Informazioni sui protocolli di collegamento, che supportano lo sviluppo di applicazioni che utilizzano risorse Internet indipendentemente dai dettagli del protocollo utilizzati dalle risorse.
ms.date: 03/30/2017
helpviewer_keywords:
...
## <a name="simple-requests-with-webclient"></a>Richieste semplici con WebClient  
 ...
 Per le applicazioni che non possono usare il modello di richiesta/risposta o per le applicazioni che devono restare in ascolto sulla rete e inviare richieste, lo spazio dei nomi **System .NET. Sockets** fornisce le <xref:System.Net.Sockets.TcpClient> <xref:System.Net.Sockets.TcpListener> classi, e <xref:System.Net.Sockets.UdpClient> . Queste classi gestiscono i dettagli dell'esecuzione di connessioni tramite protocolli di trasporto diversi ed espongono la connessione di rete all'applicazione come flusso.  
