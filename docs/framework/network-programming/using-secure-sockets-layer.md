...
Le classi <xref:System.Net> usano Secure Sockets Layer (SSL) per crittografare la connessione per diversi protocolli di rete.  
 ...
 Per usare SSL con FTP (File Transfer Protocol), impostare la proprietà <xref:System.Net.FtpWebRequest.EnableSsl> su true prima di chiamare <xref:System.Net.FtpWebRequest.GetResponse>. Analogamente, per utilizzare SSL con SMTP Simple Mail Transport Protocol (), impostare il <xref:System.Net.Mail.SmtpClient.EnableSsl> proprietà su true prima dell'invio messaggio di posta elettronica.  
