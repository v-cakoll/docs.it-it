...
|HasProtectionLevel|Yes|Ottiene un valore che indica se al membro è stato assegnato un livello di protezione.|Receive.ProtectionLevel non deve essere null.|  
|Nome|Yes|Ottiene o imposta il nome per il \<portType > elemento in Web Services Description Language (WSDL).|Receive.ServiceContractName.LocalName deve corrispondere.|  
|Spazio dei nomi|Yes|Ottiene o imposta lo spazio dei nomi di \<portType > elemento in Web Services Description Language (WSDL).|Receive.ServiceContractName.NameSpace deve corrispondere.|  
|ProtectionLevel|Yes|Specifica se l'associazione del contratto deve supportare il valore della proprietà ProtectionLevel.|Receive.ProtectionLevel deve corrispondere.|  
...
|Nome proprietà|Supportato|Descrizione|Convalida WF|  
|-------------------|---------------|-----------------|-------------------|  
|Operazione|Yes|Consente di ottenere o impostare l'azione WS-Addressing del messaggio di richiesta.|Receive.Action deve corrispondere.|  
...
|HasProtectionLevel|Yes|Ottiene un valore che indica se i messaggi di questa operazione devono essere crittografati o firmati o se devono presentare entrambi i meccanismi di protezione.|Receive.ProtectionLevel non deve essere null.|  
...
|IsOneWay|Yes|Ottiene o imposta un valore che indica se l'operazione restituisce un messaggio di risposta.|(Nessun SendReply per Receive OPPURE nessun ReceiveReply per Send).|  
...
|Nome|Yes|Ottiene o imposta il nome dell'operazione.|Receive.OperationName deve corrispondere.|  
|ProtectionLevel|Yes|Ottiene o imposta un valore che specifica se i messaggi di un'operazione devono essere crittografati o firmati o se devono presentare entrambi i meccanismi di sicurezza.|Receive.ProtectionLevel deve corrispondere.|  
|ReplyAction|Yes|Ottiene o imposta il valore dell'azione SOAP del messaggio di risposta dell'operazione.|SendReply.Action deve corrispondere.|  
|TypeId|No|Se implementato in una classe derivata, ottiene un identificatore univoco per l'attributo (ereditato dall'attributo).|(N/D)|  
...
|Nome proprietà|Supportato|Descrizione|Convalida WF|  
|-------------------|---------------|-----------------|-------------------|  
|HasProtectionLevel|Yes|Ottiene un valore che indica se per il messaggio è specificato un livello di protezione.|Nessuna convalida (Receive.Content e SendReply.Content devono corrispondere al tipo di contratto di messaggio).|  
|IsWrapped|Yes|Ottiene o imposta un valore che specifica se il corpo del messaggio contiene un elemento wrapper.|Nessuna convalida (Receive.Content e SendReply.Content devono corrispondere al tipo di contratto di messaggio).|  
...
|TypeId|Yes|Se implementato in una classe derivata, ottiene un identificatore univoco per l'attributo (ereditato dall'attributo).|Nessuna convalida (Receive.Content e SendReply.Content devono corrispondere al tipo di contratto di messaggio).|  
|WrapperName|Yes|Ottiene o imposta il nome dell'elemento wrapper del corpo del messaggio.|Nessuna convalida (Receive.Content e SendReply.Content devono corrispondere al tipo di contratto di messaggio).|  
...
|Nome|Yes|Ottiene o imposta il nome del contratto dati per il tipo.|Nessuna convalida (Receive.Content e SendReply.Content devono corrispondere al tipo di contratto di messaggio).|  
|Spazio dei nomi|Yes|Ottiene o imposta lo spazio dei nomi del contratto dati per il tipo.|Nessuna convalida (Receive.Content e SendReply.Content devono corrispondere al tipo di contratto di messaggio).|  
|TypeId|No|Se implementato in una classe derivata, ottiene un identificatore univoco per l'attributo (ereditato dall'attributo).|(N/D)|  
...
|Nome proprietà|Supportato|Descrizione|Convalida WF|  
|-------------------|---------------|-----------------|-------------------|  
|Operazione|Yes|Consente di ottenere o impostare l'azione del messaggio di errore SOAP specificato come parte del contratto dell'operazione.|SendReply.Action deve corrispondere.|  
|DetailType|Yes|Consente di ottenere il tipo di un oggetto serializzabile contenente le informazioni sull'errore.|SendReply.Content deve corrispondere al tipo.|  
