...
In questo argomento viene descritta la procedura per ospitare un'istanza del Progettazione flussi di lavoro di Windows in un'applicazione Windows Presentation Foundation (WPF).
...
2. Modificare le dimensioni della finestra per semplificare l'utilizzo del Progettazione flussi di lavoro. A tale scopo, selezionare **MainWindow** nella finestra di progettazione, premere F4 per visualizzare la finestra **Proprietà** e, nella sezione del **layout** , impostare la **larghezza** su un valore 600 e l' **altezza** su un valore pari a 350.
...
5. Nella finestra di dialogo **Editor della raccolta** fare clic tre volte sul pulsante **Aggiungi** per inserire tre colonne nel layout. La prima colonna conterrà la **casella degli strumenti**, la seconda colonna ospiterà il progettazione flussi di lavoro e la terza colonna verrà utilizzata per il controllo proprietà.
...
8. In **Esplora soluzioni**fare clic con il pulsante destro del mouse su *MainWindow. XAML* e selezionare **Visualizza codice**. Modificare il codice attenendosi ai passaggi seguenti:
    ...
    4. Registrare i metadati della finestra di progettazione per aggiungere il supporto della finestra di progettazione per tutte le attività incorporate. In questo modo è possibile trascinare le attività dalla casella degli strumenti all'attività <xref:System.Activities.Statements.Sequence> originale nel Progettazione flussi di lavoro. A tale scopo, aggiungere il metodo `RegisterMetadata` alla classe `MainWindow`:
