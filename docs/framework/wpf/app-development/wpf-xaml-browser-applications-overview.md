...
> [!NOTE]
>  Se per lo sviluppo non si utilizza [!INCLUDE[TLA#tla_visualstu](../../../../includes/tlasharptla-visualstu-md.md)] o se si desiderano ulteriori informazioni sui file di progetto, vedere [Compilazione di un'applicazione WPF (WPF)](../../../../docs/framework/wpf/app-development/building-a-wpf-application-wpf.md).  
...
|Manifesto di distribuzione (con estensione xbap)|Questo file contiene le informazioni utilizzate da [!INCLUDE[TLA#tla_clickonce](../../../../includes/tlasharptla-clickonce-md.md)] per distribuire l'applicazione e presenta l'estensione xbap.|  
 ...
 Le applicazioni XBAP vengono distribuite in un server Web, ad esempio Microsoft [!INCLUDE[TLA#tla_iis50](../../../../includes/tlasharptla-iis50-md.md)] o versioni successive. Non è necessario installare [!INCLUDE[TLA2#tla_winfx](../../../../includes/tla2sharptla-winfx-md.md)] nel server Web, mentre è necessario registrare i tipi [!INCLUDE[TLA2#tla_wpf](../../../../includes/tla2sharptla-wpf-md.md)] [!INCLUDE[TLA#tla_mime](../../../../includes/tlasharptla-mime-md.md)] e le estensioni di file. Per ulteriori informazioni, vedere [Configurare IIS 5.0 e IIS 6.0 per distribuire applicazioni WPF](../../../../docs/framework/wpf/app-development/how-to-configure-iis-5-0-and-iis-6-0-to-deploy-wpf-applications.md).  
...
## <a name="communicating-with-the-host-web-page"></a>Comunicazione con la pagina Web host  
 Quando l'applicazione è ospitata in un frame HTML, è possibile comunicare con la pagina Web che contiene l'applicazione XBAP. A tale scopo, il recupero di <xref:System.Windows.Interop.BrowserInteropHelper.HostScript%2A> proprietà <xref:System.Windows.Interop.BrowserInteropHelper>. Questa proprietà restituisce un oggetto script che rappresenta la finestra HTML. È quindi possibile accedere alle proprietà, ai metodi e agli eventi nell'[oggetto finestra](http://go.microsoft.com/fwlink/?LinkId=160274) utilizzando la normale sintassi del punto. È inoltre possibile accedere a metodi di script e variabili globali. Nell'esempio seguente viene illustrato come recuperare l'oggetto script e chiudere il browser.  
...
-   Il <xref:System.Windows.Controls.WebBrowser> controllo deve fornire un comportamento simile per il browser di host, incluse le restrizioni di sicurezza. È possibile controllare alcune di queste restrizioni di sicurezza tramite le impostazioni di sicurezza di Internet Explorer. Per ulteriori informazioni, vedere [Sicurezza](../../../../docs/framework/wpf/security-wpf.md).  
  
-   Viene generata un'eccezione quando un'applicazione XBAP viene caricata tra domini in una pagina HTML.  
  
-   Input è in un thread separato da WPF <xref:System.Windows.Controls.WebBrowser>, pertanto non è possibile intercettare l'input da tastiera e lo stato IME non condiviso.  
...
-   <xref:System.Windows.Interop.HwndHost.MessageHook>non viene generato perché <xref:System.Windows.Interop.HwndHost> non sottoclasse di una finestra in esecuzione in un altro thread o processo.  
