...
-   Associazione all'origine dati.  
 ...
 Per un elenco di codice completo delle attività illustrate in questa procedura dettagliata, vedere [Data Binding in applicazioni ibrida](http://go.microsoft.com/fwlink/?LinkID=159983).  
...
4.  Nel <xref:System.Windows.Window> elemento, aggiungere il seguente [!INCLUDE[TLA#tla_winforms](../../../../includes/tlasharptla-winforms-md.md)] mapping di spazi dei nomi.  
  
    ```xaml  
    xmlns:wf="clr-namespace:System.Windows.Forms;assembly=System.Windows.Forms"  
    ```  
  
5.  Il valore predefinito di nome <xref:System.Windows.Controls.Grid> elemento `mainGrid` assegnando il <xref:System.Windows.FrameworkElement.Name%2A> proprietà.  
  
     [!code-xaml[WPFWithWFAndDatabinding#8](../../../../samples/snippets/csharp/VS_Snippets_Wpf/WPFWithWFAndDatabinding/CSharp/WPFWithWFAndDatabinding/Window1.xaml#8)]  
  
## <a name="defining-the-data-template"></a>Definizione del modello dati  
 Viene visualizzato l'elenco master di clienti un <xref:System.Windows.Controls.ListBox> controllo. L'esempio di codice seguente definisce un <xref:System.Windows.DataTemplate> oggetto denominato `ListItemsTemplate` che controlla la struttura ad albero visuale del <xref:System.Windows.Controls.ListBox> controllo. Questo <xref:System.Windows.DataTemplate> è assegnata la <xref:System.Windows.Controls.ListBox> del controllo <xref:System.Windows.Controls.ItemsControl.ItemTemplate%2A> proprietà.  
  
#### <a name="to-define-the-data-template"></a>Per definire il modello di dati  
  
-   Copiare il seguente codice XAML nel <xref:System.Windows.Controls.Grid> dichiarazione dell'elemento.  
...
## <a name="specifying-data-bindings"></a>Specifica delle associazioni dati  
 Viene visualizzato l'elenco master di clienti un <xref:System.Windows.Controls.ListBox> controllo. L'allegato `ListItemsTemplate` associa un <xref:System.Windows.Controls.TextBlock> controllo il `ContactName` campo dal database.  
...
## <a name="adding-the-data-source-to-the-project"></a>Aggiunta dell'origine dati al progetto  
 Con [!INCLUDE[vsprvs](../../../../includes/vsprvs-md.md)], è possibile aggiungere facilmente un'origine dati al progetto. Con questa procedura vengono aggiunti dati fortemente tipizzati al progetto. Vengono anche aggiunte altre classi di supporto, ad esempio adattatori di tabelle.  
  
#### <a name="to-add-the-data-source"></a>Per aggiungere l'origine dati  
  
1.  Dal **dati** dal menu **Aggiungi nuova origine dati**.  
  
2.  Nel **configurazione guidata origine dati**, creare una connessione al database Northwind usando un set di dati. Per ulteriori informazioni, vedere [procedura: connettersi ai dati in un Database](http://msdn.microsoft.com/library/6c56e54e-8834-4297-85aa-cc1a443ba556).  
...
4.  Quando viene chiesto di scegliere gli oggetti di database, selezionare il `Customers` e `Orders` tabelle e nome del set di dati generato `NorthwindDataSet`.  
...
2.  Copiare il codice seguente nel `MainWindow` definizione di classe.  
     ...
     Questo codice dichiara le <xref:System.Windows.Forms.BindingSource> componente e le classi di supporto associato che si connettono al database.  
...
8.  Copiare il codice seguente nel <xref:System.Windows.FrameworkElement.Loaded> gestore dell'evento.  
     ...
     Questo codice viene assegnato il <xref:System.Windows.Forms.BindingSource> componente come contesto dei dati e popola la `Customers` e `Orders` oggetti scheda.  
...
9. Copiare il codice seguente nel `MainWindow` definizione di classe.  
     ...
     Questo metodo gestisce il <xref:System.Windows.Data.CollectionView.CurrentChanged> evento e aggiorna l'elemento corrente dell'associazione dati.  
...
## <a name="see-also"></a>Vedere anche  
 ...
 [Data Binding in applicazioni ibrida](http://go.microsoft.com/fwlink/?LinkID=159983)  
