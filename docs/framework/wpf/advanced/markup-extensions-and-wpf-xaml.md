...
## <a name="extension-classes"></a>Classi di estensione \*  
 ...
 Ad esempio, la classe <xref:System.Windows.StaticResourceExtension> fornisce l'implementazione della superficie di ricerca effettiva della risorsa in modo che la relativa implementazione di <xref:System.Windows.Markup.MarkupExtension.ProvideValue%2A> restituisca l'oggetto richiesto, con l'input di quell'implementazione specifica che è una stringa utilizzata per cercare il risorsa in base al relativo `x:Key`. Molti dei dettagli di questa implementazione sono irrilevanti se si usa un'estensione di markup esistente.  
