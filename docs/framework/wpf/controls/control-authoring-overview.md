...
-   **Contenuto avanzato.** Molti controlli [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] standard supportano il contenuto avanzato. Ad esempio, la proprietà di contenuto di un <xref:System.Windows.Controls.Button> è di tipo <xref:System.Object>, teoricamente qualsiasi elemento può essere visualizzato in un <xref:System.Windows.Controls.Button>.  Per visualizzare un'immagine e testo in un pulsante, è possibile aggiungere un'immagine e un <xref:System.Windows.Controls.TextBlock> per un <xref:System.Windows.Controls.StackPanel> e assegnare il <xref:System.Windows.Controls.StackPanel> per il <xref:System.Windows.Controls.ContentControl.Content%2A> proprietà. Poiché i controlli possono visualizzare dati arbitrari ed elementi visivi di [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)], c'è una minore esigenza di creare un nuovo controllo o di modificarne uno esistente per supportare una visualizzazione complessa. Per ulteriori informazioni sul modello di contenuto per <xref:System.Windows.Controls.Button> e altro contenuto di modelli in [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)], vedere [modello di contenuto WPF](../../../../docs/framework/wpf/controls/wpf-content-model.md).  
  
-   **Stili.** Oggetto <xref:System.Windows.Style> è una raccolta di valori che rappresentano le proprietà di un controllo. Usando gli stili, è possibile creare una rappresentazione riutilizzabile dell'aspetto e del comportamento desiderati di un controllo, senza scriverne uno nuovo. Ad esempio, si supponga di voler tutti i <xref:System.Windows.Controls.TextBlock> controlli di tipo di carattere Arial di colore rosso con una dimensione di 14. È possibile creare uno stile come risorsa e impostare le proprietà appropriate di conseguenza. Quindi ogni <xref:System.Windows.Controls.TextBlock> aggiungere nell'applicazione avrà lo stesso aspetto.  
  
-   **Modelli di dati.** Oggetto <xref:System.Windows.DataTemplate> consente di personalizzare la modalità di visualizzazione dati in un controllo. Ad esempio, un <xref:System.Windows.DataTemplate> può essere utilizzato per specificare la modalità di visualizzazione dati un <xref:System.Windows.Controls.ListBox>.  Per un esempio, vedere [Cenni preliminari sui modelli di dati](../../../../docs/framework/wpf/data/data-templating-overview.md).  Oltre alla personalizzazione dell'aspetto dei dati, un <xref:System.Windows.DataTemplate> può includere elementi dell'interfaccia utente, che offre una notevole flessibilità nelle interfacce utente personalizzate.  Ad esempio, usando un <xref:System.Windows.DataTemplate>, è possibile creare un <xref:System.Windows.Controls.ComboBox> in cui ogni elemento contiene una casella di controllo.  
  
-   **Modelli di controllo.** Tuttavia, molti controlli [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] utilizzare un <xref:System.Windows.Controls.ControlTemplate> per definire la struttura e l'aspetto, che separa l'aspetto di un controllo della funzionalità del controllo. È possibile modificare drasticamente l'aspetto di un controllo mediante la ridefinizione relativo <xref:System.Windows.Controls.ControlTemplate>.  Si supponga, ad esempio, di voler creare un controllo con l'aspetto di un semaforo. Il controllo ha una funzionalità e un'interfaccia utente semplici.  È costituito da tre cerchi, che possono essere illuminati soltanto uno alla volta. Fare in modo che un <xref:System.Windows.Controls.RadioButton> offre le funzionalità di un solo la selezione alla volta, ma l'aspetto predefinito del <xref:System.Windows.Controls.RadioButton> aspetto nothing le luci su un semaforo.  Poiché il <xref:System.Windows.Controls.RadioButton> utilizza un modello di controllo per definire l'aspetto, è facile ridefinire il <xref:System.Windows.Controls.ControlTemplate> per soddisfare i requisiti di controllo e utilizzare i pulsanti di opzione per realizzare il semaforo.  
  
    > [!NOTE]
    >  Sebbene un <xref:System.Windows.Controls.RadioButton> possibile utilizzare un <xref:System.Windows.DataTemplate>, <xref:System.Windows.DataTemplate> non è sufficiente in questo esempio.  Il <xref:System.Windows.DataTemplate> definisce l'aspetto del contenuto di un controllo. In caso di un <xref:System.Windows.Controls.RadioButton>, il contenuto è l'elemento viene visualizzato a destra del cerchio che indica se il <xref:System.Windows.Controls.RadioButton> è selezionata.  Nell'esempio del semaforo il pulsante di opzione deve essere costituito semplicemente da un cerchio in grado di "illuminarsi". Poiché il requisito per il semaforo in modo diverso l'aspetto predefinito del <xref:System.Windows.Controls.RadioButton>, è necessario ridefinire il <xref:System.Windows.Controls.ControlTemplate>.  In genere un <xref:System.Windows.DataTemplate> viene utilizzato per definire il contenuto o dati di un controllo e un <xref:System.Windows.Controls.ControlTemplate> viene utilizzato per definire la struttura di un controllo.  
  
-   **Trigger.** Oggetto <xref:System.Windows.Trigger> consente di modificare in modo dinamico l'aspetto e il comportamento di un controllo senza creare un nuovo controllo. Ad esempio, si supponga che si dispone di più <xref:System.Windows.Controls.ListBox> controlli nell'applicazione e gli elementi in ogni <xref:System.Windows.Controls.ListBox> debbano essere in grassetto e rosso quando vengono selezionati. Potrebbe essere il primo istinto per creare una classe che eredita da <xref:System.Windows.Controls.ListBox> ed eseguire l'override di <xref:System.Windows.Controls.Primitives.Selector.OnSelectionChanged%2A> metodo per modificare l'aspetto dell'elemento selezionato, ma un approccio migliore consiste nell'aggiungere un trigger a uno stile di un <xref:System.Windows.Controls.ListBoxItem> che modifica l'aspetto di l'elemento selezionato. Un trigger consente di modificare i valori delle proprietà o di eseguire determinate azioni in base al valore di una proprietà. Un <xref:System.Windows.EventTrigger> consente di eseguire azioni quando si verifica un evento.  
  
 Per altre informazioni su stili, modelli e trigger, vedere [Applicazione di stili e modelli](../../../../docs/framework/wpf/controls/styling-and-templating.md).  
...
### <a name="deriving-from-usercontrol"></a>Derivazione da UserControl  
 Il modo più semplice per creare un controllo in [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] consiste nel derivare da <xref:System.Windows.Controls.UserControl>. Quando si compila un controllo che eredita da <xref:System.Windows.Controls.UserControl>, aggiungere componenti esistenti per il <xref:System.Windows.Controls.UserControl>, denominare i componenti e fare riferimento ai gestori di eventi in [!INCLUDE[TLA#tla_xaml](../../../../includes/tlasharptla-xaml-md.md)]. È quindi possibile fare riferimento agli elementi denominati e definire i gestori eventi nel codice. Questo modello di sviluppo è molto simile al modello usato per lo sviluppo di applicazioni in [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)].  
  
 Se compilato correttamente, un <xref:System.Windows.Controls.UserControl> possono sfruttare i vantaggi di contenuto complesso, gli stili e trigger. Tuttavia, se il controllo eredita da <xref:System.Windows.Controls.UserControl>, persone che utilizzano il controllo non sarà in grado di utilizzare un <xref:System.Windows.DataTemplate> o <xref:System.Windows.Controls.ControlTemplate> di personalizzarne l'aspetto.  È necessario derivare la <xref:System.Windows.Controls.Control> classe o una delle relative classi derivate (diverso da <xref:System.Windows.Controls.UserControl>) per creare un controllo personalizzato che supporta i modelli.  
  
#### <a name="benefits-of-deriving-from-usercontrol"></a>Vantaggi della derivazione da UserControl  
 Prendere in considerazione la derivazione da <xref:System.Windows.Controls.UserControl> se vengono soddisfatte tutte le operazioni seguenti:  
...
### <a name="deriving-from-control"></a>Derivazione da Control  
 Derivazione da di <xref:System.Windows.Controls.Control> classe è il modello usato dalla maggior parte dei esistente [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] controlli. Quando si crea un controllo che eredita il <xref:System.Windows.Controls.Control> (classe), di definire l'aspetto tramite modelli. In tal modo, la logica operativa viene separata dalla rappresentazione visiva. È inoltre possibile garantire la separazione tra l'interfaccia utente e la logica tramite i comandi e associazioni anziché gli eventi e come evitare riferimenti agli elementi di <xref:System.Windows.Controls.ControlTemplate> laddove possibile.  Se l'interfaccia utente e la logica del controllo sono separate in modo appropriati, un utente del controllo è possibile ridefinire il controllo <xref:System.Windows.Controls.ControlTemplate> di personalizzarne l'aspetto. Sebbene la compilazione di un oggetto personalizzato <xref:System.Windows.Controls.Control> non è semplice come predefinito un <xref:System.Windows.Controls.UserControl>, un oggetto personalizzato <xref:System.Windows.Controls.Control> offre la massima flessibilità.  
  
#### <a name="benefits-of-deriving-from-control"></a>Vantaggi della derivazione da Control  
 Prendere in considerazione la derivazione da <xref:System.Windows.Controls.Control> anziché la <xref:System.Windows.Controls.UserControl> classe se una qualsiasi delle condizioni seguenti:  
  
-   Si desidera che l'aspetto del controllo sia personalizzabile tramite il <xref:System.Windows.Controls.ControlTemplate>.  
...
-   Definire un <xref:System.Windows.DependencyProperty> identificatore denominato `ValueProperty` come un `public` `static` `readonly` campo.  
  
-   Registrare il nome della proprietà con il sistema di proprietà, chiamando <xref:System.Windows.DependencyProperty.Register%2A?displayProperty=nameWithType>, per specificare quanto segue:  
...
-   Definire un metodo per il <xref:System.Windows.PropertyChangedCallback>, denominato `OnValueChanged`. `OnValueChanged` Crea un <xref:System.Windows.RoutedPropertyChangedEventArgs%601> dell'oggetto e lo prepara generare il `ValueChanged` dell'evento indirizzato. Gli eventi indirizzati sono illustrati nella sezione successiva.  
 ...
 Per altre informazioni, vedere [Proprietà di dipendenza personalizzate](../../../../docs/framework/wpf/advanced/custom-dependency-properties.md).  
...
-   Gli eventi possono essere gestiti in un elemento padre di più controlli. Nel caso di un evento di bubbling, un singolo elemento padre nell'albero degli elementi può sottoscrivere l'evento. Gli autori dell'applicazione possono quindi usare un solo gestore per rispondere all'evento di più controlli. Ad esempio, se il controllo è una parte di ogni elemento in un <xref:System.Windows.Controls.ListBox> (perché è incluso un <xref:System.Windows.DataTemplate>), lo sviluppatore di applicazioni è possibile definire il gestore eventi per eventi del controllo sul <xref:System.Windows.Controls.ListBox>. Il gestore eventi viene chiamato ogni volta che l'evento si verifica in uno dei controlli.  
  
-   Gli eventi indirizzati sono utilizzabili in un <xref:System.Windows.EventSetter>, che consente agli sviluppatori di applicazioni specificare il gestore di evento all'interno di uno stile.  
  
-   Gli eventi indirizzati sono utilizzabili in un <xref:System.Windows.EventTrigger>, questa opzione è utile per tali proprietà utilizzando [!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)]. Per altre informazioni, vedere [Panoramica dell'animazione](../../../../docs/framework/wpf/graphics-multimedia/animation-overview.md).  
  
 L'esempio seguente definisce un evento indirizzato mediante le operazioni seguenti:  
  
-   Definire un <xref:System.Windows.RoutedEvent> identificatore denominato `ValueChangedEvent` come un `public` `static` `readonly` campo.  
  
-   Registrare l'evento indirizzato chiamando il <xref:System.Windows.EventManager.RegisterRoutedEvent%2A?displayProperty=nameWithType> metodo. L'esempio specifica le informazioni seguenti quando si chiama <xref:System.Windows.EventManager.RegisterRoutedEvent%2A>:  
  
    -   Il nome dell'evento è `ValueChanged`.  
  
    -   La strategia di routing è <xref:System.Windows.RoutingStrategy.Bubble>, il che significa che un gestore eventi nell'origine (l'oggetto che genera l'evento) viene chiamato per primo, e quindi vengono chiamati i gestori eventi per gli elementi padre dell'origine in successione, a partire dal gestore dell'evento in più vicina elemento padre.  
...
-   Dichiarazione di un evento pubblico denominato `ValueChanged` e inserimento di dichiarazioni delle funzioni di accesso dell'evento. Nell'esempio viene chiamato <xref:System.Windows.UIElement.AddHandler%2A> nel `add` dichiarazione di funzione di accesso e <xref:System.Windows.UIElement.RemoveHandler%2A> nel `remove` dichiarazione di funzione di accesso per utilizzare il [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] servizi eventi.  
...
-   Creazione di un metodo virtuale protetto denominato `OnValueChanged` che genera l'evento `ValueChanged`.  
 ...
 Per altre informazioni, vedere [Cenni preliminari sugli eventi indirizzati](../../../../docs/framework/wpf/advanced/routed-events-overview.md) e [Creare un evento indirizzato personalizzato](../../../../docs/framework/wpf/advanced/how-to-create-a-custom-routed-event.md).  
  
### <a name="use-binding"></a>Usare il binding  
 Per separare l'interfaccia utente del controllo dalla relativa logica, è possibile usare il data binding. Ciò è particolarmente importante se si definisce l'aspetto del controllo utilizzando un <xref:System.Windows.Controls.ControlTemplate>. Se si usa il data binding, potrebbe non essere più necessario fare riferimento a parti specifiche dell'interfaccia utente dal codice. È consigliabile evitare di fare riferimento a elementi presenti il <xref:System.Windows.Controls.ControlTemplate> perché quando il codice fa riferimento a elementi inclusi nel <xref:System.Windows.Controls.ControlTemplate> e <xref:System.Windows.Controls.ControlTemplate> viene modificato, l'elemento di cui viene fatto riferimento deve essere incluso nel nuovo <xref:System.Windows.Controls.ControlTemplate>.  
  
 Nell'esempio seguente viene aggiornato il <xref:System.Windows.Controls.TextBlock> del `NumericUpDown` controllo assegnando un nome e la casella di testo di riferimento in base al nome in codice.  
...
-   Dispone un `public` `static` `readonly` <xref:System.Windows.DependencyProperty> del form *PropertyName* `Property` creato utilizzando il <xref:System.Windows.DependencyProperty.RegisterAttached%2A> metodo. Il nome della proprietà che viene passato a <xref:System.Windows.DependencyProperty.RegisterAttached%2A> deve corrispondere *PropertyName*.  
...
#### <a name="defining-resources-at-the-element-level"></a>Definizione delle risorse a livello di elemento  
 ...
 Lo svantaggio di questo approccio è che un <xref:System.Windows.ResourceDictionary> oggetto viene creato ogni volta che si fa riferimento.  Ad esempio, se si dispongono di 10 controlli personalizzati nella libreria, i dizionari delle risorse condivise per ogni controllo di tipo merge tramite XAML crei 10 identici <xref:System.Windows.ResourceDictionary> oggetti.  È possibile evitare questo problema creando una classe statica che unisce le risorse nel codice e restituisce il valore risultante <xref:System.Windows.ResourceDictionary>.  
...
#### <a name="defining-resources-at-the-theme-level"></a>Definizione delle risorse a livello di tema  
 [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] consente di creare risorse per diversi temi di Windows.  L'autore di un controllo può definire una risorsa per un tema specifico in modo da modificare l'aspetto del controllo a seconda del tema in uso. Ad esempio, l'aspetto di un <xref:System.Windows.Controls.Button> in Windows classico tema (il tema predefinito per Windows 2000) è diverso da un <xref:System.Windows.Controls.Button> nel tema Windows Luna (il tema predefinito per Windows XP) perché il <xref:System.Windows.Controls.Button> utilizza un altro <xref:System.Windows.Controls.ControlTemplate> per ogni tema.  
...
|`Aero.NormalColor.xaml`|Tema predefinito in Windows Vista|  
 ...
 L'[esempio di controllo personalizzato NumericUpDown con supporto per temi e automazione dell'interfaccia utente](http://go.microsoft.com/fwlink/?LinkID=160025) contiene due dizionari risorse per li controllo `NumericUpDown`: uno in generic.xaml e uno in Luna.NormalColor.xaml.  È possibile eseguire l'applicazione e passare tra il tema argento in Windows XP e un altro tema per vedere la differenza tra i due modelli del controllo. Se si usa Windows Vista, è possibile rinominare Luna.NormalColor.xaml in Aero.NormalColor.xaml e passare tra due temi, ad esempio tra il tema Windows Classico e il tema predefinito per Windows Vista.  
  
 Quando si inserisce un <xref:System.Windows.Controls.ControlTemplate> in uno dei file di dizionario risorse specifiche del tema, è necessario creare un costruttore statico per il controllo e la chiamata di <xref:System.Windows.DependencyProperty.OverrideMetadata%28System.Type%2CSystem.Windows.PropertyMetadata%29> metodo il <xref:System.Windows.FrameworkElement.DefaultStyleKey%2A>, come illustrato nell'esempio seguente.  
...
##### <a name="specifying-the-location-of-theme-resources"></a>Indicazione del percorso delle risorse del tema  
 Per trovare le risorse per un controllo, l'applicazione host deve sapere che l'assembly contiene risorse specifiche del controllo. È possibile eseguire questa operazione aggiungendo il <xref:System.Windows.ThemeInfoAttribute> all'assembly che contiene il controllo. Il <xref:System.Windows.ThemeInfoAttribute> ha un <xref:System.Windows.ThemeInfoAttribute.GenericDictionaryLocation%2A> proprietà che specifica il percorso delle risorse generiche, e un <xref:System.Windows.ThemeInfoAttribute.ThemeDictionaryLocation%2A> proprietà che specifica il percorso delle risorse specifiche del tema.  
  
 L'esempio seguente imposta il <xref:System.Windows.ThemeInfoAttribute.GenericDictionaryLocation%2A> e <xref:System.Windows.ThemeInfoAttribute.ThemeDictionaryLocation%2A> proprietà <xref:System.Windows.ResourceDictionaryLocation.SourceAssembly>, per specificare che le risorse generiche e specifiche del tema sono nello stesso assembly del controllo.  
