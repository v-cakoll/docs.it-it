...
Questo argomento offre una panoramica del livello visivo di [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)]. È incentrato sul ruolo della <xref:System.Windows.Media.Visual> classe per il rendering del supporto nel [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] modello.  

<a name="role_of_visual_object"></a>
## <a name="role-of-the-visual-object"></a>Ruolo dell'oggetto visivo  
 La <xref:System.Windows.Media.Visual> classe è l'astrazione di base da cui <xref:System.Windows.FrameworkElement> derivano tutti gli oggetti. Viene anche usata come punto di ingresso per la scrittura di nuovi controlli in [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] e in molti casi può essere considerata come l'handle di finestra (HWND) nel modello di applicazione Win32.  
  
 L' <xref:System.Windows.Media.Visual> oggetto è un [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] oggetto di base, il cui ruolo primario è fornire supporto per il rendering. I controlli dell'interfaccia utente, ad esempio <xref:System.Windows.Controls.Button> e <xref:System.Windows.Controls.TextBox> , derivano dalla <xref:System.Windows.Media.Visual> classe e li usano per rendere permanente i dati di rendering. L' <xref:System.Windows.Media.Visual> oggetto fornisce supporto per:  
...
### <a name="viewport3dvisual-class"></a>Classe Viewport3DVisual  
 <xref:System.Windows.Media.Media3D.Viewport3DVisual>Fornisce un bridge tra oggetti 2D <xref:System.Windows.Media.Visual> e <xref:System.Windows.Media.Media3D.Visual3D> . La <xref:System.Windows.Media.Media3D.Visual3D> classe è la classe base per tutti gli elementi visivi 3D. <xref:System.Windows.Media.Media3D.Viewport3DVisual>Per è necessario definire un <xref:System.Windows.Media.Media3D.Viewport3DVisual.Camera%2A> valore e un <xref:System.Windows.Media.Media3D.Viewport3DVisual.Viewport%2A> valore. La camera consente di visualizzare la scena. Il viewport stabilisce dove viene eseguito il mapping della proiezione sulla superficie 2D. Per altre informazioni su 3D in [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] , vedere [Cenni preliminari sulla grafica 3D](3-d-graphics-overview.md).  
  
### <a name="containervisual-class"></a>Classe ContainerVisual  
 La <xref:System.Windows.Media.ContainerVisual> classe viene usata come contenitore per una raccolta di <xref:System.Windows.Media.Visual> oggetti. La <xref:System.Windows.Media.DrawingVisual> classe deriva dalla <xref:System.Windows.Media.ContainerVisual> classe, consentendo l'inclusione di una raccolta di oggetti visivi.  
...
|Immagine|Rappresenta un'immagine in un'area definita da un oggetto <xref:System.Windows.Rect> .|  
|Icona|Rappresenta un disegno che esegue il rendering di un oggetto <xref:System.Windows.Media.GlyphRun> , ovvero una sequenza di glifi da una risorsa del tipo di carattere specificata. Ecco come viene rappresentato il testo.|  
|Video|Rappresenta un disegno che esegue il rendering di video.|  
  
 <xref:System.Windows.Media.DrawingContext>Consente di popolare un oggetto <xref:System.Windows.Media.Visual> con contenuto visivo. Quando si usano <xref:System.Windows.Media.DrawingContext> i comandi di disegno di un oggetto, si archivia effettivamente un set di dati di rendering che verrà usato in un secondo momento dal sistema grafico; non si sta disegnando sullo schermo in tempo reale.  
...
#### <a name="drawing-content-at-the-visual-layer"></a>Contenuto del disegno a livello visivo  
 Non è mai possibile creare direttamente un'istanza di <xref:System.Windows.Media.DrawingContext> . è tuttavia possibile acquisire un contesto di disegno da determinati metodi, ad esempio <xref:System.Windows.Media.DrawingGroup.Open%2A?displayProperty=nameWithType> e <xref:System.Windows.Media.DrawingVisual.RenderOpen%2A?displayProperty=nameWithType> . Nell'esempio seguente viene recuperato un oggetto <xref:System.Windows.Media.DrawingContext> da un oggetto <xref:System.Windows.Media.DrawingVisual> e viene utilizzato per creare un rettangolo.  
...
## <a name="how-visual-objects-are-used-to-build-controls"></a>Uso degli oggetti visivi per la compilazione di controlli  
 ...
 Per enumerare gli oggetti visivi che includono il <xref:System.Windows.Controls.Button> controllo predefinito, è possibile trovare la gerarchia di oggetti visivi illustrata di seguito:  
...
### <a name="control-templates"></a>Modelli di controllo  
 ...
 Il <xref:System.Windows.Controls.Button> controllo contiene un <xref:Microsoft.Windows.Themes.ClassicBorderDecorator> elemento, che a sua volta contiene un <xref:System.Windows.Controls.ContentPresenter> elemento. L' <xref:Microsoft.Windows.Themes.ClassicBorderDecorator> elemento è responsabile del disegno di tutti gli elementi grafici discreti che compongono il bordo e lo sfondo di un pulsante. L' <xref:System.Windows.Controls.ContentPresenter> elemento è responsabile della visualizzazione del contenuto di <xref:System.Windows.Controls.Button> . In questo caso, poiché si visualizza un'immagine, l' <xref:System.Windows.Controls.ContentPresenter> elemento contiene un <xref:System.Windows.Controls.Image> elemento.  
...
## <a name="visual-tree"></a>Struttura ad albero visuale  
 ...
 Nel codice seguente viene illustrato un <xref:System.Windows.Controls.StackPanel> elemento definito nel markup.  
...
Diagramma dell'albero logico  
 ...
 La struttura ad albero visuale e l'albero logico sono sincronizzati con il set corrente di elementi dell'applicazione che riflette qualsiasi aggiunta, eliminazione o modifica di elementi. Presentano tuttavia visualizzazioni diverse dell'applicazione. A differenza della struttura ad albero visuale, l'albero logico non espande l'elemento di un controllo <xref:System.Windows.Controls.ContentPresenter> . Ciò significa che non vi è una corrispondenza diretta tra un albero logico e una struttura ad albero visuale per lo stesso set di oggetti. Di fatto, richiamando il metodo dell'oggetto **LogicalTreeHelper** <xref:System.Windows.LogicalTreeHelper.GetChildren%2A> e il metodo dell'oggetto **VisualTreeHelper** <xref:System.Windows.Media.VisualTreeHelper.GetChild%2A> utilizzando lo stesso elemento di un parametro vengono restituiti risultati diversi.  
...
### <a name="viewing-the-visual-tree-with-xamlpad"></a>Visualizzazione della struttura ad albero visuale con XamlPad  
 ...
 Si noti che <xref:System.Windows.Controls.Label> i <xref:System.Windows.Controls.TextBox> controlli, e <xref:System.Windows.Controls.Button> visualizzano ogni gerarchia di oggetti visivi separata nel pannello **Visual Tree Explorer** di XamlPad. Ciò è dovuto [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] al fatto che i controlli hanno un oggetto <xref:System.Windows.Controls.ControlTemplate> che contiene la struttura ad albero visuale del controllo. Quando si fa riferimento in modo esplicito a un controllo, si fa riferimento in modo implicito alla relativa gerarchia visiva.  
...
### <a name="vector-graphics"></a>Grafica vettoriale  
 ...
 Il markup seguente mostra due <xref:System.Windows.Shapes.Path> elementi definiti. Il secondo elemento usa un oggetto <xref:System.Windows.Media.ScaleTransform> per ridimensionare le istruzioni di disegno del primo elemento del 300%. Si noti che le istruzioni di disegno negli <xref:System.Windows.Shapes.Path> elementi rimangono invariate.  
...
### <a name="enumerating-the-visual-tree"></a>Enumerazione della struttura ad albero visuale  
 La <xref:System.Windows.Media.VisualTreeHelper> classe fornisce funzionalità per l'enumerazione dei membri di una struttura ad albero visuale. Per recuperare un elemento padre, chiamare il <xref:System.Windows.Media.VisualTreeHelper.GetParent%2A> metodo. Per recuperare un elemento figlio o un discendente diretto di un oggetto visivo, chiamare il <xref:System.Windows.Media.VisualTreeHelper.GetChild%2A> metodo. Questo metodo restituisce un elemento figlio <xref:System.Windows.Media.Visual> dell'elemento padre in corrispondenza dell'indice specificato.  
