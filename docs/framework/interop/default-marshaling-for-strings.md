...
Ad esempio, la funzione API [`GetWindowText`](/windows/desktop/api/winuser/nf-winuser-getwindowtextw)di Windows (definita in *winuser. h*) richiede che il chiamante trasmetta un buffer di caratteri a lunghezza fissa in cui la funzione scrive il testo della finestra. `LpString` punta a un buffer allocato dal chiamante di dimensione `nMaxCount`. Il chiamante deve allocare il buffer e impostare l'argomento `nMaxCount` sulla dimensione del buffer allocato. L'esempio seguente illustra la dichiarazione della funzione `GetWindowText` come definita in *winuser.h*.
