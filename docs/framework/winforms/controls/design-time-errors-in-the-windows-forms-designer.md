...
ms.locfileid: "73460115"
---
# <a name="windows-forms-designer-error-page"></a>Pagina di errore Progettazione Windows Form

Se il Progettazione Windows Form non viene caricato a causa di un errore nel codice, in un componente di terze parti o altrove, viene visualizzata una pagina di errore anziché la finestra di progettazione. Questa pagina di errore non indica necessariamente un bug nella finestra di progettazione. Il bug potrebbe trovarsi in un punto della pagina code-behind denominato \<il nome del form >. Designer.cs. Gli errori vengono visualizzati nelle barre gialle comprimibili con un collegamento per passare alla posizione dell'errore nella tabella codici.
...
> - Per C++ le app, gli errori non hanno collegamenti per la posizione del codice.
...
### <a name="identifier-name-is-not-a-valid-identifier"></a>'\<identificatore nome >' non è un identificatore valido

Questo errore indica che un campo, un metodo, un evento o un oggetto non è stato denominato correttamente.

### <a name="name-already-exists-in-project-name"></a>'\<Name >' esiste già in '\<nome progetto >'

Messaggio di errore: "il nome\<>' esiste già in '\<nome progetto >'. Immettere un nome univoco. "
...
### <a name="toolbox-tab-name-is-not-a-toolbox-category"></a>'\<nome scheda casella degli strumenti >' non è una categoria della casella degli strumenti
...
### <a name="an-exception-occurred-while-trying-to-create-an-instance-of-object-name"></a>Si è verificata un'eccezione durante il tentativo di creare un'istanza di '\<nome oggetto >'
...
### <a name="another-editor-has-document-name-open-in-an-incompatible-mode"></a>Un altro editor ha '\<nome documento >' aperto in una modalità incompatibile

Messaggio di errore: "un altro editor ha '\<nome documento >' aperto in una modalità incompatibile. Chiudere l'editor, quindi ripetere l'operazione ".
...
### <a name="another-editor-has-made-changes-to-document-name"></a>In un altro editor sono state apportate modifiche a'\<nome documento >'
...
Questo messaggio è simile a "un altro editor ha '\<nome documento >' aperto in una modalità incompatibile", ma Visual Studio non è in grado di determinare il nome del file. Per correggere l'errore, chiudere l'editor in cui è aperto il file e riprovare.

### <a name="array-rank-rank-in-array-is-too-high"></a>La classificazione della matrice '\<Rank nella matrice >' è troppo alta
...
### <a name="assembly-assembly-name-could-not-be-opened"></a>Non è stato possibile aprire l'assembly '\<nome assembly >'

Messaggio di errore: Impossibile aprire "assembly"\<nome assembly > ". Verificare che il file esista ancora ".
...
### <a name="bad-element-type-this-serializer-expects-an-element-of-type-type-name"></a>Tipo di elemento non valido. Questo serializzatore prevede un elemento di tipo '\<nome tipo >'
...
### <a name="cannot-bind-an-event-handler-to-the-event-name-event-because-it-is-read-only"></a>Impossibile associare un gestore eventi all'evento '\<nome evento >' perché è di sola lettura
...
### <a name="cannot-name-the-object-name-because-it-is-already-named-name"></a>Non è possibile denominare l'oggetto '\<Name >' perché è già denominato '\<Name >'
...
### <a name="cannot-remove-or-destroy-inherited-component-component-name"></a>Impossibile rimuovere o eliminare definitivamente il componente ereditato '\<nome componente >'
...
### <a name="category-toolbox-tab-name-does-not-have-a-tool-for-class-class-name"></a>La categoria '\<nome scheda casella degli strumenti >' non contiene uno strumento per la classe '\<nome classe >'
...
### <a name="class-class-name-has-no-matching-constructor"></a>La classe '\<nome classe >' non ha un costruttore corrispondente
...
### <a name="code-generation-for-property-property-name-failed"></a>Generazione del codice per la proprietà'\<nome proprietà >' non riuscita
...
### <a name="component-component-name-did-not-call-containeradd-in-its-constructor"></a>Il componente '\<nome componente >' non ha chiamato container. Add () nel costruttore
...
Per correggere l'errore, contattare il fornitore del componente. In alternativa, se si tratta di un componente creato, chiamare il metodo `IContainer.Add` nel costruttore del componente.
...
### <a name="could-not-access-the-variable-variable-name-because-it-has-not-been-initialized-yet"></a>Non è stato possibile accedere alla variabile '\<nome della variabile >' perché non è ancora stata inizializzata
...
### <a name="could-not-find-type-type-name"></a>Non è stato possibile trovare il tipo '\<> nome del tipo '

Messaggio di errore: "Impossibile trovare il tipo '\<nome del tipo >'. Assicurarsi che venga fatto riferimento all'assembly che contiene il tipo. Se questo tipo è una parte del progetto di sviluppo, verificare che il progetto sia stato compilato correttamente. "
...
### <a name="could-not-load-type-type-name"></a>Non è stato possibile caricare il tipo '\<> nome del tipo '

Messaggio di errore: "Impossibile caricare il tipo '\<nome del tipo di >'. Assicurarsi che l'assembly contenente questo tipo venga aggiunto ai riferimenti del progetto ".
...
### <a name="duplicate-declaration-of-member-member-name"></a>Dichiarazione duplicata del membro '\<nome membro >'
...
### <a name="error-reading-resources-from-the-resource-file-for-the-culture-culture-name"></a>Errore durante la lettura delle risorse dal file di risorse per le impostazioni cultura '\<nome impostazioni cultura >'
...
### <a name="error-reading-resources-from-the-resource-file-for-the-default-culture-culture-name"></a>Errore durante la lettura delle risorse dal file di risorse per le impostazioni cultura predefinite '\<nome impostazioni cultura >'
...
### <a name="failed-to-parse-method-method-name"></a>Non è stato possibile analizzare il metodo '\<nome metodo >'

Messaggio di errore: "non è stato possibile analizzare il metodo '\<nome metodo >'. Il parser ha segnalato l'errore seguente:'\<stringa di errore >'. Esaminare la Elenco attività per individuare eventuali errori. "
...
### <a name="invalid-component-name-component-name"></a>Nome componente non valido:'\<nome componente >'
...
### <a name="the-assembly-assembly-name-could-not-be-found"></a>Impossibile trovare l'assembly '\<nome assembly >'

Messaggio di errore: "Impossibile trovare l'assembly '\<nome assembly >'. Verificare che venga fatto riferimento all'assembly. Se l'assembly fa parte del progetto di sviluppo corrente, verificare che il progetto sia stato compilato ".

Questo errore è simile a "Impossibile trovare il tipo '\<nome del tipo >'", ma questo errore si verifica in genere a causa di un attributo di metadati. Per correggere l'errore, verificare che venga fatto riferimento a tutti gli assembly utilizzati dagli attributi.

### <a name="the-assembly-name-assembly-name-is-invalid"></a>Il nome dell'assembly '\<nome assembly >' non è valido
...
### <a name="the-base-class-class-name-cannot-be-designed"></a>Non è possibile progettare la classe base '\<nome classe >'
...
### <a name="the-base-class-class-name-could-not-be-loaded"></a>Non è stato possibile caricare la classe di base '\<nome classe >'
...
### <a name="the-class-class-name-cannot-be-designed-in-this-version-of-visual-studio"></a>Non è possibile progettare la classe '\<nome classe >' in questa versione di Visual Studio
...
### <a name="the-component-cannot-be-added-because-it-contains-a-circular-reference-to-reference-name"></a>Non è possibile aggiungere il componente perché contiene un riferimento circolare a'\<nome riferimento >'
...
### <a name="the-designer-for-base-class-class-name-is-not-installed"></a>La finestra di progettazione per la classe base '\<nome classe >' non è installata
...
### <a name="the-designer-must-create-an-instance-of-type-type-name-but-it-cant-because-the-type-is-declared-as-abstract"></a>La finestra di progettazione deve creare un'istanza di tipo '\<nome tipo >', ma non può perché il tipo è dichiarato come abstract
...
### <a name="the-language-parser-class-class-name-is-not-implemented-properly"></a>La classe del parser '\<class name >' del linguaggio non è implementata correttamente

Messaggio di errore: "la classe del parser del linguaggio '\<nome classe >' non è implementata correttamente. Contattare il fornitore per un modulo parser aggiornato ".
...
### <a name="the-object-object-name-does-not-implement-the-icomponent-interface"></a>L'oggetto '\<nome oggetto >' non implementa l'interfaccia IComponent

Visual Studio ha tentato di creare un componente, ma l'oggetto creato non implementa l'interfaccia <xref:System.ComponentModel.IComponent>. Contattare il fornitore del componente per una correzione.

### <a name="the-object-object-name-returned-null-for-the-property-property-name-but-this-is-not-allowed"></a>L'oggetto '\<nome oggetto >' ha restituito null per la proprietà'\<nome proprietà >', ma questa operazione non è consentita
...
### <a name="the-service-service-name-is-required-but-could-not-be-located"></a>Il servizio '\<nome servizio >' è obbligatorio, ma non è stato possibile individuarlo

Messaggio di errore: "il servizio '\<nome servizio >' è obbligatorio, ma non è stato trovato. Potrebbe essersi verificato un problema con l'installazione di Visual Studio. "
...
### <a name="the-service-instance-must-derive-from-or-implement-interface-name"></a>L'istanza del servizio deve derivare da o implementare il nome dell'interfaccia '\<>'
...
### <a name="the-toolbox-item-for-component-name-could-not-be-retrieved-from-the-toolbox"></a>Impossibile recuperare l'elemento della casella degli strumenti per '\<nome componente >' dalla casella degli strumenti

Messaggio di errore: "Impossibile recuperare l'elemento della casella degli strumenti per '\<nome componente >' dalla casella degli strumenti. Verificare che l'assembly che contiene l'elemento della casella degli strumenti sia installato correttamente. L'elemento della casella degli strumenti ha generato il seguente errore: \<stringa di errore > ".
...
### <a name="the-toolbox-item-for-toolbox-item-name-could-not-be-retrieved-from-the-toolbox"></a>Impossibile recuperare dalla casella degli strumenti l'elemento della casella degli strumenti per '\<nome dell'elemento della casella degli strumenti >'

Messaggio di errore: "Impossibile recuperare l'elemento della casella degli strumenti per '\<nome dell'elemento della casella degli strumenti >' dalla casella degli strumenti. Provare a rimuovere l'elemento dalla casella degli strumenti e aggiungerlo di nuovo. "
...
### <a name="the-type-type-name-could-not-be-found"></a>Impossibile trovare il tipo '\<nome del tipo di >'

Messaggio di errore: "Impossibile trovare il tipo '\<nome del tipo di >'. Verificare che venga fatto riferimento all'assembly contenente il tipo. Se l'assembly fa parte del progetto di sviluppo corrente, verificare che il progetto sia stato compilato ".
...
### <a name="the-variable-variable-name-is-either-undeclared-or-was-never-assigned"></a>La variabile '\<nome della variabile >' non è dichiarata o non è mai stata assegnata
...
### <a name="there-is-already-a-command-handler-for-the-menu-command-menu-command-name"></a>Esiste già un gestore comando per il comando di menu '\<nome comando menu >'
...
### <a name="there-is-already-a-component-named-component-name"></a>Esiste già un componente denominato '\<nome componente >'
...
### <a name="there-is-already-a-toolbox-item-creator-registered-for-the-format-format-name"></a>Esiste già un creatore di elementi della casella degli strumenti registrato per il formato '\<nome formato >'
...
### <a name="type-type-name-does-not-have-a-constructor-with-parameters-of-types-parameter-type-names"></a>Il tipo '\<nome tipo\>' non contiene un costruttore con parametri di tipo '\<nomi di tipo di parametro >'
...
### <a name="unable-to-add-reference-reference-name-to-the-current-application"></a>Non è possibile aggiungere il riferimento '\<nome riferimento >' all'applicazione corrente

Messaggio di errore: "Impossibile aggiungere il riferimento '\<nome di riferimento >' all'applicazione corrente. Verificare che non sia già stato fatto riferimento a una versione diversa di '\<nome riferimento >'.
...
### <a name="unable-to-find-page-named-options-dialog-box-tab-name"></a>Impossibile trovare la pagina denominata '\<opzioni Nome scheda della finestra di dialogo >'
...
### <a name="unable-to-find-property-property-name-on-page-options-dialog-box-tab-name"></a>Impossibile trovare la proprietà'\<nome proprietà >' nella pagina '\<finestra di dialogo Opzioni nome scheda >'
...
### <a name="visual-studio-cannot-save-or-load-instances-of-the-type-type-name"></a>Visual Studio non è in grado di salvare o caricare istanze del tipo '\<nome del tipo >'

Si tratta di un problema con un componente di terze parti. Contattare il fornitore del componente.

### <a name="visual-studio-is-unable-to-open-document-name-in-design-view"></a>Visual Studio non è in grado di aprire '\<nome documento >' nella visualizzazione progettazione

Messaggio di errore: "Visual Studio non è in grado di aprire '\<nome documento >' nella visualizzazione progettazione. Nessun parser installato per il tipo di file ".
...
### <a name="visual-studio-was-unable-to-find-a-designer-for-classes-of-type-type-name"></a>Visual Studio non è riuscito a trovare una finestra di progettazione per le classi di tipo '\<nome del tipo >'
