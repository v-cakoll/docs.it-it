...
ms.locfileid: "70988439"
---
# <a name="windows-forms-designer-error-page"></a>Pagina di errore Progettazione Windows Form

Se il Progettazione Windows Form non viene caricato a causa di un errore nel codice, in un componente di terze parti o altrove, viene visualizzata una pagina di errore anziché la finestra di progettazione. Questa pagina di errore non indica necessariamente un bug nella finestra di progettazione. Il bug potrebbe trovarsi in un punto qualsiasi della pagina code-behind \<denominata nome-form >. Designer.cs. Gli errori vengono visualizzati nelle barre gialle comprimibili con un collegamento per passare alla posizione dell'errore nella tabella codici.
...
### <a name="toolbox-tab-name-is-not-a-toolbox-category"></a>'\<Il nome della scheda casella degli strumenti >' non è una categoria della casella degli strumenti
...
Messaggio di errore: "Un parser di linguaggio richiesto non è installato. Il nome del parser del linguaggio{0}è''.
...
Messaggio di errore: "Si è verificata un'eccezione durante il tentativo di creare\<un'istanza di" nome oggetto > ". L'eccezione è "\<stringa\>di eccezione".
...
Messaggio di errore: "Il nome del\<documento >' è aperto in un altro editor in una modalità incompatibile. Chiudere l'editor, quindi ripetere l'operazione ".
...
Messaggio di errore: Non è stato\<possibile aprire il nome assembly ' assembly ' >'. Verificare che il file esista ancora ".
...
Per correggere l'errore, contattare il fornitore del componente. In alternativa, se si tratta di un componente creato, chiamare `IContainer.Add` il metodo nel costruttore del componente.
...
### <a name="could-not-access-the-variable-variable-name-because-it-has-not-been-initialized-yet"></a>Non è stato possibile accedere alla\<variabile ' nome variabile >' perché non è ancora stata inizializzata
...
### <a name="could-not-find-type-type-name"></a>Il tipo '\<type name >' non è stato trovato
...
Questo errore si verifica perché una variabile membro è stata dichiarata due volte, ad esempio due `Button1` controlli denominati sono dichiarati nel codice. I nomi devono essere univoci tra i form ereditati. Inoltre, i nomi non possono differire solo per maiuscole/minuscole.
...
Messaggio di errore: "Non è stato possibile analizzare\<il metodo ' nome metodo >'. Il parser ha segnalato l'errore seguente:\<' error string >'. Esaminare la Elenco attività per individuare eventuali errori. "
...
Quando si definisce una classe in più file usando la parola chiave [partial](/dotnet/csharp/language-reference/keywords/partial-type) , è possibile avere una sola definizione parziale in ogni file.
...
Questo errore è simile a "Impossibile trovare il\<tipo ' type name >'", ma questo errore si verifica in genere a causa di un attributo di metadati. Per correggere l'errore, verificare che venga fatto riferimento a tutti gli assembly utilizzati dagli attributi.
...
Questo errore si è verificato perché la classe di base dell'oggetto passato alla finestra di progettazione è [astratta](/dotnet/csharp/language-reference/keywords/abstract), che non è consentita.
...
### <a name="the-language-parser-class-class-name-is-not-implemented-properly"></a>La classe del parser del\<linguaggio ' nome classe >' non è implementata correttamente
...
### <a name="the-service-service-name-is-required-but-could-not-be-located"></a>Il servizio '\<nome servizio >' è obbligatorio, ma non è stato trovato

Messaggio di errore: "Il nome del\<servizio ' del servizio >' è obbligatorio, ma non è stato possibile individuarlo. Potrebbe essersi verificato un problema con l'installazione di Visual Studio. "
...
### <a name="the-toolbox-item-for-toolbox-item-name-could-not-be-retrieved-from-the-toolbox"></a>Impossibile recuperare l'elemento della\<casella degli strumenti per il nome dell'elemento della casella degli strumenti >' dalla casella degli strumenti

Messaggio di errore: "Impossibile recuperare l'elemento della\<casella degli strumenti per il nome dell'elemento della casella degli strumenti >' dalla casella degli strumenti. Provare a rimuovere l'elemento dalla casella degli strumenti e aggiungerlo di nuovo. "
...
### <a name="the-variable-variable-name-is-either-undeclared-or-was-never-assigned"></a>La variabile '\<nome variabile >' non è dichiarata o non è mai stata assegnata
...
### <a name="there-is-already-a-command-handler-for-the-menu-command-menu-command-name"></a>Esiste già un gestore comando per il comando di menu '\<nome comando di menu >'
...
Messaggio di errore: "Esiste già un componente denominato"\<nome componente > ". I componenti devono avere nomi univoci e i nomi non devono fare distinzione tra maiuscole e minuscole. Un nome non può inoltre entrare in conflitto con il nome di un componente in una classe ereditata ".
...
Messaggio di errore: "Impossibile aggiungere il riferimento '\<nome riferimento >' all'applicazione corrente. Verificare che non sia già stato fatto\<riferimento a una versione diversa di ' nome riferimento >'.
...
### <a name="unable-to-find-property-property-name-on-page-options-dialog-box-tab-name"></a>Impossibile trovare la proprietà'\<nome proprietà >' nella pagina '\<opzioni Nome scheda finestra di dialogo >'
...
### <a name="visual-studio-is-unable-to-open-document-name-in-design-view"></a>Visual Studio non è in grado di\<aprire ' Document Name >' in visualizzazione progettazione

Messaggio di errore: "Impossibile aprire '\<Document Name >' in visualizzazione progettazione. Nessun parser installato per il tipo di file ".
