...
> [!NOTE]
>  Per impostazione predefinita, gli strumenti che generano distribuzioni ClickOnce predefinita queste distribuzioni per la richiesta di attendibilità da computer sul quale vengono eseguite. Se si decide che si desidera che i vantaggi di sicurezza dell'esecuzione in attendibilità parziale, è necessario modificare questa impostazione predefinita in Visual Studio o uno degli strumenti di Windows Software Development Kit (SDK) (Mage.exe o MageUI.exe). Per altre informazioni sulla sicurezza di Windows Form e su come determinare il livello di attendibilità appropriato per l'applicazione, vedere [protezione in Windows Forms Overview](security-in-windows-forms-overview.md).  
