...
> [!NOTE]
> Nella versione di .NET Framework 1.0 Service Pack 1 e Service Pack 2, Internet zona codice gruppo riceve il "Nothing" set di autorizzazioni. In tutte le altre versioni di .NET Framework, il gruppo di codice dell'area Internet riceve il set di autorizzazioni Internet.
>
...
Quando si sviluppa un'applicazione che verrà eseguita in un ambiente parzialmente attendibile, occorre tenere traccia delle autorizzazioni indispensabili all'applicazione e di quelle che potrebbe usare facoltativamente. Quando si conoscono tutte le autorizzazioni, è necessario effettuare una richiesta dichiarativa per le autorizzazioni a livello di applicazione. .NET Framework runtime le autorizzazioni necessarie all'applicazione e le autorizzazioni in modo specifico non vuole quelle indesiderate. Per altre informazioni sulla richiesta di autorizzazioni, vedere [Richiesta di autorizzazioni](https://docs.microsoft.com/previous-versions/dotnet/netframework-4.0/yd267cce(v=vs.100)).
