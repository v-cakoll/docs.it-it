...
## <a name="parameters"></a>Parametri  
 `celt`  
 in Numero di `ICorDebugBreakpoint` istanze da recuperare.  
  
 `breakpoints`  
 out Matrice di puntatori, ciascuno dei quali punta a un `ICorDebugBreakpoint` oggetto che rappresenta un punto di interruzione.  
  
 `pceltFetched`  
 out Puntatore al numero di `ICorDebugBreakpoint` istanze effettivamente restituite. Questo valore può essere null se `celt` è uno.  
