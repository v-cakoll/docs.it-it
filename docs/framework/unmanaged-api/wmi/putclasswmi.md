...
`pObject`    
[in] Puntatore a una definizione di classe valido. Deve essere inizializzata correttamente con tutti i valori di proprietà obbligatoria.
...
|---------|---------|---------|
| `WBEM_FLAG_USE_AMENDED_QUALIFIERS` | 0x20000 | Se impostato, WMI non archivia i qualificatori con la versione modificata. </br> Se non impostato, si presuppone che non viene localizzato con questo oggetto e tutti i qualificatori vengono storedwith questa istanza. |
| `WBEM_FLAG_CREATE_OR_UPDATE` | 0 | Se non esiste, o sovrascrivere il file se esiste già, creare la classe. |
...
| `WBEM_FLAG_OWNER_UPDATE` | 0x10000. | I provider di push è necessario specificare questo flag quando si chiama `PutClassWmi` per indicare che questa classe è stata modificata. |
| `WBEM_FLAG_UPDATE_COMPATIBLE` | 0 | Consente a una classe da aggiornare se sono presenti nessuna classe derivata e nessuna istanza di tale classe. Consente inoltre aggiornamenti in tutti i casi se la modifica è qualificatori irrilevanti, ad esempio il qualificatore di descrizione. Se la classe dispone di istanze o le modifiche devono qualificatori importanti, l'aggiornamento ha esito negativo. |
| `WBEM_FLAG_UPDATE_SAFE_MODE` | 0x20 | Consente gli aggiornamenti delle classi, anche se esistono classi figlio, purché la modifica non causa conflitti con le classi figlie. Ad esempio, questo flag consente una nuova proprietà da aggiungere alla classe di base che non è stata indicata in precedenza in una qualsiasi delle classi figlio. Se la classe dispone di istanze, l'aggiornamento ha esito negativo. |
| `WBEM_FLAG_UPDATE_FORCE_MODE` | 0x40 | forza gli aggiornamenti delle classi quando esistono classi figlio in conflitto. Ad esempio, questo flag forza un aggiornamento, un qualificatore di classe è definito in una classe figlio, se la classe di base tenta di aggiungere lo stesso qualificatore che è in conflitto con graduazione uno esistente. In modalità force, il conflitto di tis viene risolto eliminando il qualificatore in conflitto nella classe figlio. |
...
`ppCallResult`  
[out] Se `null`, questo parametro è inutilizzato. Se `lFlags` contiene `WBEM_FLAG_RETURN_IMMEDIATELY`, la funzione restituisce immediatamente il `WBEM_S_NO_ERROR`. Il `ppCallResult` parametro riceve un puntatore a un nuovo [IWbemCallResult](https://msdn.microsoft.com/library/aa391425(v=vs.85).aspx) oggetto.
...
Questa funzione esegue il wrapping di una chiamata al [IWbemServices::PutClass](https://msdn.microsoft.com/library/aa392113(v=vs.85).aspx) metodo.

L'utente non può creare classi con nomi che iniziano o terminano con un carattere di sottolineatura chacater
