...
| `WBEM_FLAG_OWNER_UPDATE` | 0x10000 | I provider di push devono specificare questo flag `PutClassWmi` quando si chiama per indicare che questa classe è stata modificata. |
...
| `WBEM_E_INVALID_CLASS` | 0x80041010 | La classe specificata non è valida. In genere, indica che `pObject` specifica un oggetto istanza. |
...
| `WBEM_E_NOT_FOUND` | 0x80041002 | `WBEM_FLAG_UPDATE_ONLY`è stato specificato `lFlags`in e la classe non è stata trovata. |
