...
`wszName`\
in Nome del metodo. Questo parametro non può essere `null` e deve puntare a una `LPCWSTR`valida.
...
In `ppInSignature` e `ppOutSignature` descrivono rispettivamente i parametri in e out come proprietà in un'istanza `IWbemClassObject` della classe di sistema [_Parameters](/windows/desktop/WmiSdk/--parameters). Le proprietà in `ppInSignature` sono denominate `Param`*n*, dove *n* è la posizione del parametro nella firma del metodo, ad esempio `Param1`, `Param2`e così via. Anche le proprietà in `ppOutSignature` sono denominate `Param`*n*e il valore restituito è denominato `ReturnValue`. Per ulteriori informazioni e un esempio, vedere [Metodo IWbemClassObject:: GetMethod](/windows/desktop/api/wbemcli/nf-wbemcli-iwbemclassobject-getmethod).
