...
`strNetworkResource` [in] Puntatore a un valore valido `BSTR` che contiene il percorso dell'oggetto dello spazio dei nomi WMI corretto. Vedere il [osservazioni](#remarks) sezione per ulteriori informazioni.

`strUser` [in] Un puntatore a un valore valido `BSTR` che contiene il nome utente. Oggetto `null` valore indica il contesto di sicurezza corrente. Se l'utente è di un dominio diverso da quello corrente, `strUser` può inoltre contenere il nome di dominio e separato da una barra rovesciata. `strUser` può anche essere utente nome principale (UPN) formattare, suhc come *userName@domainName*. Vedere il [osservazioni](#remarks) sezione per ulteriori informazioni.
