...
Il chiamante inizia la sequenza di enumerazione utilizzando la funzione `WBEM_S_NO_MORE_DATA` [BeginMethodEnumeration](beginmethodenumeration.md), quindi chiama la funzione [NextMethod](nextmethod.md )finché il metodo non restituisce . Facoltativamente, il chiamante completa `EndMethodEnumeration`la sequenza chiamando . Il chiamante può terminare `EndMethodEnumeration` l'enumerazione in anticipo chiamando in qualsiasi momento.
