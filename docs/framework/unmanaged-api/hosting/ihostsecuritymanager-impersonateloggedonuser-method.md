...
## <a name="parameters"></a>Parametri  
 `hToken`  
 in Token che rappresenta le credenziali dell'utente da rappresentare.  
  
## <a name="return-value"></a>Valore restituito  
  
|HRESULT|Descrizione|  
|-------------|-----------------|  
|S_OK|`ImpersonateLoggedOnUser` ha restituito un esito positivo.|  
...
## <a name="remarks"></a>Note  
 Chiamare `LogonUser` o una funzione Win32 correlata per ottenere un handle per le credenziali dell'identità utente corrente.  
  
 Il tipo di `HANDLE` non è conforme a COM, ovvero la sua dimensione è specifica di un sistema operativo e richiede il marshalling personalizzato. Questo token viene quindi utilizzato solo all'interno del processo, tra CLR e l'host.  
