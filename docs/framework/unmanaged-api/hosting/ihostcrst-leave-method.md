...
Lascia la sezione critica rappresentata dall'istanza corrente di [IHostCrst](ihostcrst-interface.md).  
...
|HRESULT|Descrizione|  
|-------------|-----------------|  
|S_OK|`Leave`la restituzione è riuscita.|  
...
## <a name="remarks"></a>Osservazioni  
 `Leave`consente a CLR di comunicare direttamente con l'implementazione del threading dell'host, anziché utilizzare la funzione Win32 corrispondente `LeaveCriticalSection` . Un thread che acquisisce la proprietà della sezione critica rappresentata dall' `IHostCrst` istanza corrente deve chiamare `Leave` una volta per ogni volta che accede alla sezione critica.  
