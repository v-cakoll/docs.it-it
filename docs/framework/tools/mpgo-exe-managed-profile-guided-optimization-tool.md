...
|`-Timeout` \<*secondi*>|The amount of time to allow the Windows 8.x Store app to run before the app exits.|
...
- Non è possibile usare percorsi tra virgolette con la barra finale, perché anche le macro di Visual Studio usano la barra finale per impostazione predefinita (For example, `–OutDir "C:\Output Folder\"` is invalid.) To work around this restriction, you can escape the trailing slash. Ad esempio, usare invece `-OutDir "$(OutDir)\"`.  
