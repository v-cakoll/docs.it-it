...
|`/ac`  *ﬁle*|Aggiunge un altro certificato dal *file* al blocco di firme.|  
...
|`/p7`*Percorso*|Specifica di produrre un file PKCS (Public Key Cryptography Standards) #7 per ogni file di contenuto specificato. PKCS #7 i file sono denominati *nomefile*\\ *.p7.*|  
...
|`/ag` *CatDBGUID*|Trova il catalogo nel database dei cataloghi identificato da *GUIDDatabaseCatalogo*.|  
...
|`/pg` *PolicyGUID*|Specifica i criteri di verifica in base al GUID. *GUIDCriteri* corrisponde all'IDAzione dei criteri di verifica. Non è possibile usare questa opzione con le opzioni `catdb`.|  
