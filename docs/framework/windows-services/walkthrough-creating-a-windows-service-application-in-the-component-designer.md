...
1.  [Creazione di un servizio](#BK_CreateProject) usando il modello di progetto del **servizio Windows** e configurarlo. Tale modello crea automaticamente una classe che eredita da <xref:System.ServiceProcess.ServiceBase?displayProperty=nameWithType> e scrive la maggior parte del codice di base del servizio, ad esempio il codice per avviare il servizio.  
...
3.  [Impostazione dello stato del servizio](#BK_SetStatus). Per impostazione predefinita, i servizi creati con <xref:System.ServiceProcess.ServiceBase?displayProperty=nameWithType> implementano solo un subset di flag di stato disponibili. Se le operazioni di avvio, sospensione e arresto del servizio richiedono molto tempo, è possibile implementare i valori di stato come Avvio in sospeso o Arresto in sospeso per indicare che è in corso un'operazione.  
  
4.  [Aggiunta di programmi di installazione al servizio](#BK_AddInstallers) per l'applicazione di servizio.  
  
5.  (Facoltativo) [Impostare i parametri di avvio](#BK_StartupParameters), specificare gli argomenti di avvio predefiniti e consentire agli utenti di eseguire l'override delle impostazioni predefinite quando avviano il servizio manualmente.  
...
#### <a name="to-create-and-configure-your-service"></a>Per creare e configurare il servizio  
...
2.  Nell'elenco dei modelli di progetto di Visual Basic o Visual C# scegliere **Servizio Windows**e assegnare al progetto il nome **MyNewService**. Scegliere **OK**.  
     ...
     Il modello di progetto aggiunge automaticamente una classe di componente denominata `Service1` che eredita dalla classe <xref:System.ServiceProcess.ServiceBase?displayProperty=nameWithType>.  
...
## <a name="adding-features-to-the-service"></a>Aggiunta di funzionalità al servizio  
...
#### <a name="to-add-custom-event-log-functionality-to-your-service"></a>Per aggiungere la funzionalità del log eventi personalizzato al servizio  
...
#### <a name="to-define-what-occurs-when-the-service-starts"></a>Per definire quello che accade quando il servizio viene avviato  
...
-   Nell'editor di codice individuare il metodo <xref:System.ServiceProcess.ServiceBase.OnStart%2A> di cui è stato eseguito automaticamente l'override al momento della creazione del progetto e sostituire il codice esistente con il codice seguente. In questo modo viene aggiunta una voce nel log eventi all'avvio dell'esecuzione del servizio:  
    ...
    ```  
     ...
     Potrebbe essere necessario eseguire le attività usando un thread di lavoro in background invece di eseguire tutto il lavoro sul thread principale. Per un esempio, vedere la pagina di riferimento <xref:System.ServiceProcess.ServiceBase?displayProperty=nameWithType>.  
  
#### <a name="to-define-what-occurs-when-the-service-is-stopped"></a>Per definire quello che accade quando il servizio viene interrotto  
...
#### <a name="to-define-other-actions-for-the-service"></a>Per definire altre azioni del servizio  
...
## <a name="setting-service-status"></a>Impostazione dello stato del servizio  
...
#### <a name="to-implement-service-pending-status"></a>Per implementare lo stato in sospeso del servizio  
  
1.  Aggiungere un'istruzione `using` o una dichiarazione `Imports` allo spazio dei nomi <xref:System.Runtime.InteropServices?displayProperty=nameWithType> nel file MyNewService.cs o MyNewService.vb:  
...
3.  A questo punto, nella classe `MyNewService` dichiarare la [funzione SetServiceStatus](/windows/desktop/api/winsvc/nf-winsvc-setservicestatus) mediante pInvoke:  
...
## <a name="adding-installers-to-the-service"></a>Aggiunta di programmi di installazione al servizio  
...
#### <a name="to-create-the-installers-for-your-service"></a>Per creare i programmi di installazione per il servizio  
...
## <a name="set-startup-parameters"></a>Impostare i parametri di avvio  
...
#### <a name="adding-startup-parameters"></a>Aggiunta di parametri di avvio  
...
#### <a name="to-build-your-service-project"></a>Per compilare il progetto di servizio  
...
<a name="BK_Install"></a>   
## <a name="installing-the-service"></a>Installazione del servizio  
...
#### <a name="to-install-a-windows-service"></a>Per installare un servizio Windows  
...
## <a name="starting-and-running-the-service"></a>Avvio ed esecuzione del servizio  
  
#### <a name="to-start-and-stop-your-service"></a>Per avviare e interrompere il servizio  
...
#### <a name="to-verify-the-event-log-output-of-your-service"></a>Per verificare l'output del log eventi del servizio  
...
#### <a name="to-uninstall-your-service"></a>Per disinstallare il servizio  
