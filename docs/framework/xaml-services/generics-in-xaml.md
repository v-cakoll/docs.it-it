...
## <a name="generics-support-in-wpf-and-other-v35-frameworks"></a>Supporto dei generics in WPF e altri framework v3.5  
 Per l'utilizzo di XAML 2006 quando la destinazione è WPF, in particolare [X:Class](../../../docs/framework/xaml-services/x-class-directive.md) deve anche essere specificato per lo stesso elemento come `x:TypeArguments`, e tale elemento deve essere l'elemento radice in un documento XAML. L'elemento radice deve eseguire il mapping a un tipo generico con almeno un argomento di tipo. Un esempio è <xref:System.Windows.Navigation.PageFunction%601>.  
