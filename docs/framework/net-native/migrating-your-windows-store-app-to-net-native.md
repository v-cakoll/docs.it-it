...
- [<span data-ttu-id="a52e0-112">Scenari e API non supportati</span><span class="sxs-lookup"><span data-stu-id="a52e0-112">Unsupported scenarios and APIs</span></span>](#Unsupported)
...
- <span data-ttu-id="a52e0-187">I tipi di valore maggiori di 1 megabyte non sono supportati.</span><span class="sxs-lookup"><span data-stu-id="a52e0-187">Value types larger than 1 megabyte aren't supported.</span></span>
...
- <span data-ttu-id="a52e0-201">L'espansione di tipo generico infinito genera un errore del compilatore.</span><span class="sxs-lookup"><span data-stu-id="a52e0-201">Infinite generic type expansion results in a compiler error.</span></span> <span data-ttu-id="a52e0-202">Ad esempio, questo codice non viene compilato:</span><span class="sxs-lookup"><span data-stu-id="a52e0-202">For example, this code fails to compile:</span></span>
...
<span data-ttu-id="a52e0-228">**Proxy**</span><span class="sxs-lookup"><span data-stu-id="a52e0-228">**Proxy**</span></span>
...
- <span data-ttu-id="a52e0-262">L'intestazione `SetCookie2` nelle risposte verrà ignorata come obsoleta.</span><span class="sxs-lookup"><span data-stu-id="a52e0-262">The `SetCookie2` header in responses is ignored as obsolete.</span></span>
...
<span data-ttu-id="a52e0-308">Visual Basic non è attualmente supportata in .NET Native.</span><span class="sxs-lookup"><span data-stu-id="a52e0-308">Visual Basic isn't currently supported in .NET Native.</span></span> <span data-ttu-id="a52e0-309">I seguenti tipi negli <xref:Microsoft.VisualBasic> <xref:Microsoft.VisualBasic.CompilerServices> spazi dei nomi e non sono disponibili in .NET native:</span><span class="sxs-lookup"><span data-stu-id="a52e0-309">The following types in the <xref:Microsoft.VisualBasic> and <xref:Microsoft.VisualBasic.CompilerServices> namespaces aren't available in .NET Native:</span></span>
...
### <a name="differences-in-serializers"></a><span data-ttu-id="a52e0-317">Differenze nei serializzatori</span><span class="sxs-lookup"><span data-stu-id="a52e0-317">Differences in serializers</span></span>
...
- <span data-ttu-id="a52e0-329"><xref:System.Xml.Serialization.XmlSerializer> non riesce a generare il codice per un tipo che ha metodi con uno qualsiasi dei seguenti attributi:</span><span class="sxs-lookup"><span data-stu-id="a52e0-329"><xref:System.Xml.Serialization.XmlSerializer> fails to generate code for a type that has methods attributed with any of the following attributes:</span></span>
...
- <span data-ttu-id="a52e0-330"><xref:System.Xml.Serialization.XmlSerializer> non soddisfa l'interfaccia di serializzazione personalizzata di <xref:System.Xml.Serialization.IXmlSerializable> .</span><span class="sxs-lookup"><span data-stu-id="a52e0-330"><xref:System.Xml.Serialization.XmlSerializer> doesn't honor the <xref:System.Xml.Serialization.IXmlSerializable> custom serialization interface.</span></span> <span data-ttu-id="a52e0-331">Se si ha una classe che implementa questa interfaccia, <xref:System.Xml.Serialization.XmlSerializer> considera il tipo come un oggetto Plain Old CLR Object (POCO) di CLR e ne serializza solo le proprietà pubbliche.</span><span class="sxs-lookup"><span data-stu-id="a52e0-331">If you have a class that implements this interface, <xref:System.Xml.Serialization.XmlSerializer> considers the type a plain old CLR object (POCO) type and serializes only its public properties.</span></span>

- <span data-ttu-id="a52e0-332">La serializzazione di un <xref:System.Exception> oggetto normale non funziona correttamente con <xref:System.Runtime.Serialization.DataContractSerializer> e <xref:System.Runtime.Serialization.Json.DataContractJsonSerializer> .</span><span class="sxs-lookup"><span data-stu-id="a52e0-332">Serializing a plain <xref:System.Exception> object doesn't work well with <xref:System.Runtime.Serialization.DataContractSerializer> and <xref:System.Runtime.Serialization.Json.DataContractJsonSerializer>.</span></span>

<a name="VS"></a>

## <a name="visual-studio-differences"></a><span data-ttu-id="a52e0-333">Differenze di Visual Studio</span><span class="sxs-lookup"><span data-stu-id="a52e0-333">Visual Studio differences</span></span>

<span data-ttu-id="a52e0-334">**Eccezioni e debug**</span><span class="sxs-lookup"><span data-stu-id="a52e0-334">**Exceptions and debugging**</span></span>

<span data-ttu-id="a52e0-335">Quando si eseguono app compilate usando .NET Native nel debugger, le eccezioni first-chance sono abilitate per i seguenti tipi di eccezione:</span><span class="sxs-lookup"><span data-stu-id="a52e0-335">When you're running apps compiled by using .NET Native in the debugger, first-chance exceptions are enabled for the following exception types:</span></span>

- <xref:System.MemberAccessException>

- <xref:System.TypeAccessException>

<span data-ttu-id="a52e0-336">**Creazione di app**</span><span class="sxs-lookup"><span data-stu-id="a52e0-336">**Building apps**</span></span>

<span data-ttu-id="a52e0-337">Usare gli strumenti di compilazione x86 usati per impostazione predefinita da Visual Studio.</span><span class="sxs-lookup"><span data-stu-id="a52e0-337">Use the x86 build tools that are used by default by Visual Studio.</span></span> <span data-ttu-id="a52e0-338">Non è consigliabile usare gli strumenti di MSBuild AMD64, disponibili in C:\Program Files (x86)\MSBuild\12.0\bin\amd64, perché possono creare problemi di compilazione.</span><span class="sxs-lookup"><span data-stu-id="a52e0-338">We don't recommend using the AMD64 MSBuild tools, which are found in C:\Program Files (x86)\MSBuild\12.0\bin\amd64; these may create build problems.</span></span>

<span data-ttu-id="a52e0-339">**Profiler**</span><span class="sxs-lookup"><span data-stu-id="a52e0-339">**Profilers**</span></span>

- <span data-ttu-id="a52e0-340">Il profiler della CPU di Visual Studio e il profiler della memoria XAML non visualizzano Just My Code correttamente.</span><span class="sxs-lookup"><span data-stu-id="a52e0-340">The Visual Studio CPU Profiler and XAML Memory Profiler don't display Just-My-Code correctly.</span></span>

- <span data-ttu-id="a52e0-341">Il profiler della memoria di XAML non visualizza in modo accurato i dati di heap gestito.</span><span class="sxs-lookup"><span data-stu-id="a52e0-341">The XAML Memory Profiler doesn't accurately display managed heap data.</span></span>

- <span data-ttu-id="a52e0-342">Il profiler della CPU non identifica correttamente i moduli e consente di visualizzare i nomi di funzione con prefisso.</span><span class="sxs-lookup"><span data-stu-id="a52e0-342">The CPU Profiler doesn't correctly identify modules, and displays prefixed function names.</span></span>

<span data-ttu-id="a52e0-343">**Progetti di librerie unit test**</span><span class="sxs-lookup"><span data-stu-id="a52e0-343">**Unit Test Library projects**</span></span>

<span data-ttu-id="a52e0-344">L'abilitazione di .NET Native in una libreria di unit test per un progetto di app di Windows Store non è supportata e causa la mancata compilazione del progetto.</span><span class="sxs-lookup"><span data-stu-id="a52e0-344">Enabling .NET Native on a Unit Test Library for a Windows Store apps project isn't supported and causes the project to fail to build.</span></span>

## <a name="see-also"></a><span data-ttu-id="a52e0-345">Vedere anche</span><span class="sxs-lookup"><span data-stu-id="a52e0-345">See also</span></span>

- [<span data-ttu-id="a52e0-346">Per iniziare</span><span class="sxs-lookup"><span data-stu-id="a52e0-346">Getting Started</span></span>](getting-started-with-net-native.md)
- [<span data-ttu-id="a52e0-347">Riferimento a file di configurazione di direttive di runtime (rd.xml)</span><span class="sxs-lookup"><span data-stu-id="a52e0-347">Runtime Directives (rd.xml) Configuration File Reference</span></span>](runtime-directives-rd-xml-configuration-file-reference.md)
- [<span data-ttu-id="a52e0-348">Panoramica di .NET per applicazioni Windows Store</span><span class="sxs-lookup"><span data-stu-id="a52e0-348">.NET For Windows Store apps overview</span></span>](https://docs.microsoft.com/previous-versions/windows/apps/br230302%28v=vs.140%29)
- [<span data-ttu-id="a52e0-349">Supporto .NET Framework per applicazioni Windows Store e Windows Runtime</span><span class="sxs-lookup"><span data-stu-id="a52e0-349">.NET Framework Support for Windows Store Apps and Windows Runtime</span></span>](../../standard/cross-platform/support-for-windows-store-apps-and-windows-runtime.md)
