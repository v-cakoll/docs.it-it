...
2. Consente di specificare se devono essere applicate le regole di analisi IRI. A questo scopo, è possibile usare il file machine.config o il file app.config.  
 ...
 L'abilitazione dell'analisi IRI (iriParsing enabled = `true`) effettuerà la normalizzazione e il controllo dei caratteri in base alle regole IRI più recenti nella specifica RFC 3987. Il valore predefinito è `false` e effettuerà la normalizzazione e il controllo dei caratteri in base a RFC 2396 e RFC 3986 (per i valori letterali IPv6).  
