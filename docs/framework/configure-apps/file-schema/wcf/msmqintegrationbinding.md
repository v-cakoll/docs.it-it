...
|receiveContextEnabled|Valore booleano che specifica se il contesto di ricezione per l'elaborazione di messaggi nelle code è abilitato. Quando è impostato su `true`, un servizio può "Visualizza" un messaggio nella coda per avviare l'elaborazione e, se si verifica un errore e viene generata un'eccezione, rimane nella coda. I servizi possono anche "bloccare" i messaggi per ritentare l'elaborazione in un secondo momento. ReceiveContext offre un meccanismo per "completamento" del messaggio una volta elaborato, pertanto può essere rimosso dalla coda. I messaggi non vengano letti e riscritti nelle code in rete e singoli messaggi non saranno rimbalzati tra diverse istanze del servizio durante l'elaborazione.|  
