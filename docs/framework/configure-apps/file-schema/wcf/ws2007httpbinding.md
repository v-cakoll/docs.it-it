...
|`messageEncoding`|Definisce il codificatore usato per codificare il messaggio. Di seguito vengono elencati i valori validi:<br /><br /> -   `Text`: utilizzare un codificatore di messaggi di testo.<br />-   `Mtom`: usare un codificatore Message Transmission Organization Mechanism 1,0 (MTOM).<br /><br /> Il valore predefinito è `Text`.<br /><br /> L'attributo è di tipo <xref:System.ServiceModel.WSMessageEncoding>.|  
