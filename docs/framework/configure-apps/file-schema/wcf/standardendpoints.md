...
|[\<webHttpEndpoint>](webhttpendpoint.md)|Definisce un endpoint standard con un'associazione di [ \<> WebHttpBinding](webhttpbinding.md) fissa che aggiunge automaticamente il [ \<comportamento di > WebHttp](webhttp.md) . Usare questo endpoint per la scrittura di un servizio REST.|  
