...
1. Creare il servizio. Per un'esercitazione su questa procedura di base, vedere [Getting Started Tutorial](getting-started-tutorial.md).  
...
1. Creare un codice client del servizio tramite [ServiceModel Metadata Utility Tool (Svcutil.exe)](servicemodel-metadata-utility-tool-svcutil-exe.md). Per ulteriori informazioni, vedere [accesso ai servizi tramite un client WCF](accessing-services-using-a-wcf-client.md).  
...
2. Dopo aver creato il client WCF, impostare la proprietà <xref:System.ServiceModel.Security.WindowsClientCredential.AllowedImpersonationLevel%2A> della classe <xref:System.ServiceModel.Security.WindowsClientCredential> su uno dei valori di enumerazione di <xref:System.Security.Principal.TokenImpersonationLevel>.  
    ...
    > [!NOTE]
    > Per usare <xref:System.Security.Principal.TokenImpersonationLevel.Delegation>, deve essere usata l'autenticazione Kerberos negoziata, a volte denominata *multifase* o *in più passaggi* . Per una descrizione di come implementare questa, vedere [procedure consigliate per la sicurezza](./feature-details/best-practices-for-security-in-wcf.md).  
...
- [Rappresentazione del client](./samples/impersonating-the-client.md)
