...
La specifica WS-Addressing definisce la nozione di un riferimento dell'endpoint come un modo di indirizzare un particolare endpoint servizio Web. In WCF, i riferimenti agli endpoint vengono modellati tramite il `EndpointAddress` Class-`EndpointAddress` è il tipo del campo Address della classe `ServiceEndpoint`.

Parte del modello di riferimento all'endpoint è che ogni riferimento può portare alcuni parametri per riferimento che aggiungono informazioni di identificazione aggiuntive. In WCF questi parametri di riferimento vengono modellati come istanze della classe `AddressHeader`.
