...
Il supporto AJAX in Windows Communication Foundation (WCF) è ottimizzato per l'uso con ASP.NET `ScriptManager` AJAX tramite il controllo. Per un esempio di utilizzo di WCF con ASP.NET AJAX, vedere gli [esempi di AJAX](../../../../docs/framework/wcf/samples/ajax-service-using-http-post.md).
