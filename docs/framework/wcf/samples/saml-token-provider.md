...
- Se si sta compilando un token personalizzato.
 ...
 Questo esempio illustra come compilare un provider di token personalizzato che consente l'uso di un token SAML ottenuto dall'esterno del framework client WCF.
...
- Come viene fornito il token SAML al framework client WCF.
...
```
 ...
 I passaggi seguenti illustrano come sviluppare un provider di token SAML personalizzato e integrarlo con WCF: Security Framework:
...
1. Eseguire Setup. bat dalla cartella di installazione dell'esempio all'interno di un prompt dei comandi di Visual Studio 2012 eseguire con privilegi di amministratore. In questo modo vengono installati tutti i certificati necessari per l'esecuzione dell'esempio.
