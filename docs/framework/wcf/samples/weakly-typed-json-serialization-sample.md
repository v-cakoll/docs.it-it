...
```  
 ...
 Il tipo `JsonObject` fornito in questo esempio introduce una rappresentazione con tipizzazione debole dell'oggetto JSON deserializzato. `JsonObject` si basa sul mapping naturale tra oggetti JSON e dizionari di .NET Framework e il mapping tra matrici JSON e matrici di .NET Framework. Nel codice seguente viene illustrato il tipo `JsonObject` .  
