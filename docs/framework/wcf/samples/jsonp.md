...
```  
 ...
 L'esempio è costituito da due applicazioni web ASP.NET: una contiene solo un servizio WCF e un'altra contiene la pagina Web. aspx, che chiama il servizio. Quando la soluzione viene eseguita, in [!INCLUDE[vs_current_long](../../../../includes/vs-current-long-md.md)] verranno ospitati i due siti web su porte diverse, creando un ambiente in cui il servizio e il client sono presenti in domini diversi.  
