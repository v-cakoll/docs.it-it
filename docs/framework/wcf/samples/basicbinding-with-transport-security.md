...
Questo esempio dimostra l'uso della sicurezza del trasporto SSL con l'associazione di base. Questo esempio si basa sul [Introduzione](getting-started-sample.md) che implementa un servizio di calcolatrice.
...
Il codice programma nell'esempio è identico a quello del servizio [Introduzione](getting-started-sample.md) . Le definizioni dell'endpoint e dell'associazione nelle impostazioni del file di configurazione sono state modificate per consentire la comunicazione protetta, come illustrato nella configurazione di esempio seguente.
