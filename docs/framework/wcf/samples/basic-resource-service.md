...
ms.locfileid: "43520136"
---
# <a name="basic-resource-service"></a>Servizio risorse di base
In questo esempio viene illustrato come implementare un servizio basato su HTTP usando il modello di programmazione REST di Windows Communication Foundation (WCF) che espone una raccolta di clienti che supporta il recupero, aggiungere, eliminare e sostituire operazioni. Questo esempio è costituito da 2 componenti - un servizio HTTP WCF indipendente (Service.cs) e un'applicazione console (program.cs) che crea il servizio ed effettua chiamate ad esso.  
  
## <a name="sample-details"></a>Dettagli dell'esempio  
 Il servizio WCF espone una raccolta di clienti in modo risorse-oriented/REST. In breve, ciò significa disporre di URI univoci per la raccolta di clienti e per ogni cliente nella raccolta. Il servizio supporta l'invio di un HTTP `GET` all'URI della raccolta per recuperare l'intera raccolta e di HTTP `POST` all'URI della raccolta per aggiungere un nuovo cliente alla raccolta. Anche a livello di URI per singolo cliente, supporta `GET` HTTP per ottenere i dettagli del cliente, `PUT` HTTP per sostituire i dettagli del cliente e `DELETE` HTTP per rimuovere il cliente dalla raccolta. Quando un nuovo cliente viene aggiunto alla raccolta, il servizio gli assegna un URI univoco e lo archivia come parte dei dettagli del cliente. Inoltre, comunica l'URI al client utilizzando l'intestazione HTTP relativa al percorso della risposta.  
  
 Il file App.config configura il servizio WCF con un endpoint <xref:System.ServiceModel.Description.WebHttpEndpoint> predefinito in cui la proprietà <xref:System.ServiceModel.Description.WebHttpEndpoint.HelpEnabled%2A> è impostata su `true`. Di conseguenza, WCF consente di creare una pagina automatica della Guida basata su HTML all'indirizzo `http://localhost:8000/Customers/help` che fornisce informazioni su come costruire HTTP richieste al servizio e come accedere alla risposta del servizio HTTP, ad esempio, un esempio di come sia i dettagli del cliente rappresentati in XML e JSON.  
