...
> [!NOTE]
>  [!INCLUDE[vsprvs](../../../includes/vsprvs-md.md)]è configurato in modo che riconosca il file app. config come file di configurazione per il progetto quando viene eseguito utilizzando il [Host servizio WCF (WcfSvcHost.exe)](../../../docs/framework/wcf/wcf-service-host-wcfsvchost-exe.md), ovvero la configurazione predefinita. Se la libreria di servizi viene inserita in un eseguibile, è necessario spostare il codice di configurazione nel file di configurazione dell'eseguibile, in quanto i file di configurazione per le DLL non sono validi.  
...
#### <a name="changing-the-address-of-the-feed"></a>Modifica dell'indirizzo del feed  
 Il modello di pubblicazione utilizza Internet Explorer durante l'esecuzione. Quando si fa clic su progetto in **Esplora soluzioni** in [!INCLUDE[vsprvs](../../../includes/vsprvs-md.md)]selezionare **proprietà**, quindi selezionare il **Debug** scheda è possibile verificare l'indirizzo predefinito di il modello. Internet Explorer tenterà di aprire il feed a questo indirizzo.  
