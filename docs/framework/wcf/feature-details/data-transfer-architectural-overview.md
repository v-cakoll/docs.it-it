...
|In ingresso dallo stack dei canali del flusso|Un oggetto `Stream` che rappresenta i dati che entrano sulla rete con un <xref:System.Xml.XmlReader> su di esso|Scrivere il contenuto archiviato `XmlReader` usando `WriteNode`|Restituisce l'oggetto memorizzato `XmlReader`|  
|In ingresso dallo stack dei canali senza utilizzo del flusso|Un buffer che contiene dati del corpo con un `XmlReader` su di esso|Scrive il contenuto archiviato `XmlReader` usando `WriteNode`|Restituisce il lang archiviato|  
...
### <a name="transport-channels-and-message-encoders"></a>Canali di trasporto e codificatori di messaggi  
 ...
 La maggior parte dei canali di trasporto utilizza componenti detti *codificatori di messaggi* destinati ad eseguire la codifica e la decodifica. Un codificatore di messaggi è una sottoclasse della classe <xref:System.ServiceModel.Channels.MessageEncoder> . `MessageEncoder` include vari `ReadMessage` e `WriteMessage` overload del metodo per la conversione tra `Message` e gruppi di byte.  
