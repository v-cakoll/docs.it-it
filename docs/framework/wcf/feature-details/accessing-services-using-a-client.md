...
> [!NOTE]
>  Utilizzare la `using` istruzione (`Using` in Visual Basic) non è consigliata. poiché la fine dell'istruzione `using` può causare eccezioni che possono mascherare altre eccezioni che potrebbe essere necessario conoscere. Per altre informazioni, vedere [come evitare problemi con l'istruzione Using](../../../../docs/framework/wcf/samples/avoiding-problems-with-the-using-statement.md).  
