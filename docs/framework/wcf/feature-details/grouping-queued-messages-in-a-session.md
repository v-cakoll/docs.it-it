...
1. Definire un contratto di servizio che richieda una sessione. Eseguire questa operazione con <xref:System.ServiceModel.ServiceContractAttribute> l'attributo specificando:  
  
    ```csharp
    SessionMode=SessionMode.Required  
    ```  
  
2. Poiché questi metodi non restituiscono alcun dato, contrassegnare le operazioni del contratto come unidirezionali. Questa operazione viene eseguita con <xref:System.ServiceModel.OperationContractAttribute> l'attributo specificando:  
