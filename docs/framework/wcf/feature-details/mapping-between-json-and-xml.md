...
|`null`|Nessuno consentito.|Il valore letterale `null`. Nel mapping da JSON a XML, `null` può essere racchiuso tra spazi vuoti ('ws' nella sezione 2) di cui non viene eseguito il mapping al codice XML.<br /><br /> Esempio: l'elemento seguente viene mappato a un frammento JSON.<br /><br /> `<root type="null"/>`<br /><br /> oppure<br /><br /> `<root type="null"></root>`<br /><br /> :<br /><br /> In entrambi i casi il frammento JSON è `Null`.|  
