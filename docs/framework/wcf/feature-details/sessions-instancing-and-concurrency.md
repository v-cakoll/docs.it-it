...
>  Può essere difficile comprendere, scrivere correttamente e sviluppare codice che utilizza in modo sicuro più di un thread. Prima di utilizzare valori <xref:System.ServiceModel.ConcurrencyMode.Multiple> o <xref:System.ServiceModel.ConcurrencyMode.Reentrant> , verificare che il servizio sia progettato correttamente per queste modalità. Per altre informazioni, vedere <xref:System.ServiceModel.ServiceBehaviorAttribute.ConcurrencyMode%2A>.  
 ...
 L'uso della concorrenza è correlato alla modalità di istanza. In <xref:System.ServiceModel.InstanceContextMode.PerCall> istanze, concorrenza non è rilevante, perché ogni messaggio viene elaborato da un nuovo <xref:System.ServiceModel.InstanceContext> e, pertanto mai più di un thread attivo nel <xref:System.ServiceModel.InstanceContext>.  
...
|-------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------|  
|PerCall|-Il comportamento di canale con sessione: una sessione e <xref:System.ServiceModel.InstanceContext> per ogni chiamata.<br />-Il comportamento di canale senza sessione: viene generata un'eccezione.|-Il comportamento di canale con sessione: una sessione e <xref:System.ServiceModel.InstanceContext> per ogni chiamata.<br />-Il comportamento di canale senza sessione: un <xref:System.ServiceModel.InstanceContext> per ogni chiamata.|-Il comportamento di canale con sessione: viene generata un'eccezione.<br />-Il comportamento di canale senza sessione: un <xref:System.ServiceModel.InstanceContext> per ogni chiamata.|  
|PerSession|-Il comportamento di canale con sessione: una sessione e <xref:System.ServiceModel.InstanceContext> per ogni canale.<br />-Il comportamento di canale senza sessione: viene generata un'eccezione.|-Il comportamento di canale con sessione: una sessione e <xref:System.ServiceModel.InstanceContext> per ogni canale.<br />-Il comportamento di canale senza sessione: un <xref:System.ServiceModel.InstanceContext> per ogni chiamata.|-Il comportamento di canale con sessione: viene generata un'eccezione.<br />-Il comportamento di canale senza sessione: un <xref:System.ServiceModel.InstanceContext> per ogni chiamata.|  
|Single|-Il comportamento di canale con sessione: una sessione e un solo <xref:System.ServiceModel.InstanceContext> per tutte le chiamate.<br />-Il comportamento di canale senza sessione: viene generata un'eccezione.|-Il comportamento di canale con sessione: una sessione e <xref:System.ServiceModel.InstanceContext> per il singleton creato o specificato dall'utente.<br />-Il comportamento di canale senza sessione: un <xref:System.ServiceModel.InstanceContext> per il singleton creato o specificato dall'utente.|-Il comportamento di canale con sessione: viene generata un'eccezione.<br />-Il comportamento di canale senza sessione: un <xref:System.ServiceModel.InstanceContext> per ogni singleton creato o per il singleton specificato dall'utente.|  
