---
title: Client
ms.date: 03/30/2017
ms.assetid: cce50025-0318-4a5e-9ce8-c67feab83b06
ms.openlocfilehash: 844412b211fe6187e3475fc71b9c3ccca8dd178f
ms.sourcegitcommit: 3c1c3ba79895335ff3737934e39372555ca7d6d0
ms.translationtype: MT
ms.contentlocale: it-IT
ms.lasthandoff: 09/05/2018
ms.locfileid: "43743108"
---
# <a name="clients"></a>Client
Gli argomenti di questa sezione riguardano l'architettura client, come per accedere a Windows Communication Foundation (WCF) del servizio, come configurare un client WCF e alcune considerazioni sulla protezione per i client WCF.  
  
## <a name="in-this-section"></a>In questa sezione  
 [Architettura client WCF](../../../../docs/framework/wcf/feature-details/client-architecture.md)  
 Descrive l'architettura client WCF.  
  
 [Accesso ai servizi tramite client WCF](../../../../docs/framework/wcf/feature-details/accessing-services-using-a-client.md)  
 Viene descritto come accedere a un servizio WCF.  
  
 [Configurazione del client WCF](../../../../docs/framework/wcf/feature-details/client-configuration.md)  
 Viene descritto come configurare un client WCF.  
  
 [Protezione di client](../../../../docs/framework/wcf/securing-clients.md)  
 Vengono descritte alcune considerazioni sulla sicurezza per i client WCF.  
  
## <a name="see-also"></a>Vedere anche  
 [Esempi di client](https://msdn.microsoft.com/library/849fc452-8718-4d4e-ba57-905eed943f63)
