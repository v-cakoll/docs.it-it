...
-   Quando si importa lo schema `DataSet` o `DataTable`, è importante fare riferimento a questi tipi. Con lo strumento da riga di comando Svcutil.exe, questo può essere eseguito passando il nome dell'assembly System.Data.dll i `/reference` passare. Se si importa lo schema del dataset tipizzato, è necessario fare riferimento al tipo del dataset tipizzato. Con Svcutil.exe, passare al percorso dell'assembly del set di dati tipizzato per il `/reference` passare. [!INCLUDE[crabout](../../../../includes/crabout-md.md)] fa riferimento a tipi, vedere la [importazione dello Schema per generare classi](../../../../docs/framework/wcf/feature-details/importing-schema-to-generate-classes.md).  
