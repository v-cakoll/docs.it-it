...
Servizi AJAX Windows Communication Foundation (WCF) è possibile accedere da qualsiasi pagina Web compatibile con JavaScript, senza necessità di ASP.NET AJAX. Questo argomento descrive come creare un servizio WCF di questo tipo.  
...
## <a name="creating-an-ajax-endpoint"></a>Creazione di un endpoint AJAX  
 Il modo più semplice per abilitare il supporto AJAX in un servizio WCF consiste nell'usare il <xref:System.ServiceModel.Activation.WebServiceHostFactory> nel file con estensione svc associato al servizio, come nell'esempio seguente.  
...
```  
 ...
 Per un esempio funzionante, vedere la [servizio AJAX con JSON e XML](../../../../docs/framework/wcf/samples/ajax-service-with-json-and-xml-sample.md).  
...
```  
 ...
 Questa operazione è accessibile tramite HTTP POST a `http://serviceaddress/endpointaddress/GetCities` e restituire un messaggio XML.  
 ...
 Nei servizi AJAX viene spesso usato il formato dati JSON. Per creare un'operazione che restituisce JSON invece di XML, impostare la proprietà <xref:System.ServiceModel.Web.WebGetAttribute.ResponseFormat%2A> (o <xref:System.ServiceModel.Web.WebInvokeAttribute.ResponseFormat%2A>) su <xref:System.ServiceModel.Web.WebMessageFormat.Json>. Il [serializzazione JSON autonoma](../../../../docs/framework/wcf/feature-details/stand-alone-json-serialization.md) argomento mappa viene visualizzata come predefinita .NET dati e tipi di contratto tipi in formato JSON.  
