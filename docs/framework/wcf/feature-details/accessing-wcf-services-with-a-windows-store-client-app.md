...
Nelle applicazioni di Windows Store sono supportate le seguenti modalità di sicurezza:
...
Nelle applicazioni di Windows Store sono supportati i tipi di credenziali client seguenti:
...
8. Windows (sicurezza del trasporto)  
 ...
 Per consentire alle applicazioni Windows Store di accedere e inviare le credenziali di Windows predefinite, è necessario abilitare questa funzionalità nel file Package.appmanifest. Aprire questo file e selezionare la scheda Funzionalità e selezionare "Credenziali predefinite di Windows". In questo modo le applicazioni possono connettersi alle risorse Intranet che richiedono le credenziali del dominio.  
  
> [!IMPORTANT]
> Affinché le applicazioni Windows Store esecano chiamate tra computer, è necessario abilitare un'altra funzionalità denominata "Rete domestica/lavoro". Questa impostazione si trova anche nel file Package.appmanifest nella scheda Funzionalità. In questo modo viene fornito l'accesso in ingresso e in uscita dell'applicazione in uso alle reti delle posizioni affidabili dell'utente come casa e lavoro. Le porte critiche in ingresso sono sempre bloccate. Per accedere ai servizi su Internet è inoltre necessario abilitare la funzionalità Internet (client).  
...
- [WCF nel blog delle app di Windows Store](https://docs.microsoft.com/archive/blogs/piyushjo/wcf-in-windows-8-metro-styled-apps-absolutely-supported)
...
- [App e chiamate tra computer di Windows Store](https://docs.microsoft.com/archive/blogs/piyushjo/calling-a-wcf-service-from-a-metro-application-cross-machine-scenario)
