...
### <a name="configuring-the-definitionidentity"></a>Configurazione di DefinitionIdentity  
 Quando un servizio del flusso di lavoro viene creato utilizzando la finestra <xref:System.ServiceModel.Activities.WorkflowService.DefinitionIdentity%2A> di progettazione del flusso di lavoro, viene impostato utilizzando la finestra **proprietà** . Fare clic all'esterno dell'attività radice del servizio nella finestra di progettazione per selezionare il servizio del flusso di lavoro, quindi scegliere **finestra Proprietà** dal menu **Visualizza** . Selezionare **WorkflowIdentity** nell'elenco a discesa visualizzato accanto alla proprietà **DefinitionIdentity** , quindi espandere e specificare le proprietà desiderate <xref:System.Activities.WorkflowIdentity> . Nell'esempio seguente l'oggetto <xref:System.ServiceModel.Activities.WorkflowService.DefinitionIdentity%2A> viene configurato <xref:System.Activities.WorkflowIdentity.Name%2A> `MortgageWorkflow` con e un <xref:System.Activities.WorkflowIdentity.Version%2A> di `1.0.0.0`. La proprietà <xref:System.Activities.WorkflowIdentity.Package%2A> è facoltativa e in questo esempio è `null`.  
 ...
 Quando un servizio di flusso di lavoro è self-hosted, la proprietà <xref:System.ServiceModel.Activities.WorkflowService.DefinitionIdentity%2A> viene configurata quando il servizio di flusso di lavoro viene costruito. Nell'esempio seguente, l'oggetto <xref:System.ServiceModel.Activities.WorkflowService.DefinitionIdentity%2A> viene configurato con gli stessi valori dell'esempio precedente, <xref:System.Activities.WorkflowIdentity.Name%2A> `MortgageWorkflow` con e un <xref:System.Activities.WorkflowIdentity.Name%2A> di `1.0.0.0`.  
...
```  
 ...
 Un <xref:System.ServiceModel.Activities.WorkflowService.DefinitionIdentity%2A> oggetto non è obbligatorio, anche se solo una versione del servizio del flusso di lavoro può avere un **valore null**<xref:System.ServiceModel.Activities.WorkflowService.DefinitionIdentity%2A>.  
