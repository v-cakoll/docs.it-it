---
title: StartErrorPublish
ms.date: 03/30/2017
ms.assetid: d261929a-bd41-4611-8951-d1fb16bda0bc
ms.openlocfilehash: f4b8e11803dee757130e34726a384eb59af4e51c
ms.sourcegitcommit: d2e1dfa7ef2d4e9ffae3d431cf6a4ffd9c8d378f
ms.translationtype: MT
ms.contentlocale: it-IT
ms.lasthandoff: 09/07/2019
ms.locfileid: "70797539"
---
# <a name="starterrorpublish"></a>StartErrorPublish
Id: 158  
  
 Gravità: Errore  
  
 Categoria: SMSvcHost  
  
## <a name="description"></a>DESCRIZIONE  
 Questo evento indica che l'avvio del servizio NT non è riuscito. Impossibile pubblicare l'endpoint di attesa. Vengono indicati il nome e l'ID del processo.  
  
## <a name="see-also"></a>Vedere anche

- [Registrazione eventi](index.md)
- [Riferimenti generali sugli eventi](events-general-reference.md)
