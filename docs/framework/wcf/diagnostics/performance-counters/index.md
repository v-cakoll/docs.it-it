...
## <a name="viewing-performance-data"></a>Visualizzazione dei dati relativi alle prestazioni  
 Per visualizzare i dati acquisiti dai contatori delle prestazioni è possibile usare lo strumento Performance Monitor (Perfmon.exe) di Windows. Per avviare questo strumento, passare a **Start**, quindi fare clic su **esegui** e digitare `perfmon.exe` nella finestra di dialogo.  
...
- Il nome dell'istanza del contatore dell'endpoint può essere ottenuto tramite la proprietà "CounterInstanceName" dell'istanza di [endpoint](../wmi/endpoint.md) WMI.  
  
- Il nome dell'istanza del contatore delle operazioni può essere ottenuto tramite il metodo "GetOperationCounterInstanceName" dell'istanza dell' [endpoint](../wmi/endpoint.md) WMI.  
  
 Per ulteriori informazioni su WMI, vedere [utilizzo di Strumentazione gestione Windows per la diagnostica](../wmi/index.md).  
