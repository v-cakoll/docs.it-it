...
## <a name="exception-list"></a>Elenco di eccezioni
...
|CannotPerformS4UImpersonationOnPlatform|L'associazione specificata per il contratto specificato supporta la rappresentazione solo in Windows Server 2003 e nelle versioni più recenti di Windows. Utilizzare l'autenticazione SspiNegotiated e un'associazione con conversazione protetta in cui è stato attivato l'annullamento.|
...
|DnsIdentityCheckFailedForOutgoingMessage|Il controllo dell'identità non è riuscito per il messaggio in uscita. L'endpoint remoto deve avere l'identità del sistema dei nomi di dominio specificata. non corrisponde all'attestazione DNS fornita dall'endpoint remoto. Se si tratta di un endpoint remoto legittimo, per risolvere il problema è possibile specificare esplicitamente l'identità DNS come proprietà Identity dell'indirizzo EndpointAddress durante la creazione del proxy del canale.|
|DuplicateIdInMessageToBeVerified|L'ID specificato si è verificato due volte nel messaggio fornito per la verifica.|
...
|RequiredSecurityHeaderElementNotSigned|L'elemento dell'intestazione di sicurezza specificato con l'ID specificato deve essere firmato.|
...
|SecuritySessionNotPending|Nessuna sessione di sicurezza con l'ID specificato è in sospeso.|
