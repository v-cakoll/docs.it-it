...
description: Informazioni sulle applicazioni client, su come configurare, creare e usare un client WCF e su come proteggere le applicazioni client.
ms.date: 03/30/2017
dev_langs:
- csharp
- vb
...
ms.locfileid: "85245911"
---
# <a name="wcf-client-overview"></a>Panoramica client WCF
...
## <a name="using-wcf-client-objects"></a>Uso di oggetti client WCF  
 Un'applicazione client è un'applicazione gestita che utilizza un client WCF per comunicare con un'altra applicazione. Per la creazione di un'applicazione client per un servizio WCF sono necessari i passaggi seguenti:  
...
### <a name="contracts-bindings-and-addresses"></a>Contratti, associazioni e indirizzi.  
 ...
 Questo file di configurazione specifica un endpoint di destinazione nell'elemento `<client>`. Per ulteriori informazioni sull'utilizzo di più endpoint di destinazione, vedere <xref:System.ServiceModel.ClientBase%601.%23ctor%2A> o <xref:System.ServiceModel.ChannelFactory%601.%23ctor%2A> costruttori.  
...
- Creare un'istanza della classe di implementazione del contratto di callback e usarla per creare l' <xref:System.ServiceModel.InstanceContext?displayProperty=nameWithType> oggetto passato al costruttore client WCF.  
