...
L'analisi del sentiment può essere usata per prevedere il sentiment positivo o negativo del feedback dei clienti. Si tratta di un esempio di attività di machine learning per la classificazione binaria.
...
La classificazione dei problemi può essere usata per classificare i problemi dei feedback dei clienti, ad esempio in GitHub, usando il titolo del problema e la descrizione. Si tratta di un esempio di attività di Machine Learning di classificazione multiclasse.
...
La stima dei prezzi può essere usata per stimare i prezzi delle case in base alla posizione, alla dimensione e ad altre caratteristiche della casa. Si tratta di un esempio dell'attività Machine Learning di regressione.
...
Il generatore di modelli supporta i set di dati nei formati TSV, CSV, txt, oltre al formato del database SQL. Se si dispone di un file con estensione txt, le colonne devono essere separate da `,`, `;` o `/t` e il file deve avere una riga di intestazione.
...
|Scenario|Attività ML|Data|Label|Funzionalità|
|-|-|-|-|-|
...
|Classificazione immagini|Classificazione multiclasse|[Immagini dei fiori](http://download.tensorflow.org/example_images/flower_photos.tgz)|Tipo di fiore: Daisy, Dandelion, Roses, girasoli, tulipani|Dati dell'immagine|

## <a name="train"></a>Esegui il training
...
#### <a name="regression-for-example-price-prediction"></a>Regressione (ad esempio, stime dei prezzi)

La metrica predefinita per i problemi di regressione è RSquared, il valore di RSquared è compreso tra 0 e 1. 1 è il miglior valore possibile o, in altre parole, più il valore di RSquared è più vicino a 1 migliore sarà l'esecuzione del modello.

Altre metriche segnalate come la perdita assoluta, la perdita quadrata e la perdita di RMS sono metriche aggiuntive, che possono essere usate per comprendere il modo in cui il modello esegue e il confronto con altri modelli di regressione.
...
#### <a name="multi-class-classification-for-example-issue-classification-image-classification"></a>Classificazione multiclasse (ad esempio, classificazione dei problemi, classificazione delle immagini)
