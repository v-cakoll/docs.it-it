...
1. Ottenere i dati di training nel formato di una `IDataView`
2. Creare la pipeline di apprendimento come una sequenza di operatori elementari (algoritmi di stima).
3. Chiamare `Fit` sulla pipeline per ottenere il modello sottoposto a training.

In questo [file di esempio](https://github.com/dotnet/machinelearning/tree/master/test/data/generated_regression_dataset.csv) l'etichetta stimata (`target`) corrisponde all'ultima colonna (la dodicesima). Tutte le altre sono caratteristiche:
