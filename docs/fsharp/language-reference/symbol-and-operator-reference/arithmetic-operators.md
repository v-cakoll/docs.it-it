...
|`/`(divisione, divisa)|Divisione per zero causa un <xref:System.DivideByZeroException> per i tipi integrali. Per tipi a virgola mobile, divisione per zero consente i valori a virgola mobile speciali `+Infinity` o `-Infinity`. È inoltre disponibile una condizione di possibili underflow quando un numero a virgola mobile è troppo piccolo per essere rappresentato dal tipo.|
|`%`mod (modulo)|Restituisce il resto di un'operazione di divisione. Il segno del risultato è uguale a quello del primo operando.|
...
|`-`negativo (negazione)|Può essere applicato a qualsiasi espressione aritmetica. Modifica il segno del valore.|
