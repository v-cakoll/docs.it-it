...
Alcuni tipi sono considerati *tipi primitivi*, ad esempio il tipo `bool` booleano e i tipi a virgola mobile e integrali di diverse dimensioni, che includono i tipi per byte e caratteri. Questi tipi sono descritti in [tipi primitivi](primitive-types.md).
...
|array|*nome-tipo* [] o<br /><br />matrice *di nomi di tipo*|`int[]`<br /><br />`array<int>`<br /><br />`int array`|
|matrice bidimensionale|*nome-tipo* [,]|`int[,]`<br /><br />`float[,]`|
|matrice tridimensionale|*nome-tipo* [,,]|`float[,,]`|
|tuple|*tipo-name1* &#42; *digitare name2* ...|Ad esempio, `(1,'b',3)` è di tipo`int * char * int`|
|tipo generico|*parametro di tipo* *nome-tipo-generico*<br /><br />oppure<br /><br />*generic-type-name*&lt;*type-parameter-list*&gt;|`'a list`<br /><br />`list<'a>`<br /><br />`Dictionary<'key, 'value>`|
|tipo costruito (un tipo generico con un argomento di tipo specifico fornito)|*tipo-argomento* *nome-tipo-generico*<br /><br />oppure<br /><br />*generic-type-name*&lt;*type-argument-list*&gt;|`int option`<br /><br />`string list`<br /><br />`int ref`<br /><br />`option<int>`<br /><br />`list<string>`<br /><br />`ref<int>`<br /><br />`Dictionary<int, string>`|
|tipo di funzione con un solo parametro|*Parameter-tipo1*  - &gt; *-tipo restituito*|Funzione che accetta un oggetto `int` e restituisce un `string` oggetto di tipo`int -> string`|
|tipo di funzione con più parametri|*parametro-tipo1*  - parametro- - tipo2&gt; ...-Return-Type&gt; &gt;|Funzione che accetta un `int` oggetto e un `float` oggetto e restituisce un `string` tipo con`int -> float -> string`|
|funzione di ordine superiore come parametro|(*tipo di funzione*)|`List.map`con tipo`('a -> 'b) -> 'a list -> 'b list`|
...
|[Strutture](structures.md)|Descrive il `struct` tipo, un tipo di oggetto che corrisponde a un tipo di valore .NET. Il `struct` tipo rappresenta in genere una piccola aggregazione di dati.|
