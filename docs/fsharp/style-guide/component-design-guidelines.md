...
### <a name="add-xml-documentation-comments-to-your-code"></a><span data-ttu-id="b9761-123">Aggiungere commenti in formato documentazione XML nel codice</span><span class="sxs-lookup"><span data-stu-id="b9761-123">Add XML documentation comments to your code</span></span>
...
### <a name="always-follow-best-practices-for-using-strings-in-net"></a><span data-ttu-id="b9761-131">Seguire sempre le procedure consigliate per l'uso delle stringhe in .NET</span><span class="sxs-lookup"><span data-stu-id="b9761-131">Always follow best practices for using strings in .NET</span></span>
...
| <span data-ttu-id="b9761-179">Metodo</span><span class="sxs-lookup"><span data-stu-id="b9761-179">Method</span></span> |  <span data-ttu-id="b9761-180">PascalCase</span><span class="sxs-lookup"><span data-stu-id="b9761-180">PascalCase</span></span> |  <span data-ttu-id="b9761-181">Verbo</span><span class="sxs-lookup"><span data-stu-id="b9761-181">Verb</span></span> | <span data-ttu-id="b9761-182">ToString</span><span class="sxs-lookup"><span data-stu-id="b9761-182">ToString</span></span> | |
...
#### <a name="use-either-pascalcase-or-camelcase-for-public-functions-and-values-in-f-modules"></a><span data-ttu-id="b9761-219">Usare PascalCase o camelCase per le funzioni pubbliche e i valori in F# i moduli</span><span class="sxs-lookup"><span data-stu-id="b9761-219">Use either PascalCase or camelCase for public functions and values in F# modules</span></span>
...
#### <a name="use-classes-to-encapsulate-mutable-state"></a><span data-ttu-id="b9761-235">Usare le classi per incapsulare lo stato modificabile</span><span class="sxs-lookup"><span data-stu-id="b9761-235">Use classes to encapsulate mutable state</span></span>
...
#### <a name="use-a-module-to-group-functions-which-act-on-collections"></a><span data-ttu-id="b9761-243">Usare un modulo per le funzioni di gruppo che agiscono sulle raccolte</span><span class="sxs-lookup"><span data-stu-id="b9761-243">Use a module to group functions which act on collections</span></span>
...
<span data-ttu-id="b9761-270">Usando `[<CompiledName>]`, è possibile usare convenzioni di denominazione .NET per non F# consumer dell'assembly.</span><span class="sxs-lookup"><span data-stu-id="b9761-270">By using `[<CompiledName>]`, you can use .NET naming conventions for non F# consumers of the assembly.</span></span>
...
#### <a name="avoid-the-use-of-implementation-inheritance-for-extensibility"></a><span data-ttu-id="b9761-279">Evitare l'utilizzo dell'ereditarietà dell'implementazione per l'estensibilità</span><span class="sxs-lookup"><span data-stu-id="b9761-279">Avoid the use of implementation inheritance for extensibility</span></span>
...
<span data-ttu-id="b9761-313">I vincoli di membro aritmetico e F# vincoli del confronto sono standard per F# di programmazione.</span><span class="sxs-lookup"><span data-stu-id="b9761-313">Arithmetic member constraints and F# comparison constraints are a standard for F# programming.</span></span> <span data-ttu-id="b9761-314">Si consideri il codice di esempio seguente:</span><span class="sxs-lookup"><span data-stu-id="b9761-314">For example, consider the following code:</span></span>
...
### <a name="operator-definitions"></a><span data-ttu-id="b9761-322">Definizioni dell'operatore</span><span class="sxs-lookup"><span data-stu-id="b9761-322">Operator Definitions</span></span>
...
### <a name="type-abbreviations"></a><span data-ttu-id="b9761-333">Abbreviazioni dei tipi</span><span class="sxs-lookup"><span data-stu-id="b9761-333">Type Abbreviations</span></span>
...
## <a name="guidelines-for-libraries-for-use-from-other-net-languages"></a><span data-ttu-id="b9761-342">Linee guida per le raccolte per l'utilizzo in altri linguaggi .NET</span><span class="sxs-lookup"><span data-stu-id="b9761-342">Guidelines for libraries for Use from other .NET Languages</span></span>
...
<span data-ttu-id="b9761-405">Evitare di altri utilizzi del tipo di unità.</span><span class="sxs-lookup"><span data-stu-id="b9761-405">Avoid other uses of the unit type.</span></span> <span data-ttu-id="b9761-406">Si tratta di una buona:</span><span class="sxs-lookup"><span data-stu-id="b9761-406">These are good:</span></span>
...
#### <a name="avoid-using-tuples-as-return-values"></a><span data-ttu-id="b9761-413">Evitare di usare le tuple come valori restituiti</span><span class="sxs-lookup"><span data-stu-id="b9761-413">Avoid using tuples as return values</span></span>

<span data-ttu-id="b9761-414">Preferire invece la restituzione di un tipo denominato che contiene i dati aggregati o tramite i parametri out per restituire più valori.</span><span class="sxs-lookup"><span data-stu-id="b9761-414">Instead, prefer returning a named type holding the aggregate data, or using out parameters to return multiple values.</span></span> <span data-ttu-id="b9761-415">Anche se esistono le tuple e le tuple struct in .NET, incluso supporto del linguaggio c# per le tuple struct, vengono in genere non fornirà l'API ideale e previsto per gli sviluppatori .NET.</span><span class="sxs-lookup"><span data-stu-id="b9761-415">Although tuples and struct tuples exist in .NET (including C# language support for struct tuples), they will most often not provide the ideal and expected API for .NET developers.</span></span>

#### <a name="avoid-the-use-of-currying-of-parameters"></a><span data-ttu-id="b9761-416">Evitare l'uso di currying dei parametri</span><span class="sxs-lookup"><span data-stu-id="b9761-416">Avoid the use of currying of parameters</span></span>

<span data-ttu-id="b9761-417">Usare invece le convenzioni di chiamata .NET `Method(arg1,arg2,…,argN)`.</span><span class="sxs-lookup"><span data-stu-id="b9761-417">Instead, use .NET calling conventions `Method(arg1,arg2,…,argN)`.</span></span>

```fsharp
member this.TupledArguments(str, num) = String.replicate num str
```

<span data-ttu-id="b9761-418">Suggerimento: Se si sta progettando le librerie per l'utilizzo in qualsiasi linguaggio .NET, quindi non resta altro per effettivamente eseguite alcune sperimentale C# e per garantire che le librerie "aspetto destra" da questi linguaggi di programmazione Visual Basic.</span><span class="sxs-lookup"><span data-stu-id="b9761-418">Tip: If you’re designing libraries for use from any .NET language, then there’s no substitute for actually doing some experimental C# and Visual Basic programming to ensure that your libraries "feel right" from these languages.</span></span> <span data-ttu-id="b9761-419">È anche possibile usare strumenti, ad esempio .NET Reflector e il Visualizzatore oggetti di Visual Studio per garantire che le librerie e la relativa documentazione vengono visualizzati come previsto per gli sviluppatori.</span><span class="sxs-lookup"><span data-stu-id="b9761-419">You can also use tools such as .NET Reflector and the Visual Studio Object Browser to ensure that libraries and their documentation appear as expected to developers.</span></span>

## <a name="appendix"></a><span data-ttu-id="b9761-420">Appendice</span><span class="sxs-lookup"><span data-stu-id="b9761-420">Appendix</span></span>

### <a name="end-to-end-example-of-designing-f-code-for-use-by-other-net-languages"></a><span data-ttu-id="b9761-421">Esempio end-to-end di progettazione F# codice per l'uso da altri linguaggi .NET</span><span class="sxs-lookup"><span data-stu-id="b9761-421">End-to-end example of designing F# code for use by other .NET languages</span></span>

<span data-ttu-id="b9761-422">Si consideri la classe seguente:</span><span class="sxs-lookup"><span data-stu-id="b9761-422">Consider the following class:</span></span>
...
<span data-ttu-id="b9761-423">Il derivato F# tipo di questa classe è il seguente:</span><span class="sxs-lookup"><span data-stu-id="b9761-423">The inferred F# type of this class is as follows:</span></span>
...
<span data-ttu-id="b9761-424">Diamo un'occhiata a come questo F# tipo viene visualizzato a un programmatore che utilizza un altro linguaggio .NET.</span><span class="sxs-lookup"><span data-stu-id="b9761-424">Let’s take a look at how this F# type appears to a programmer using another .NET language.</span></span> <span data-ttu-id="b9761-425">Ad esempio, approssimativo c# "firma" è come segue:</span><span class="sxs-lookup"><span data-stu-id="b9761-425">For example, the approximate C# “signature” is as follows:</span></span>
...
<span data-ttu-id="b9761-426">Esistono alcuni aspetti importanti da notare su come F# costrutti di rappresenta qui.</span><span class="sxs-lookup"><span data-stu-id="b9761-426">There are some important points to notice about how F# represents constructs here.</span></span> <span data-ttu-id="b9761-427">Ad esempio:</span><span class="sxs-lookup"><span data-stu-id="b9761-427">For example:</span></span>

* <span data-ttu-id="b9761-428">I metadati, ad esempio i nomi degli argomenti sono stato mantenuto.</span><span class="sxs-lookup"><span data-stu-id="b9761-428">Metadata such as argument names has been preserved.</span></span>

* <span data-ttu-id="b9761-429">F#i metodi che accettano due argomenti diventi C# i metodi che accettano due argomenti.</span><span class="sxs-lookup"><span data-stu-id="b9761-429">F# methods that take two arguments become C# methods that take two arguments.</span></span>

* <span data-ttu-id="b9761-430">Funzioni e gli elenchi diventano i riferimenti ai tipi corrispondenti di F# library.</span><span class="sxs-lookup"><span data-stu-id="b9761-430">Functions and lists become references to corresponding types in the F# library.</span></span>

<span data-ttu-id="b9761-431">Il codice seguente viene illustrato come modificare il codice per tenere conto di queste operazioni.</span><span class="sxs-lookup"><span data-stu-id="b9761-431">The following code shows how to adjust this code to take these things into account.</span></span>
...
<span data-ttu-id="b9761-432">Il derivato F# è di tipo del codice come indicato di seguito:</span><span class="sxs-lookup"><span data-stu-id="b9761-432">The inferred F# type of the code is as follows:</span></span>
...
<span data-ttu-id="b9761-433">La firma c# è ora come indicato di seguito:</span><span class="sxs-lookup"><span data-stu-id="b9761-433">The C# signature is now as follows:</span></span>
...
<span data-ttu-id="b9761-434">Le correzioni apportate per preparare questo tipo per l'uso come parte di una libreria .NET "vanilla" sono i seguenti:</span><span class="sxs-lookup"><span data-stu-id="b9761-434">The fixes made to prepare this type for use as part of a vanilla .NET library are as follows:</span></span>

* <span data-ttu-id="b9761-435">Regolato diversi nomi: `Point1`, `n`, `l`, e `f` è diventato `RadialPoint`, `count`, `factor`, e `transform`, rispettivamente.</span><span class="sxs-lookup"><span data-stu-id="b9761-435">Adjusted several names: `Point1`, `n`, `l`, and `f` became `RadialPoint`, `count`, `factor`, and `transform`, respectively.</span></span>

* <span data-ttu-id="b9761-436">Utilizzato un tipo restituito `seq<RadialPoint>` invece di `RadialPoint list` modificando una costruzione elenco utilizzando `[ ... ]` a una sequenza di costruzione utilizzando `IEnumerable<RadialPoint>`.</span><span class="sxs-lookup"><span data-stu-id="b9761-436">Used a return type of `seq<RadialPoint>` instead of `RadialPoint list` by changing a list construction using `[ ... ]` to a sequence construction using `IEnumerable<RadialPoint>`.</span></span>

* <span data-ttu-id="b9761-437">Il tipo di delegato di .NET usata `System.Func` anziché un F# tipo di funzione.</span><span class="sxs-lookup"><span data-stu-id="b9761-437">Used the .NET delegate type `System.Func` instead of an F# function type.</span></span>

<span data-ttu-id="b9761-438">Questo rende molto accattivante utilizzare nel codice c#.</span><span class="sxs-lookup"><span data-stu-id="b9761-438">This makes it far nicer to consume in C# code.</span></span>
