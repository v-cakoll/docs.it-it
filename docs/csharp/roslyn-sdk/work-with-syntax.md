...
2. Per abilitare strumenti, ad esempio refactoring e IDE, per creare, modificare e ridisporre il codice sorgente in modo naturale senza dover usare modifiche di testo diretto. Mediante la creazione e modifica degli alberi, gli strumenti possono facilmente creare e modificare il codice sorgente.
...
Ogni nodo ha un metodo <xref:Microsoft.CodeAnalysis.SyntaxNode.ChildNodes?displayProperty=nameWithType>, che restituisce un elenco di nodi figlio in ordine sequenziale in base alla posizione nel testo di origine. L'elenco non contiene token. Ogni nodo dispone anche di metodi per esaminare i discendenti, <xref:Microsoft.CodeAnalysis.SyntaxNode.DescendantNodes%2A>ad <xref:Microsoft.CodeAnalysis.SyntaxNode.DescendantTokens%2A>esempio, <xref:Microsoft.CodeAnalysis.SyntaxNode.DescendantTrivia%2A> o, che rappresentano un elenco di tutti i nodi, i token o le banalità presenti nel sottoalbero radice da tale nodo.
