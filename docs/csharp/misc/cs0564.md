...
Il primo operando di un operatore shift di overload deve essere dello stesso tipo del tipo che lo contiene, mentre il tipo del secondo operando deve essere int  
 ...
 Si è provato a eseguire l'overload di un operatore shift (<\< o >>) con gli operandi di tipo in modo non corretto. Il primo operando deve essere il tipo e il secondo operando deve essere del tipo `int`.  
