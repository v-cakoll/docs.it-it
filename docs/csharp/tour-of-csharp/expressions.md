...
* Ad eccezione degli operatori di assegnazione e null-coalescing, tutti gli operatori binari sono *associativi a sinistra,* ovvero le operazioni vengono eseguite da sinistra a destra. L'espressione `x + y + z` viene ad esempio valutata come `(x + y) + z`.
* Gli operatori di assegnazione, gli `??` `??=` operatori e gli `?:` operatori null-coalescing e l'operatore condizionale sono *associativi a destra*, ovvero le operazioni vengono eseguite da destra a sinistra. L'espressione `x = y = z` viene ad esempio valutata come `x = (y = z)`.
