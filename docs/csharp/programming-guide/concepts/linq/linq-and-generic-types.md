...
## <a name="ienumerablet-variables-in-linq-queries"></a>Variabili IEnumerable<T\> nelle query LINQ  
 Le variabili di query <xref:System.Collections.Generic.IEnumerable%601> LINQ sono tipizzate come o come un tipo derivato, ad <xref:System.Linq.IQueryable%601>esempio . Nel caso di una variabile di query tipizzata come `IEnumerable<Customer>`, significa semplicemente che la query, quando eseguita, genererà una sequenza di zero o più oggetti `Customer`.  
  
 [!code-csharp[csLINQGettingStarted#34](~/samples/snippets/csharp/VS_Snippets_VBCSharp/CsLINQGettingStarted/CS/Class1.cs#34)]  
  
 Per ulteriori informazioni, vedere Relazioni tra tipi [in Operazioni di query LINQ](./type-relationships-in-linq-query-operations.md).  
