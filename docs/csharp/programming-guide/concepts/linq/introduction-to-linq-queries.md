...
## <a name="the-data-source"></a>Origine dati  
 ...
 Con [!INCLUDE[vbtecdlinq](~/includes/vbtecdlinq-md.md)], è innanzitutto necessario creare un mapping relazionale a oggetti in fase di progettazione manualmente o utilizzando [gli strumenti LINQ to SQL in Visual Studio](/visualstudio/data-tools/linq-to-sql-tools-in-visual-studio2). È possibile scrivere le query sugli oggetti e in fase di esecuzione [!INCLUDE[vbtecdlinq](~/includes/vbtecdlinq-md.md)] gestisce la comunicazione con il database. Nell'esempio seguente `Customers` rappresenta una tabella specifica nel database e il tipo del risultato della query, <xref:System.Linq.IQueryable%601>, deriva da <xref:System.Collections.Generic.IEnumerable%601>.  
...
## <a name="the-query"></a><a name="query"></a>La query  
