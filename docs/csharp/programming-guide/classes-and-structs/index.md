...
## <a name="example"></a>Esempio  
 Nell'esempio seguente la classe `CustomClass` inclusa nello spazio dei nomi `ProgrammingGuide` contiene tre membri: un costruttore di istanze, una proprietà denominata `Number` e un metodo denominato `Multiply`. Il `Main` metodo nella `Program` classe crea un'istanza (oggetto) di `CustomClass` e il metodo e la proprietà dell'oggetto sono accessibili tramite la notazione del punto.
