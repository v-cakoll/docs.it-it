---
title: Come definire l'uguaglianza di valori per un tipo - Guida per programmatori C
ms.date: 07/20/2015
helpviewer_keywords:
...
ms.locfileid: "79157091"
---
# <a name="how-to-define-value-equality-for-a-type-c-programming-guide"></a>Come definire l'uguaglianza di valori per un tipo (Guida per programmatori C
...
In entrambi i casi, e in entrambe le classi e struct, l'implementazione deve `x` `y` seguire `z` le cinque garanzie di equivalenza (per le seguenti regole, presupporre che , e non siano null):  
...
5. Qualsiasi valore non null non è uguale a null. Tuttavia, CLR controlla null su tutte le `NullReferenceException` chiamate `this` al metodo e genera un if il riferimento sarebbe null. Pertanto, `x.Equals(y)` genera un'eccezione quando `x` è null. Che rompe le regole 1 o `Equals`2, a seconda dell'argomento a .
...
3. Facoltativo, ma [==](../../language-reference/operators/equality-operators.md#equality-operator-) consigliato: eseguire l'overload degli operatori e [!](../../language-reference/operators/equality-operators.md#inequality-operator-)  
...
5. Facoltativo: per supportare le definizioni per "maggiore <xref:System.IComparable%601> di" o "minore [<=](../../language-reference/operators/comparison-operators.md#less-than-or-equal-operator-) di", implementare l'interfaccia per il tipo e anche eseguire l'overload degli operatori e [>=](../../language-reference/operators/comparison-operators.md#greater-than-or-equal-operator-) .  
...
## <a name="example"></a>Esempio
 ...
 Gli [==](../../language-reference/operators/equality-operators.md#equality-operator-) operatori e [!](../../language-reference/operators/equality-operators.md#inequality-operator-)  
