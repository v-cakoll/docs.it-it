...
Nell'esempio seguente [int](../builtin-types/integral-numeric-types.md), [short](../builtin-types/integral-numeric-types.md) e `float` sono inclusi in un'espressione matematica che dà come risultato un valore `float`. Tenere presente che `float` è un alias per il tipo <xref:System.Single?displayProperty=nameWithType>. Si noti che nell'istruzione non sono inclusi valori [double](double.md).
