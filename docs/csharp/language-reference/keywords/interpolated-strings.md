...
```  
contiene due espressioni interpolate, '{name}' e '{ora: hh}'. La stringa di formato composito equivalente è:
...
> [!IMPORTANT]
> È possibile disporre di qualsiasi spazio vuoto tra il `$` e `"` che inizia la stringa. In questo modo, un errore in fase di compilazione.
...
Verbatim interpolati utilizzare stringhe di `$` carattere seguito dal `@` carattere. Per ulteriori informazioni sulle stringhe verbatim, vedere il [stringa](string.md) argomento. Il codice seguente è una versione modificata del frammento di codice precedente, utilizzando una stringa interpolata verbatim:

[!code-csharp[interpolated-strings4](../../../../samples/snippets/csharp/language-reference/keywords/interpolated-strings5.cs#1)]  

Le modifiche alla formattazione sono necessarie perché non sono conformi alle stringhe verbatim `\` sequenze di escape.
