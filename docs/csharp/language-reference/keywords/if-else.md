...
È anche possibile `if` annidare un'istruzione all'interno di un blocco else, come illustrato nel codice parziale seguente. Nell'esempio le istruzioni `if` vengono annidate all'interno di due blocchi else e di un blocco then. I commenti specificano che le condizioni sono true o false in ogni blocco.
