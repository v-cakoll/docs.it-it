...
```
 ...
 In C# 1,1, un bug del compilatore ha reso possibile la definizione di una classe con conversioni implicite definite dall'utente sia in `int` che in `bool` e per l'utilizzo di un operatore `AND` bit per bit (`&`) sugli oggetti di quel tipo. In C# 2.0 questo bug è stato corretto per rendere il compilatore conforme alla specifica C#, pertanto il codice seguente causerà ora l'errore CS0034:
