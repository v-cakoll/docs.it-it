...
```  
 ...
 Il vincolo `new()` applica l'indipendenza dai tipi garantendo che qualsiasi tipo concreto specificato per `T` abbia un costruttore predefinito senza parametri. L'errore CS0304 si verifica se si tenta di usare l'operatore `new` nel corpo della classe per creare un'istanza del parametro di tipo `T` quando `T` non specifica il vincolo `new()`. Sul lato client, se il codice tenta di creare un'istanza della classe generica con un tipo che non ha un costruttore senza parametri, tale codice genera l'[errore del compilatore CS0310](../../../csharp/language-reference/compiler-messages/cs0310.md).  
