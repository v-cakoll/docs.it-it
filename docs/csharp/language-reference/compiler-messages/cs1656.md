...
Impossibile assegnare a 'variabile' perché è un 'tipo di variabile di sola lettura'  
 ...
 Questo errore si verifica quando un'assegnazione a una variabile viene eseguita in un contesto di sola lettura. I contesti di sola lettura includono le variabili di iterazione [foreach](../../../csharp/language-reference/keywords/foreach-in.md) , le variabili [using](../../../csharp/language-reference/keywords/using-statement.md) e le variabili [fixed](../../../csharp/language-reference/keywords/fixed-statement.md). Per risolvere l'errore, evitare le assegnazioni a una variabile di istruzione in blocchi `using`, istruzioni `foreach` e istruzioni `fixed`.  
  
## <a name="example"></a>Esempio  
 L'esempio seguente genera l'errore CS1656 poiché viene tentata la sostituzione di elementi completi di una raccolta all'interno di un ciclo `foreach`. Un modo per risolvere l'errore consiste nel sostituire il ciclo `foreach` con un ciclo [for](../../../csharp/language-reference/keywords/for.md). Un altro modo, non illustrato qui, consiste nel modificare i membri dell'elemento esistente. Ciò è possibile con le classi, ma non con gli struct.  
