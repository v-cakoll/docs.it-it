...
4.  Quando si verifica l'evento, [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)] chiama automaticamente il `Sub` stored procedure. Il codice può usare un `RaiseEvent` istruzione per generare l'evento.  
...
3.  Quando si verifica l'evento, [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)] chiama automaticamente il `Sub` stored procedure. Il codice può usare un `RaiseEvent` istruzione per generare l'evento.  
