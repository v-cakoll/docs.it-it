...
- Fornire una [nuova clausola operator](../../../../visual-basic/language-reference/operators/new-operator.md) dopo aver specificato le lunghezze della dimensione nella dichiarazione della variabile di matrice. Nelle righe di codice seguenti vengono mostrate dichiarazioni non valide di questo tipo.  
...
- Omettere la parola chiave `New` quando si specificano i valori dell'elemento. La seguente riga di codice mostra una dichiarazione non valida di questo tipo.  
  
     `Dim INVALIDoneDimShortArray() As Short = Short() {0, 1, 2, 3}`  
  
- Specifica di una clausola `New` senza parentesi graffe (`{}`). Nelle righe di codice seguenti vengono mostrate dichiarazioni non valide di questo tipo.  
