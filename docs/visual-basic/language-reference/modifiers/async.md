...
Il `Async` modificatore indica che il metodo o [espressione lambda](../../../visual-basic/programming-guide/language-features/procedures/lambda-expressions.md) che ha modificato è asincrono. Tali metodi sono definiti come *metodi async*.  
 ...
 Un metodo asincrono è un modo pratico per eseguire le operazioni potenzialmente di lunga durata senza bloccare il thread del chiamante. Il chiamante di un metodo asincrono può riprendere il lavoro senza attendere il completamento del metodo async.  
...
## <a name="example"></a>Esempio  
 Negli esempi seguenti viene illustrato un gestore eventi async, un'espressione lambda async e un metodo async. Per un esempio completo che utilizza questi elementi, vedere [procedura dettagliata: accesso al Web da utilizzare Async e Await](../../../visual-basic/programming-guide/concepts/async/walkthrough-accessing-the-web-by-using-async-and-await.md). È possibile scaricare il codice della procedura dettagliata dalla pagina [Developer Code Samples](http://go.microsoft.com/fwlink/?LinkId=255191) (Esempi di codice per sviluppatori).  
