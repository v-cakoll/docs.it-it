...
6.  Assicurarsi che non si determinava "cascata di eventi" generando un evento che chiama una routine evento già nello stack. Una catena di eventi è simile a una chiamata di routine ricorsiva non terminata, ma è meno ovvio, poiché la chiamata viene effettuata da [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)] invece di una chiamata esplicita nel codice. Utilizzare il `Calls` finestra di dialogo per visualizzare le routine attive nello stack.  
