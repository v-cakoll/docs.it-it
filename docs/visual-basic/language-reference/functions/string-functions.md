...
|<xref:Microsoft.VisualBasic.Strings.Filter%2A>|Restituisce una matrice in base zero contenente un subset di una matrice di `String` in base ai criteri di filtro specificati.|
...
In questo esempio viene usato `Len` per restituire il numero di caratteri in una stringa.
...
> [!NOTE]
> Per le impostazioni locali che utilizzano un orario a 24 ore, gli indicatori AM/PM (`t` e `tt`) non visualizzano nulla.
