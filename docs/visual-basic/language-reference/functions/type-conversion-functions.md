...
A partire da Visual Basic 15,8, le prestazioni della conversione a virgola mobile sono ottimizzate quando si passa il valore <xref:System.Single> o <xref:System.Double> restituito dai metodi seguenti a una delle funzioni di conversione integer (`CByte`, `CShort`, `CInt`, `CLng`, `CSByte`, `CUShort`, `CUInt`, `CULng`):
