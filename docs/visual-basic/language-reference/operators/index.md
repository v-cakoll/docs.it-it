---
title: Operatori (Visual Basic)
ms.date: 10/17/2018
helpviewer_keywords:
- operators [Visual Basic]
ms.assetid: 3d3421df-fcc5-4888-9249-d78f16774ce6
ms.openlocfilehash: e295d492c8c2e5cbd69f295e83f44b63c74ca553
ms.sourcegitcommit: 9b552addadfb57fab0b9e7852ed4f1f1b8a42f8e
ms.translationtype: MT
ms.contentlocale: it-IT
ms.lasthandoff: 04/23/2019
ms.locfileid: "62013569"
---
# <a name="operators-visual-basic"></a>Operatori (Visual Basic)
## <a name="in-this-section"></a>Contenuto della sezione

 [Precedenza degli operatori in Visual Basic](operator-precedence.md)  
  
 [Elencata degli operatori per funzionalità](operators-listed-by-functionality.md)  
  
 [Tipi di dati dei risultati degli operatori](data-types-of-operator-results.md)  
  
 [Operatore DirectCast](directcast-operator.md)  
  
 [Operatore TryCast](trycast-operator.md)  
  
 [operatore new](new-operator.md)  

 [Operatori condizionali null](null-conditional-operators.md)

 [Operatori aritmetici](arithmetic-operators.md)  
  
 [Operatori di assegnazione](assignment-operators.md)  
  
 [Operatori di scorrimento bit](bit-shift-operators.md)  
  
 [Operatori di confronto](comparison-operators.md)  
  
 [Operatori di concatenazione](concatenation-operators.md)  
  
 [Operatori logici/bit per bit](logical-bitwise-operators.md)  
  
 [Operatori vari](miscellaneous-operators.md)  
  
## <a name="related-sections"></a>Sezioni correlate  
 [Informazioni di riferimento per il linguaggio Visual Basic](../../../visual-basic/language-reference/index.md)  
  
 [Visual Basic](../../../visual-basic/index.md)
