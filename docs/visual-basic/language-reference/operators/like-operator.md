...
## <a name="parts"></a>Parti  
 `result`  
 Obbligatorio. Any `Boolean` variable. The result is a `Boolean` value indicating whether or not the `string` satisfies the `pattern`.  
  
 `string`  
 Obbligatorio. Qualsiasi espressione `String` .  
  
 `pattern`  
 Obbligatorio. Any `String` expression conforming to the pattern-matching conventions described in "Remarks."  
