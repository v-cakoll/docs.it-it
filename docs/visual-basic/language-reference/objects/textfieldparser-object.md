---
title: Oggetto TextFieldParser (Visual Basic)
ms.date: 07/20/2015
helpviewer_keywords:
- TextFieldParser object
- TextFieldParser class, aboutTextFieldParser class
- TextFieldParser class
ms.assetid: d44bd2b0-7dfc-410b-a48b-534c1e97460b
ms.openlocfilehash: 15eb5a30cf73f118dd4ad3a8d2194c8190d9c706
ms.sourcegitcommit: 3c1c3ba79895335ff3737934e39372555ca7d6d0
ms.translationtype: MT
ms.contentlocale: it-IT
ms.lasthandoff: 09/06/2018
ms.locfileid: "43803284"
---
# <a name="textfieldparser-object"></a>TextFieldParser Object
Fornisce i metodi e le proprietà per l'analisi dei file di testo strutturati.  
  
## <a name="syntax"></a>Sintassi  
  
```  
Public Class TextFieldParser  
```  
  
## <a name="remarks"></a>Note  
 Per informazioni sui metodi e proprietà del `TextFieldParser` oggetti, vedere <xref:Microsoft.VisualBasic.FileIO.TextFieldParser>.  
  
 Per altre informazioni, vedere [legge dai file](../../../visual-basic/developing-apps/programming/drives-directories-files/reading-from-files.md).  
  
## <a name="requirements"></a>Requisiti  
 **Namespace:** <xref:Microsoft.VisualBasic.FileIO>  
  
 **Classe:** <xref:Microsoft.VisualBasic.FileIO.TextFieldParser>  
  
 **Assembly:** [!INCLUDE[vbprvbruntime](~/includes/vbprvbruntime-md.md)]
