...
|Sì|Yes|Sì, ma solo se la variabile `counter` esistente è definita all'esterno della routine. Tale variabile rimane separata. Se l'ambito della variabile esistente `counter` è locale alla procedura, si verificherà un errore in fase di compilazione.|
|Yes|No|Sì.|
