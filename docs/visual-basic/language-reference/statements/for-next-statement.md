...
|`start`|Obbligatoria. Espressione numerica. Il valore iniziale di `counter`.|
|`end`|Obbligatoria. Espressione numerica. Valore finale della `counter`.|
...
|`Exit For`|Facoltativa. Trasferisce il controllo all'esterno del ciclo `For`.|
|`Next`|Obbligatoria. Termina la definizione del ciclo `For`.|
...
Si utilizza una struttura di `For`...`Next` quando si desidera ripetere un set di istruzioni impostando un numero di volte.
...
È possibile inserire un numero qualsiasi di istruzioni `Exit For` in un `For`...`Next` loop. Se utilizzato all'interno di `For`annidati...`Next` Loops, `Exit For` esce dal ciclo più interno e trasferisce il controllo al successivo livello di nidificazione.
...
- Un'istruzione `Try`...`Catch`...`Finally` intercetta un'eccezione. È possibile utilizzare `Exit For` alla fine del blocco di `Finally`.
...
|`datatype` è presente?|Il `counter` è già definito?|Risultato (se `counter` definisce una nuova variabile locale con ambito per l'intero ciclo di `For...Next`)|
|----------------------------|-----------------------------------|-------------------------------------------------------------------------------------------------------------|
|No|Sì|No, perché `counter` è già definito. Se l'ambito di `counter` non è locale alla procedura, viene generato un avviso in fase di compilazione.|
...
|Sì|Sì|Sì, ma solo se la variabile di `counter` esistente è definita all'esterno della routine. Tale variabile rimane separata. Se l'ambito della variabile di `counter` esistente è locale alla procedura, si verificherà un errore in fase di compilazione.|
|Sì|No|Sì.|
