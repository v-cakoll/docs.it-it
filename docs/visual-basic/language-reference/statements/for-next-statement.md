...
|`datatype`|Parametro facoltativo. Data type of `counter`. For more information, see [Counter Argument](#BKMK_Counter) later in this topic.|
|`start`|Obbligatorio. Numeric expression. Il valore iniziale di `counter`.|
|`end`|Obbligatorio. Numeric expression. The final value of `counter`.|
|`step`|Parametro facoltativo. Numeric expression. The amount by which `counter` is incremented each time through the loop.|
|`statements`|Parametro facoltativo. One or more statements between `For` and `Next` that run the specified number of times.|
|`Continue For`|Parametro facoltativo. Transfers control to the next loop iteration.|
|`Exit For`|Parametro facoltativo. Transfers control out of the `For` loop.|
|`Next`|Obbligatorio. Terminates the definition of the `For` loop.|
