...
Un'espressione restituisce un tipo per il quale Common Language Runtime (CLR) non può eseguire il boxing.  
 ...
 Se nell'istruzione che contiene l'espressione non è stata usata la funzione `CType` o `CObj` , [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)] ha provato a eseguire una conversione implicita che genera questo errore.  
