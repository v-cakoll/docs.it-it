...
> [!NOTE]
> L'opzione `-sdkpath` non è disponibile nell'ambiente di sviluppo di Visual Studio. è disponibile solo quando si esegue la compilazione dalla riga di comando. L'opzione `-sdkpath` viene impostata quando viene caricato un progetto di Visual Basic dispositivo.  
  
 È possibile specificare che il compilatore deve eseguire la compilazione senza un riferimento alla libreria di runtime Visual Basic usando l'opzione del compilatore `-vbruntime`. Per ulteriori informazioni, vedere [-vbruntime](../../../visual-basic/reference/command-line-compiler/vbruntime.md).  
