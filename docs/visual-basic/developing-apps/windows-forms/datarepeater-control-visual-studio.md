---
title: Controllo DataRepeater (Visual Studio)
ms.date: 07/20/2015
helpviewer_keywords:
- DataRepeater, topic list
ms.assetid: 68e0cf5e-e2cb-480a-80cc-d13eb6c9b3dd
ms.openlocfilehash: 0d317310a7ae9369fab32acfe30a61d2c20c96c2
ms.sourcegitcommit: efff8f331fd9467f093f8ab8d23a203d6ecb5b60
ms.translationtype: MT
ms.contentlocale: it-IT
ms.lasthandoff: 09/01/2018
ms.locfileid: "43422517"
---
# <a name="datarepeater-control-visual-studio"></a>Controllo DataRepeater (Visual Studio)
Il controllo Visual Basic PowerPacks <xref:Microsoft.VisualBasic.PowerPacks.DataRepeater> è un contenitore scorrevole per controlli che visualizzano dati ripetuti, ad esempio le righe in una tabella di database. Può essere usato come alternativa al controllo <xref:System.Windows.Forms.DataGridView> quando è necessario maggiore controllo sul layout dei dati.  
  
 I controlli PowerPacks non sono inclusi in Visual Studio, ma è possibile scaricarli dal [area download](https://www.microsoft.com/en-us/download/details.aspx?id=25169).  
  
## <a name="in-this-section"></a>In questa sezione  
 [Introduzione al controllo DataRepeater](../../../visual-basic/developing-apps/windows-forms/introduction-to-the-datarepeater-control-visual-studio.md)  
 Viene presentato e descritto il controllo `DataRepeater` .  
  
 [Procedura: Visualizzare i dati associati in un controllo DataRepeater](../../../visual-basic/developing-apps/windows-forms/how-to-display-bound-data-in-a-datarepeater-control-visual-studio.md)  
 Viene illustrato come usare il controllo `DataRepeater` per visualizzare i dati da un'origine dati.  
  
 [Procedura: Visualizzare i controlli non associati in un controllo DataRepeater](../../../visual-basic/developing-apps/windows-forms/how-to-display-unbound-controls-in-a-datarepeater-control-visual-studio.md)  
 Viene illustrato come usare il controllo `DataRepeater` per visualizzare dati aggiuntivi.  
  
 [Procedura: Modificare il layout di un controllo DataRepeater](../../../visual-basic/developing-apps/windows-forms/how-to-change-the-layout-of-a-datarepeater-control-visual-studio.md)  
 Viene illustrato come modificare l'orientamento di un controllo `DataRepeater` .  
  
 [Procedura: Modificare l'aspetto di un controllo DataRepeater](../../../visual-basic/developing-apps/windows-forms/how-to-change-the-appearance-of-a-datarepeater-control-visual-studio.md)  
 Viene illustrato come personalizzare un controllo `DataRepeater` in fase di progettazione e di esecuzione.  
  
 [Procedura: Visualizzare le intestazioni degli elementi in un controllo DataRepeater](../../../visual-basic/developing-apps/windows-forms/how-to-display-item-headers-in-a-datarepeater-control-visual-studio.md)  
 Viene illustrato come controllare gli indicatori di selezione in un controllo `DataRepeater` .  
  
 [Procedura: Disabilitare l'aggiunta e l'eliminazione di elementi DataRepeater](../../../visual-basic/developing-apps/windows-forms/how-to-disable-adding-and-deleting-datarepeater-items-visual-studio.md)  
 Viene illustrato come impedire agli utenti di aggiungere o eliminare elementi in un controllo `DataRepeater` .  
  
 [Procedura: Cercare dati in un controllo DataRepeater](../../../visual-basic/developing-apps/windows-forms/how-to-search-data-in-a-datarepeater-control-visual-studio.md)  
 Viene illustrato come implementare funzionalità di ricerca in un controllo `DataRepeater` .  
  
 [Procedura: creare un Form Master-Details mediante due controlli DataRepeater (Visual Studio)](../../../visual-basic/developing-apps/windows-forms/how-to-create-a-master-detail-form-by-using-two-datarepeater-controls.md)  
 Viene illustrato come visualizzare i record correlati usando due controlli `DataRepeater` .  
  
 [Procedura dettagliata: Visualizzazione di dati in un controllo DataRepeater](../../../visual-basic/developing-apps/windows-forms/walkthrough-displaying-data-in-a-datarepeater-control-visual-studio.md)  
 Viene fornita una dimostrazione completa dell'uso di un controllo `DataRepeater` .  
  
 [Risoluzione dei problemi relativi al controllo DataRepeater](../../../visual-basic/developing-apps/windows-forms/troubleshooting-the-datarepeater-control-visual-studio.md)  
 Vengono descritti i potenziali problemi e le relative soluzioni.  
  
## <a name="reference"></a>Riferimenti  
 <xref:Microsoft.VisualBasic.PowerPacks>  
  
 <xref:Microsoft.VisualBasic.PowerPacks.DataRepeater>  
  
## <a name="related-sections"></a>Sezioni correlate  
 [Modalità virtuale nel controllo DataRepeater](../../../visual-basic/developing-apps/windows-forms/virtual-mode-in-the-datarepeater-control-visual-studio.md)
