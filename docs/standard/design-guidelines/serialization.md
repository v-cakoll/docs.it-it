 ...
 **✓ SI** considerare durante la progettazione di nuovi tipi di serializzazione.  
 ...
 **Provare a ✓** supporto di serializzazione del contratto dati se le istanze del tipo potrebbe essere necessario essere persistenti o utilizzato nei servizi Web.  
  
 **Provare a ✓** che supporta la serializzazione XML anziché o oltre alla serializzazione del contratto dati se è necessario un maggiore controllo sul formato XML che viene generato quando il tipo è serializzato.  
 ...
 **Provare a ✓** che supporta la serializzazione di Runtime se le istanze del tipo necessario attraversare i limiti di servizi remoti .NET.  
 ...
 **Provare a ✓** il contrassegno di membri di dati pubblico del tipo se il tipo può essere utilizzato in attendibilità parziale.  
 ...
 **Provare a ✓** utilizzando i callback di serializzazione per l'inizializzazione di istanze deserializzate.  
 ...
 **✓ Provare a** utilizzando il <xref:System.Runtime.Serialization.KnownTypeAttribute> per indicare i tipi concreti che devono essere utilizzati durante la deserializzazione di un grafico di oggetti complessi.  
 ...
 **X evitare** la progettazione di tipi in modo specifico per la serializzazione XML, a meno che non esista un motivo molto complessa per controllare la forma del codice XML generato. Questa tecnologia è stata sostituita dalla serializzazione dei contratti dati descritta nella sezione precedente.  
 ...
 **Provare a ✓** implementazione del Pattern serializzabile di Runtime se si desidera il controllo completo sul processo di serializzazione. ad esempio se si desidera trasformare i dati quando vengono serializzati o deserializzati.  
 ...
 **✓ SI** rendere il costruttore di serializzazione protetto e fornire due parametri tipizzati e denominati esattamente come illustrato nell'esempio di.  
 ...
 **✓ SI** si applicano a una richiesta di collegamento <xref:System.Runtime.Serialization.ISerializable.GetObjectData%2A?displayProperty=nameWithType> implementazione. In questo modo si garantisce che solo è considerato completamente attendibile core e il serializzatore Runtime hanno accesso al membro.  
