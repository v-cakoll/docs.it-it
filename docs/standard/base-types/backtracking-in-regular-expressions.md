...
### <a name="atomic-groups"></a>Gruppi atomici
 L' `(?>` elemento del linguaggio *sottoespressione* `)` Elimina il backtracking nella sottoespressione. Una volta completata la corrispondenza, non verrà restituita alcuna parte della relativa corrispondenza al backtracking successivo. Ad esempio, nel modello `(?>\w*\d*)1` , se non è `1` possibile trovare una corrispondenza per, `\d*` non verrà restituita alcuna corrispondenza anche se ciò significa che la corrispondenza potrebbe essere `1` corretta. I gruppi atomici possono aiutare a evitare i problemi di prestazioni associati alle corrispondenze non riuscite.
...
### <a name="lookbehind-assertions"></a>asserzioni lookbehind  
 ...
 `(?<=`*sottoespressione* `)` è un'asserzione lookbehind positiva; ovvero il carattere o i caratteri prima della posizione corrente devono corrispondere alla *sottoespressione*. `(?<!`*sottoespressione* `)` è un'asserzione lookbehind negativa. ovvero il carattere o i caratteri prima della posizione corrente non devono corrispondere alla *sottoespressione*. Le asserzioni lookbehind positive e negative sono entrambe particolarmente utili quando *sottoespressione* è un subset della sottoespressione precedente.  
