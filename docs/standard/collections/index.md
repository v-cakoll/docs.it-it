...
Un `List<T>` oggetto può essere enumerato in modo efficiente `for` utilizzando un ciclo `foreach` o un ciclo. Un `ImmutableList<T>`, tuttavia, esegue un processo scadente all'interno `for` di un ciclo, a causa del tempo `n`di O (log) per il relativo indicizzatore. L'enumerazione di `ImmutableList<T>` un oggetto `foreach` utilizzando un ciclo è `ImmutableList<T>` efficiente perché utilizza un albero binario per archiviare i dati anziché una semplice matrice `List<T>` come utilizza. Una matrice può essere indicizzata molto rapidamente in, mentre un albero binario deve essere riavviato fino a quando non viene trovato il nodo con l'indice desiderato.
