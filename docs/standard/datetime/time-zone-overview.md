...
- <xref:System.TimeZoneInfo.ConvertTimeBySystemTimeZoneId%2A>, che usa <xref:System.TimeZoneInfo> gli identificatori del fuso orario (anziché gli oggetti) come parametri per convertire l'ora in un fuso orario designato all'ora in un altro fuso orario designato.
