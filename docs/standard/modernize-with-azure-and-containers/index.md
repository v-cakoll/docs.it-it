...
description: Informazioni su come spostare le applicazioni esistenti per il cloud di Azure e i contenitori con questo e-book.
...
Questa guida è disponibile gratuitamente sotto forma di un libro elettronico (e-book) disponibile attraverso più canali di Microsoft, ad esempio http://dot.net/architecture.
...
Successivamente, come lo scenario ideale per molte applicazioni .NET Framework esistente eseguire la migrazione, è possibile eseguire la migrazione a un'applicazione Cloud pronto DevOps, per ottenere notevoli vantaggi da un impegno minimo. Questo approccio apre anche la strada all'ottimizzazione per il cloud, come possibile passaggio futuro. La figura 1-6 mostra un esempio.
...
L'obiettivo di questa guida è specifico. Viene visualizzato il percorso che è possibile eseguire per ottenere un'accuratezza e spostamento di applicazioni .NET esistenti, senza una riprogettazione e senza apportare modifiche al codice. Infine, viene illustrato come rendere l'applicazione Cloud pronto DevOps.

Questa Guida non viene illustrato come utilizzare le applicazioni cloud nativo, ad esempio come evoluzione di un'architettura di microservizi. Per riprogettare le applicazioni o per creare applicazioni completamente nuove basate su microservizi, vedere l'e-book [.NET Microservices: Architecture for containerized .NET applications](https://aka.ms/microservicesebook) (Microservizi .NET: architettura per applicazioni .NET in contenitori).
...
Questa guida è stata scritta per gli sviluppatori e architetti di soluzioni che desiderano modernizzare le applicazioni ASP.NET esistenti basati su .NET Framework, per una migliore agilità di spedizione e il rilascio di applicazioni.
