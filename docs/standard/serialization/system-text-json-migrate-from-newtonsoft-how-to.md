...
La maggior parte di questo articolo descrive come usare l'API <xref:System.Text.Json.JsonSerializer>, ma include anche informazioni aggiuntive su come usare il <xref:System.Text.Json.JsonDocument> (che rappresenta il Document Object Model o DOM), <xref:System.Text.Json.Utf8JsonReader>e i tipi di <xref:System.Text.Json.Utf8JsonWriter>. L'articolo è organizzato in sezioni nell'ordine seguente:

* [Differenze nel comportamento **predefinito** di jsonSerializer rispetto a Newtonsoft. JSON](#differences-in-default-jsonserializer-behavior-compared-to-newtonsoftjson)
* [Scenari che usano JsonSerializer che richiedono soluzioni alternative](#scenarios-using-jsonserializer-that-require-workarounds)
* [Scenari attualmente non supportati da JsonSerializer](#scenarios-that-jsonserializer-currently-doesnt-support)
* [JsonDocument e Jsonelement rispetto a JToken (ad esempio, JObject, JArray)](#jsondocument-and-jsonelement-compared-to-jtoken-like-jobject-jarray)
* [Utf8JsonReader rispetto a JsonTextReader](#utf8jsonreader-compared-to-jsontextreader)
* [Utf8JsonWriter rispetto a JsonTextWriter](#utf8jsonwriter-compared-to-jsontextwriter)
...
Quando `Newtonsoft.Json` deserializza in `object` proprietà in POCO o in dizionari di tipo `Dictionary<string, object>`,:
...
<xref:System.Text.Json> archivia una `JsonElement` boxed per i valori primitivi e complessi all'interno della proprietà `System.Object` o del valore del dizionario. Tuttavia, tratta `null` lo stesso `Newtonsoft.Json` e restituisce un riferimento null quando il payload presenta il `null` valore letterale JSON.
...
`Newtonsoft.Json` dispone di un'impostazione globale che fa sì che le proprietà con valori null vengano escluse dalla serializzazione: [NullValueHandling. ignore](https://www.newtonsoft.com/json/help/html/T_Newtonsoft_Json_NullValueHandling.htm). L'opzione corrispondente in <xref:System.Text.Json> è <xref:System.Text.Json.JsonSerializerOptions.IgnoreNullValues%2A>.
...
Gli scenari seguenti non sono supportati dalla funzionalità incorporata, ma il codice di esempio viene fornito per le soluzioni alternative. Per la maggior parte delle soluzioni alternative è necessario implementare [convertitori personalizzati](system-text-json-converters-how-to.md).
...
Durante la deserializzazione, <xref:System.Text.Json> non genera un'eccezione se non viene ricevuto alcun valore nel codice JSON per una delle proprietà del tipo di destinazione. Ad esempio, se si dispone di una classe `WeatherForecast`:
...
Sono possibili soluzioni alternative per gli scenari seguenti, ma alcune di esse sarebbero relativamente difficili da implementare. Questo articolo non fornisce esempi di codice per le soluzioni alternative per questi scenari.

Non si tratta di un elenco completo delle funzionalità `Newtonsoft.Json` che non hanno equivalenti in `System.Text.Json`. L'elenco include molti degli scenari richiesti nei [problemi di GitHub](https://github.com/dotnet/runtime/issues?q=is%3Aopen+is%3Aissue+label%3Aarea-System.Text.Json) o nei post [StackOverflow](https://stackoverflow.com/questions/tagged/system.text.json) .

Se si implementa una soluzione alternativa per uno di questi scenari e si può condividere il codice, selezionare il pulsante "**pagina**" nella parte inferiore della pagina. Questo crea un problema di GitHub e lo aggiunge ai problemi elencati nella parte inferiore della pagina.
...
* Tipi di raccolta nello spazio dei nomi <xref:System.Collections.Specialized>
