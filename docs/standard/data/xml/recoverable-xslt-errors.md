...
|--------------------|-------------|-----------------------------------|  
|Nodo di tipo text corrispondente sia a `xsl:strip-space` sia a `xsl:preserve-space`.|3.4|Recupera|  
...
|Il risultato della creazione di un'istanza del contenuto di `xsl:processing-instruction` contiene la stringa "?>"|7.3|Recupera|  
|Il risultato della creazione di un'istanza del contenuto di `xsl:processing-instruction` contiene la stringa "--" o termina con "-".|7.4|Recupera|  
...
|Esistono più attributi con lo stesso nome ma con valori diversi che non sono elementi sezioni cdata denominati in `xsl:output` e tali attributi hanno la stessa precedenza di importazione.|16|Recupera|  
|Il processore non supporta la codifica nell'attributo di codifica `xsl:output`.|16.1|Recupera|  
