...
ms.locfileid: "82102437"
---
# <a name="fundamentals-of-garbage-collection"></a>Principi fondamentali di Garbage Collection

In Common Language Runtime (CLR), il Garbage Collector (GC) funge da gestore automatico della memoria. Il Garbage Collector gestisce l'allocazione e il rilascio di memoria per un'applicazione. Per gli sviluppatori che lavorano con codice gestito, ciò significa che non è necessario scrivere codice per eseguire attività di gestione della memoria. La gestione automatica della memoria può eliminare i problemi comuni, ad esempio dimenticando di liberare un oggetto e causare una perdita di memoria o il tentativo di accedere alla memoria per un oggetto che è già stato liberato.
...
Il Garbage Collector offre i vantaggi seguenti:The garbage collector provides the following benefits:

- Libera gli sviluppatori dal dover rilasciare manualmente la memoria.

- Alloca gli oggetti nell'heap gestito in maniera efficiente.

- Recupera gli oggetti inutilizzati, ne cancella la memoria e tiene la memoria a disposizione per le future allocazioni. Gli oggetti gestiti ottengono automaticamente contenuto pulito per iniziare, in modo che i relativi costruttori non devono inizializzare ogni campo dati.
...
- Uno sviluppatore di applicazioni usa solo lo spazio degli indirizzi virtuali e non modifica mai direttamente la memoria fisica. Il Garbage Collector alloca e libera automaticamente la memoria virtuale nell'heap gestito.
  ...
  Se si scrive codice nativo, si usano le funzioni di Windows per lavorare con lo spazio di indirizzi virtuali. Queste funzioni allocano e liberano automaticamente la memoria virtuale negli heap nativi.
...
- La memoria utilizzata dagli oggetti allocati nell'heap gestito supera una soglia accettabile. Questa soglia viene continuamente modificata durante l'esecuzione del processo.

- Viene chiamato il metodo <xref:System.GC.Collect%2A?displayProperty=nameWithType> . In quasi tutti i casi, non è necessario chiamare questo metodo, perché il Garbage Collector viene eseguito continuamente. Il metodo viene usato principalmente in situazioni eccezionali e per scopi di test.
...
Minore è il numero di oggetti allocati nell'heap, minore sarà il lavoro del Garbage Collector. Quando si allocano oggetti, non utilizzare valori arrotondati per e l'secondo momento che superano le proprie esigenze, ad esempio l'allocazione di una matrice di 32 byte quando sono necessari solo 15 byte.

Quando viene attivata un'operazione di Garbage Collection, il Garbage Collector recupera la memoria occupata da oggetti morti. Durante il processo di recupero, gli oggetti attivi vengono compattati in modo da poter essere spostati insieme e lo spazio inutilizzato viene rimosso, riducendo le dimensioni dell'heap. In questo modo si garantisce che gli oggetti allocati insieme rimangano insieme nell'heap gestito per conservarne la posizione.
...
L'heap può essere considerato l'insieme di due heap: l'[heap degli oggetti grandi](large-object-heap.md) e l'heap degli oggetti piccoli. L'heap oggetti grandi contiene oggetti di 85.000 byte e più grandi, che sono in genere matrici. È raro che un oggetto istanza sia estremamente grande.
...
- Gli oggetti più recenti hanno durate più brevi e gli oggetti meno recenti hanno una durata maggiore.
- Gli oggetti più recenti tendono ad essere correlati tra loro e accessibili dall'applicazione nello stesso momento.

La Garbage Collection avviene principalmente con il recupero di oggetti di breve durata. Per ottimizzare le prestazioni del Garbage Collector, l'heap gestito è suddiviso in tre generazioni, 0, 1 e 2, in modo che possa gestire oggetti di lunga durata e di breve durata separatamente. Il Garbage Collector archivia i nuovi oggetti nella generazione 0.The garbage collector stores new objects in generation 0. Gli oggetti creati nelle prime fasi della durata dell'applicazione che non vengono raccolti vengono promossi e archiviati nelle generazioni 1 e 2. Poiché è più veloce compattare una parte dell'heap gestito rispetto all'intero heap, questo schema consente al Garbage Collector di rilasciare la memoria in una generazione specifica anziché rilasciare la memoria per l'intero heap gestito ogni volta che esegue una raccolta.
...
- **Generazione 0**. È la generazione più recente e contiene oggetti di breve durata. Un esempio di oggetto di breve durata è una variabile temporanea. Le operazioni di Garbage Collection vengono eseguite il più delle volte in questa generazione.
  ...
  La maggior parte degli oggetti vengono recuperati per la procedura di Garbage Collection nella generazione 0 e non sopravvivono alla generazione successiva.
  
  Se un'applicazione tenta di creare un nuovo oggetto quando la generazione 0 è piena, il Garbage Collector esegue una raccolta nel tentativo di liberare spazio degli indirizzi per l'oggetto. Il Garbage Collector esamina prima di tutto gli oggetti presenti nella generazione 0, anziché tutti gli oggetti presenti nell'heap gestito. Una raccolta di generazione 0 da sola spesso recupera memoria sufficiente per consentire all'applicazione di continuare a creare nuovi oggetti.
...
- **Generazione 1**. Questa generazione contiene oggetti di breve durata e funge da buffer tra gli oggetti di breve durata e gli oggetti di lunga durata.
  ...
  Dopo che il Garbage Collector esegue una raccolta di generazione 0, compatta la memoria per gli oggetti raggiungibili e li promuove alla generazione 1. Poiché la durata degli oggetti non raccolti è solitamente più lunga, la promozione a una generazione superiore risulta opportuna. Il Garbage Collector non deve riesaminare gli oggetti nelle generazioni 1 e 2 ogni volta che esegue una raccolta di generazione 0.The garbage collector doesn't have to reexamine the objects in generations 1 and 2 each time it performs a collection of generation 0.
  
  Se una raccolta di generazione 0 non recupera memoria sufficiente per l'applicazione per creare un nuovo oggetto, il Garbage Collector può eseguire una raccolta di generazione 1, quindi generazione 2. Gli oggetti presenti nella generazione 1 non raccolti vengono promossi alla generazione 2.
...
- **Generazione 2**. Questa generazione contiene oggetti di lunga durata. Un esempio di oggetto di lunga durata è un oggetto in un'applicazione server che contiene dati statici attivi per la durata del processo.
  ...
  Gli oggetti nella generazione 2 che sopravvivono a una raccolta rimangono nella generazione 2 fino a quando non sono ritenuti irraggiungibili in una raccolta futura.

Le operazioni di Garbage Collection vengono eseguite in generazioni specifiche a seconda delle condizioni. Raccogliere una generazione significa raccogliere gli oggetti in quella generazione e in tutte le generazioni più recenti. Un'operazione di Garbage Collection di generazione 2 è nota anche come Garbage Collection completa, perché recupera gli oggetti in tutte le generazioni, ovvero tutti gli oggetti nell'heap gestito.

### <a name="survival-and-promotions"></a>Esclusione e promozioni

Gli oggetti che non vengono recuperati in un'operazione di Garbage Collection sono noti come sopravvissuti e vengono promossi alla generazione successiva:

- Gli oggetti che sopravvivono a un'operazione di Garbage Collection di generazione 0 vengono promossi alla generazione 1.Objects that survive a generation 0 garbage collection are promoted to generation 1.
- Gli oggetti che sopravvivono a un'operazione di Garbage Collection di generazione 1 vengono promossi alla generazione 2.Objects that survive a generation 1 garbage collection are promoted to generation 2.
- Gli oggetti che sopravvivono a un'operazione di Garbage Collection di generazione 2 rimangono nella generazione 2.Objects that survive a generation 2 garbage collection remain in generation 2.
...
Poiché gli oggetti nelle generazioni 0 e 1 sono di breve durata, queste generazioni sono conosciute come *generazioni effimere.*
...
La dimensione del segmento temporaneo varia a seconda che un sistema sia a 32 bit o a 64 bit e dal tipo di Garbage Collector in esecuzione ([workstation o server GC](workstation-server-gc.md)). Nella tabella seguente vengono illustrate le dimensioni predefinite del segmento temporaneo.
...
- Una fase di compattazione in cui lo spazio occupato dagli oggetti inutilizzati viene recuperato e gli oggetti esclusi compattati. Durante questa fase, gli oggetti rimasti dopo un'operazione di Garbage Collection vengono spostati verso l'estremità meno recente del segmento.
  ...
  - Limite di memoria su un contenitore.
...
Per la maggior parte degli oggetti creati dall'applicazione, è possibile basarsi su Garbage Collection per eseguire automaticamente le attività di gestione della memoria necessarie. Per le risorse non gestite è tuttavia necessario il rilascio esplicito. Il tipo più comune di risorsa non gestita è rappresentato da un oggetto che esegue il wrapping di una risorsa del sistema operativo, quale un handle di file, un handle di finestra o una connessione di rete. Sebbene il Garbage Collector sia in grado di tenere traccia della durata di un oggetto gestito che incapsula una risorsa non gestita, non dispone di conoscenze specifiche su come pulire la risorsa.

Quando si crea un oggetto che incapsula una risorsa non gestita, è consigliabile fornire il `Dispose` codice necessario per pulire la risorsa non gestita in un metodo pubblico. Specificando un metodo `Dispose` si consente agli utenti dell'oggetto di liberarne esplicitamente la memoria dopo l'uso dell'oggetto. Quando si utilizza un oggetto che incapsula una `Dispose` risorsa non gestita, assicurarsi di chiamare in base alle esigenze.

È inoltre necessario fornire un modo per il rilascio delle risorse non `Dispose`gestite nel caso in cui un consumer del tipo si dimentichi di chiamare . È possibile usare un handle sicuro per eseguire il <xref:System.Object.Finalize?displayProperty=nameWithType> wrapping della risorsa non gestita o eseguire l'override del metodo.

Per ulteriori informazioni sulla pulizia delle risorse non gestite, vedere Pulire le [risorse non gestite.](unmanaged.md)
