...
- *ModuleName* è il nome di un modulo di codice .NET Framework (un file con estensione *netmodule* ) che include uno o più tipi. È possibile creare un file con *estensione netmodule* compilando il codice con l' `/target:module` opzione in C# o Visual Basic.
...
Nell'esempio seguente viene usato l' <xref:System.Reflection.AssemblyKeyFileAttribute> attributo con un file di chiave denominato Key *file. snk*, che si trova nella directory in cui viene compilato l'assembly.  
...
Nell'esempio seguente viene usato il compilatore C# e viene firmato l'assembly *UtilityLibrary. dll* con un nome sicuro usando il file di chiave *sgKey. snk*.  
