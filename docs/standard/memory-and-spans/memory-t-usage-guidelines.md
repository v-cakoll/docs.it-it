...
- **Leasing**. Il lease è il periodo di tempo per cui un particolare componente è autorizzato a essere il consumer del buffer.
...
Possiamo anche scrivere questo [`using`](../../csharp/language-reference/keywords/using-statement.md)esempio con:
...
**#1 della regola: per un'API sincrona, usare Span\<T> anziché Memory\<T> come parametro, se possibile.**
...
**#2: utilizzare ReadOnlySpan\<T>\<o ReadOnlyMemory T> se il buffer deve essere di sola lettura.**
...
**Regola #3: se il\<metodo accetta `void`Memory T> e\<restituisce , non è necessario utilizzare l'istanza di Memory T> dopo la restituzione del metodo.**
...
**Regola #4: se il metodo\<accetta un'> Memory T e\<restituisce un'attività, non è necessario utilizzare l'istanza di Memory T> dopo la transizione dell'attività a uno stato terminale.**
...
**Regola #5: se il\<costruttore accetta Memory T> come parametro, si presuppone\<che i metodi di istanza sull'oggetto costruito siano consumer dell'istanza di Memory T>.**
...
**Regola #6: se si dispone\<di una proprietà di memoria impostabile T> (o di un metodo di istanza\<equivalente) sul tipo, si presuppone che i metodi di istanza su tale oggetto siano consumer dell'istanza di Memory T>.**
...
**Regola #7: se si dispone\<di un riferimento A> IMemoryOwner, è necessario eliminare a un certo punto o trasferirne la proprietà (ma non entrambi).**
...
**Regola #8: Se si dispone\<di un parametro T> IMemoryOwner nella superficie dell'API, si sta accettando la proprietà di tale istanza.**
...
In base alla regola 1, <xref:System.Span%601> è in genere il tipo corretto da usare per le API sincrone. È possibile <xref:System.Span%601> bloccare [`fixed`](../../csharp/language-reference/keywords/fixed-statement.md) le istanze tramite la parola chiave , come nell'esempio seguente.
